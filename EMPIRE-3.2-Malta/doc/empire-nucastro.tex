...
consecutive numbers without checking the discrete level numbering in the corresponding EMPIRE file (
$file.lev).
...
\part{NUCLEAR ASTROPHYSICS\label{Chap:S-FACTOR}}

   Although EMPIRE is a model code used primarily for nuclear data evaluation, it has the inherent capabilities to be used as a nuclear reaction model code for nuclear astrophysics applications.  In that vein, the S-factor has been implemented into the EMPIRE code.  The formalism that follows is valid for nonresonant reactions only (follows Cauldrons in the Cosmos by Claus E. Rolfs and William S. Rodney - University of Chicago Press 1988).
...
\begin{equation}
\S(E)=\sigma(E) E \exp[2\pi\eta]\end{equation}

\linebreak where \sigma(E) is the cross section in b
\linebreak E is the center of mass energy in keV
\linebreak 2\pi\eta = 31.29 Z$_{p}$Z$_{t}$\sqrt{\frac{\mu}{E}}
\linebreak \mu = \frac{M$_p$*M$_t$}{M$_p$ + M$_t$}
\linebreak Z$_p$ = charge of the projectile
\linebreak Z$_t$ = charge of the target

\linebreak The assumption is that within the Gamow window (E = E$_0$ \pm \frac{delta}{2} - the narrow burning regime where reactions take place at a given stellar temperature T), the S-factor is constant (no energy dependence or very nearly a constant).  If S(E) \neq S(E$_{0}$),  then there is an energy dependence (and provided it is a slowly varying function of energy), then for the above formalism approximation methods can be applied.
The current calculation of the S-factor in EMPIRE follows the above formalism and can output both gammas or neutrons in the exit channel (with either an \alpha or proton in the incident channel).  
