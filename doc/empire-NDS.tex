...
\author{B.V. Carlson}
\affiliation{Instituto Tecnológico de Aeronáutica, Sao Jose dos Campos SP, Brazil}
...
When the partial wave expansion is substituted in the optical Schrö\-din\-ger
...
in the optical Schrö\-din\-ger equation, the latter can be reduced
...
\psi_{l^{\prime}j^{\prime}c^{\prime},ljc}^{J}(r)\rightarrow\Psi_{J}(r)\,, & \; & {\mathcal{U}}_{l^{\prime}j^{\prime}c^{\prime},ljc}^{J}(r)\rightarrow U_{J}(r)\,,\nonumber \end{eqnarray}
 the Schrödinger equation can be recast as
...
p(E,l)=\int_{-\infty}^{\infty}f(B')T_{l}(E-B')dB'\,.\end{equation}
 The transmission coefficient in the Hill-Wheeler approach \cite{HillWheeler}
reads \begin{equation}
T_{l}(E)=\left\{ 1+\exp\left[-2\pi(E-E_{rot})/\hbar\omega\right]\right\}^{-1},\end{equation}
 with $E_{rot}$ being the rotation energy at angular momentum $l$. The
distributed fusion-barrier option in EMPIRE allows for an extra-push
energy that can be specified in the input and added to the fusion
barrier. The latter, if not specified in the input, is by default
calculated using the BAR subroutine of CCFUS (see above). 
\item The fusion cross sections for each $l$ can be read from an external file.
The code converts them into transmission coefficients to be used in
Eq.~\ref{fusion}. \\
