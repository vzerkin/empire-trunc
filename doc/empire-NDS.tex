...
begining of the unresolved resonance region for neutron induced reactions ($%
\backsim$ 10 keV) and goes up to few hundred MeV for Heavy Ion induced
reactions.

The code accounts for the major nuclear reaction mechanisms, such as optical
model, Coupled Channels and DWBA code (ECIS03), deformation dependent
Multi-step Direct (ORION + TRISTAN), NVWY Multi-step Compound,
preequilibrium exciton model with cluster emission (PCROSS) and another one
with full angular momentum coupling (DEGAS) and the full featured
Hauser-Feshbach model with $\gamma$-cascade and widths fluctuations.
Advanced treatment of the fission channel takes into account transmission
through a triple humped fission barrier with absorption in the wells. The
fission probability is derived in WKB approximation within optical model for
fission. This formalism is capable of describing the complex resonant
structure in the neutron induced fission on light actinides. Several options
for nuclear level densities include EMPIRE-specific approach accounting for
effects of the dynamic deformation of a fast rotating nucleus, the classical
Gilbert-Cameron, and the tables pre-calculated with a microscopic model.
Heavy Ion fusion cross section can be calculated within the simplified
coupled channels approach (CCFUS).
...
schemes, level densities, fission barriers, moments of inertia (MOMFIT), and
$\gamma$-ray strength functions. The results can be converted into the
ENDF-6 formatted files using the accompanying code EMPEND and completed with
neutron resonances extracted from the existing evaluations. Capabilities of
generating covariances, using both KALMAN and Monte Carlo methods, will be
future extensions of the system and are still being advanced and refined.
The package contains the full EXFOR library of experimental reaction data
that are automatically retrieved during the calculations. Publication
quality graphs can be obtained using the powerful and flexible plotting
package ZVView. The graphic user interface, written in Tcl/Tk, provides for
...
exciton model \cite{Griffin:66}, which is based on the solution of the master
equation \cite{Cline:71} in the form proposed by Cline \cite{Cline:72} and
Ribansky \cite{Ribansky:73}:
\begin{eqnarray}
...
\cite{Akkermans:80}, which produces an exact result for any initial condition of
the problem. We are interested in the preequilibrium contribution only, so
the \textit{never-come-back} assumption is used meaning that $\lambda
_{+}(E,n)\gg \lambda _{-}(E,n)$ and only transitions increasing the
complexity of the system (and therefore the exciton number) are allowed.
Integrating the master equation \ref{master} over time we can obtain its
solution $\tau (n)$, which represents the time during which the system
remains in the state characterized by $n$ excitons. The preequi1ibrium
...
requiring a blind description of fission for nuclei far from the stability valley.
\begin{figure}[htbp]
...
The fission probability is usually defined as the ratio of the
...
To expedite a long chain of operations to be executed once ENDF-6 formatting is completed the EMPIRE system incorporates a number of stand-alone codes:
\begin{itemize}
...
\item \textbf{FIXUP\index{FIXUP}}: used to perform additional checks on the data,
...
RECENT, LINEAR and SIGMA1 codes will be used to reconstruct pointwise cross
