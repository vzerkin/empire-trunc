...
\backsim$ keV) and goes up to few hundred MeV for Heavy Ion induced
reactions.

The code accounts for the major nuclear reaction mechanisms, including
direct, pre-equilibrium and compound nucleus. Direct reactions are described
by a generalized optical model (ECIS03) or by the simplified coupled
channels approach (CCFUS). The preequilibrium mechanism could be treated by
a deformation dependent Multi-step Direct (ORION + TRISTAN), NVWY Multi-step
Compound, preequilibrium exciton model with cluster emission (PCROSS) and
another one with full angular momentum coupling (DEGAS). Finally the
...
multiple-humped fission barriers with absorption in the wells. The fission
probability is derived in WKB approximation within optical model for
fission.

Several options for nuclear level densities include EMPIRE-specific approach
accounting for effects of the dynamic deformation of a fast rotating
nucleus, the classical Gilbert-Cameron approach and the tables
pre-calculated with a microscopic model based on HFB single-particle level
...
schemes, level densities, fission barriers, moments of inertia and $\gamma $%
-ray strength functions.

The results can be converted into the ENDF-6 formatted files using the
accompanying code EMPEND and completed with neutron resonances extracted
from the existing evaluations. Capabilities of generating covariances, using
both KALMAN and Monte Carlo methods, are still being advanced and refined
and will be future extensions of the system. The package contains the full
EXFOR library of experimental reaction data that are automatically retrieved
...
This paper describes capabilities of the code, outlines physical models and
indicates parameter libraries used by EMPIRE to predict reaction cross
sections and spectra, mainly for nucleon induced reactions.
\end{abstract}
...
Roots of the EMPIRE nuclear reaction model code originate at the Institute
...
In 2003 EMPIRE has found a new home at the National Nuclear Data Center at
...
write-up is to present capabilities of the system and explain means it is
...
The current 3.0 version of the EMPIRE is a milestone in the evolution of the
...
The flowchart shown in Fig.~\ref{fig:empire-flowchart} illustrates
...
{fig:physics-core} shows sequence of calculations and a simplified flow of
the data. The first module reads an input file with specification of a
reaction and retrieves all model parameters necessary in the calculations.
...
done in terms of the spherical optical model, Coupled Channel or Distorted
...
that are used to treat emission of particles, clusters and $\gamma $'s from
the composite nucleus before it reaches thermal equilibrium and becomes the
...
DEGAS and PCROSS, and the Monte Carlo simulation (DDHMS)). Presence of so
...
{tbl:PEcapabilities}, demonstrates that no module provides full set of
...
only one to properly account for the multiparticle PE emission. On the other
hand, lack of the exclusive spectra excludes this model from nuclear data
...
latter. In order to maximize advantages EMPIRE allows for simultaneous use
of several models. In such a case, internal set of rules is invoked to
...
ECIS03%
\index{ECIS} can be invoked in four different ways. For all cases $T_{l}$
...
level density at a given excitation energy. This decrease or leave the $ph-$%
...
1p-1h $ reduced parts of the two Green functions.
...
built from correlated basis states. This justifies also the \textit{ansatz}
of Eq. \ref{hintr} where $V^{intr}$ was defined as to act only between $ph-$%
classes.
...
\ref{sgrpa} is used also in two-step reactions at the appropriate excitation
...
coupling constant is used by default. Also in $\lambda=0$ and $\lambda=1$
cases user may choose to provide the respective energies in the input. Each
...
\index{p-h level densities}, $p$ and $h$ are number of particles and holes ($%
...
The module PCROSS includes a preequilibrium mechanism as defined in the
exciton model \cite{Griffin:66}, which is based on the solution of the
...
difference to the DEGAS treatment. To obtain the numerical solution of the
system of the algebraic equations for $\tau (n)$ given by equation \ref%
{master}, we use the algorithm proposed by Akkermans, Gruppelaar and Reffo
\cite{Akkermans:80}, which produces an exact result for any initial
condition of the problem. We are interested in the preequilibrium
contribution only, so the \textit{never-come-back} assumption is used
meaning that $\lambda _{+}(E,n)\gg \lambda _{-}(E,n)$ and only transitions
increasing the complexity of the system (and therefore the exciton number)
are allowed. Integrating the master equation \ref{master} over time we can
obtain its solution $\tau (n)$, which represents the time during which the
system remains in the state characterized by $n$ excitons. The
preequi1ibrium spectra can be calculated as
\begin{equation}
\frac{d\sigma _{a,b}}{d\epsilon _{b}}(\epsilon _{b})=\sigma
_{a,b}^{r}(E_{inc})D_{a,b}(E_{inc})\times  \notag \\
\sum_{n}W_{b}(E,n,\epsilon _{b})\tau (n),
\end{equation}
where $\sigma _{a,b}^{r}(E_{inc})$ is the cross section of reaction \textit{%
...
cross\bigskip\ section $\sigma _{a,b}^{dir}(E_{inc})$, we can calculate the
depletion factor in the following way
\begin{equation}
D_{a,b}(E_{inc})=1-\frac{\sigma _{a,b}^{dir}(E_{inc})}{\sigma
_{a,b}^{r}(E_{inc})},
\end{equation}

Let's give a brief description of the main components of the exciton model
...
In the PCROSS code we use the parametrization of transition rate proposed by
...
the internal transition rates the expressions found by Machner \cite%
{Machner:81}:
\begin{equation}
...
Fig.~\ref{goldna} illustrates an essential improvement brought by the
Iwamoto-Harada model to the interpretation of the $\alpha$-particle emission
...
used in recent evaluations. Combined with direct contribution described in
...
\caption{EMPIRE calculations of the neutron spectra emitted at 30 degrees
...
M. Chadwick and P. Oblo\v{z}insk%
\'{ }y (DDHMS), the model provides a nearly complete set of observables.
...
Figure~\ref{pb208HMS} presents effect of the HMS contribution on spectrum of
...
\index{DEGAS} is missing angular distributions, which are calculated by the
...
\pi^{\prime},\pi_{i})}$. The parity selection rules are implicit in Eq. \ref%
{pwidth}.
...
User may increase the maximum multipolarity from the default value of 2 (E1,
M1 and E2 transitions only) up to the maximal value of 10. This change
affects calculations of $\gamma$-transitions between states in the continuum
...
\end{equation}
\noindent where $C=[R/(\hbar c)]^2 = 3.7*10^{-5}A^{2/3}$ if nuclear radius $%
...
revealed by the recent large scale experimental and theoretical studies. It
...
(i) The barriers associated with the discrete transition states are
...
\end{eqnarray}
where $N$ is the number of humps and the number of wells ($j$ runs from 2 to
$N$ because the first well, corresponding to equilibrium deformation, is not
included in the parametrization). The energies $E_{fi(j)}$ represent maxima
and minima of the potential, $\beta_{i(j)}$ are the corresponding abscissae,
the harmonic oscillator frequencies $\omega_{i(j)}$ define the curvature of
each parabola and $\mu$ is the inertial mass parameter, assumed independent
of $\beta$ and approximated by the semi-empirical expression $%
\mu\approx0.054A^{5/3}$ MeV${}^{-1}$, where $A$ is the mass number.

In the following, the humps will be denoted with capital letters and the
...
\label{tsrot}
\end{eqnarray}
where: $\epsilon_{i(j)}(K,\pi)$ are the band-head energies and $%
...
within wells is simulated by introducing negative imaginary potentials, $%
iW_j $, in the corresponding deformation ranges (Fig.\ref{fis-bar}). This
causes absorption of the incoming flux in this wells ~\cite{Bjornholm:80,
Bhandari:79, Back:74}. The strengths $W_j$ are assumed to be quadratic
functions of deformation, $\beta$, like the real part, and to be energy
dependent$:$
\begin{equation}
...
{fis-multimod} for two modes known as super-long symmetric (SL) and standard
asymmetric (S1).
...
(ii) Comprehensive microscopic Hartree-Fock-Bogolyubov (HFB) calculations
...
and integration have been implemented in order to use numerically defined
...
\newline
\qquad (iii) undergo $\gamma $-transition to the isomeric state; the
...
in terms of the momentum integrals for the humps~\cite{Froman:65}:
\begin{equation}
K_{i}=\pm\left\vert
...
\end{equation}
As it is known, in the case of a single parabolic barrier, formula (\ref%
...
the above equation. For example, in case of a triple-humped barrier, in Eq.%
...
For the indirect fission coefficient, the way the sum rule applies depends
...
The significant difference appears between calculations considering partial
...
%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
In most of the cases, there are no available information about the barriers'
...
Prompt fission neutron (PFN) spectra and average number of prompt fission
neutron ($\overline{\nu}$) calculations are important additions that make the
evaluated files for actinides complete. Furthermore, both $\overline{\nu}$ and PFN
spectra are also consistent with the calculated fission cross sections using
the models described above. The calculated PFN spectra were recently added
to the EMPIRE based on a modified Los Alamos model considering both
...
Pre-fission neutron spectra are derived from the EMPIRE calculated exclusive
...
\index{level densities} and includes several respective parametrizations. In
...
representation depends on a case being considered. For the nucleon induced
reactions, with CN excited up to about 20 MeV, the Gilbert-Cameron approach
...
Ejectile emission energy is denoted by $\varepsilon$, excitation energy by $%
...
dispersive potential for actinides (2408 and 4608) and global dispersive
...
\index{HF-BCS level densities} are part of the RIPL%
...
\index{RIPL}-3 library. The level densities were calculated using
Hartree-Fock-Bogoliubov model combined with collective model for the
...
These new level densities are intended to replace HF-BCS level densities
tabulated in RIPL-2.
...
The list of Tab.~\ref{endf7list} includes 19 materials considered to be
...
analysis and design of advanced fuels. Evaluations for these priority
...
In case of Ge, Nd, Sm, Gd and Dy evaluations were performed for complete
...
A few ENDF/B-VII.0 related works carried out at BNL have been published in
...
^{231}$Pa that makes modeling of the fission channel much more complex. The
...
data so well (see Fig.~\ref{231Pa}) that ENDF/B-VII.0 evaluations simply
...
Recently a new evaluation of \ the neutron induced reactions on tungsten
isotopes up to 60 MeV has been presented \cite{catrko07}. Accurate nuclear
data for tungsten isotopes are required because tungsten is a candidate for
material for first-wall components in fusion devices, target material for
systems based on high-current accelerators, as well as neutron dosimetry
with the $^{186}$W(n,$\gamma$) reaction. In spite of some recent attempts to
...
on $^{180-186}$W nuclei were obtained from the isospin-dependent dispersive
coupled-channel OMP \cite{casoqu06,casitr06}. The same potential was used to
calculate direct excitation of the collective levels in the continuum by
DWBA method up to about 10 MeV excitation energy. Pre-equilibrium emission
was considered using one-component exciton model PCROSS, which includes
nucleon, gamma and cluster emission

Total, capture, emission cross sections of neutrons and charged particles,
...
of all low energy cross sections has been achieved.

\begin{figure}[tbph]
\scalebox{0.35}{\rotatebox{-90}{\includegraphics{w186_XS.eps}}}
\caption{Evaluated cross sections of $^{186}$W below 5 MeV \protect\cite%
{catrko07}. Shown are total, elastic, inelastic, scattering to the first two excited levels
and the capture.}
\label{w186_XS}
\end{figure}
...
provides also direct comparison of EMPIRE and TALYS results.
...
Radiopharmaceuticals play an important role in modern medicine in terms of
both diagnostic investigations and therapeutic treatment. The role of
...
Ignatyuk for theoretical modeling of the studied reactions within this CRP.
As an example, one of the reactions under study for the production of
beta-emitting $^{186}$Re radioisotope by a proton induced reaction on $%
...
function associated to the preequilibrium emission and direct processes in
...
radioisotope $^{114m}$In. Experimental data and results of the EMPIRE
calculations are shown in Fig.\ref{fig:cd116_p3n}. The agreement is not perfect,
but calculation of the production of an isomer after emission
...
innovative way of production of~certain medical radioisotopes. Applications
...
The ease of use, combined with the power of parallel super-computing, open
...
playing a unique role in current nuclear data evaluation efforts.
...
mechanism. M. Mebel should be thanked for providing the routine for
...
Ph. Young, M.B. Chadwick, A. Koning, S. Hilaire, S. Goriely and T. Kawano
