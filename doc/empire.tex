...
computational format file by MAT/MF/MT numbers and incident energy
...
in PLOTTAB format (written by A.Trkov)~\cite{ENDVER},
\item [{\textbf{SIXTAB\index{SIXTAB}}}] converts ENDF file MF6 to Law
7 representation (written by A.Trkov)~\cite{ENDVER},
...
ENDF-6 formatted file (written by Ch. Dunford)
...
\item [{\textbf{LINEAR}}] converts MF=3 (cross sections) of the ENDF-6
file into a linear-linear interpolable form (written by D.E. Cullen)~\cite{PREPRO}
...
cross section in linearly interpolable form (written by D.E. Cullen)~\cite{PREPRO}
...
\item [{(i)}] the simplified Coupled-Channels\index{Coupled-Channels}
...
\item [{(ii)}] the distributed fusion-barrier\index{distributed fusion-barrier}
...
(see above) .
\item [{(iii)}] the fusion cross sections for each $l$ read from the external
...
\begin{labeling}{00.00.0000}
\item [{(iv)}] the total fusion cross section specified in input (single
incident energy only).
\item [{(v)}] the critical angular momentum $l_{cr}$ for compound nucleus
...
(329 materials) extracted from the ENDF/B-VI.8 library. The file is
...
For example, calculations with the input file \emph{Mo100.inp} can
