...
\noindent where $\sigma(E)$ is the cross section in b   
and E is the center of mass energy in keV 

\center {$2\pi\eta = 31.29Z$_{p}$Z$_{t}${\sqrt{\frac{\mu}{E}}}$}

\center {\mu = M$_p$M$_t$/(M$_p$ + M$_t$)} 

\noindent Z$_p$ is the charge of the projectile  and 
Z$_t$ is the charge of the target

\par The assumption is that within the Gamow window ($E = E$_0$ \pm \frac{\Delta}{2}$ which is the narrow burning regime where reactions take place at a given stellar temperature T), the S-factor is constant (no energy dependence or very nearly a constant).  If S(E) $\neq$ S(E$_{0}$),  then there is an energy dependence (and provided it is a slowly varying function of energy), then for the above formalism approximation methods can be applied.
