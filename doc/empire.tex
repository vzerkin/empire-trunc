...
%
\textsf{\normalsize 3) Jo\v{z}ef Stefan Institute, Ljubljana, Slovenia}\\
%
\textsf{\normalsize 4) University of Bucharest, Bucharest, Romania}\\
%
...
transition from superfluid behaviour at low energy~\cite{75IS,83I}, 
...
{74B} (see fission level densities in Section~\ref{fission}).
\\
...
{bsk14} to construct incoherent particle-hole (ph) state densities $\omega
...
expressed relative to the quadrupole mode~\cite{ig85}, leading to a similar
expression, i.e, 
\begin{equation}
...
\begin{figure}[htbp]
\begin{center}
\scalebox{0.9}{\includegraphics{emp-fis-bar-man.eps}}
...
\begin{figure}[htbp]
\begin{center}
\scalebox{0.7}{\includegraphics{emp-fis-mod-man.eps}}
...
The fission paths have been estimated within the HFB model with BSk14 Skyrme force \cite{Goriely:07-mass}
...
\begin{figure}[htbp]
\begin{center}
\scalebox{0.45}{\includegraphics{emp-fis-tot-bar.eps}}
...
It should be noted that the uncertainty of above estimates is significant~\cite{80B2},
...
In \cite{si} is presented a recursive method to calculate the transmission coefficients through a barrier 
...
\begin{figure}[htbp]
\begin{center}
\scalebox{1.0}{\includegraphics{emp-fis-mech.eps}}
...
\begin{figure}[htbp]
\begin{center}
\scalebox{0.7}{\includegraphics{emp-fis-ld0.eps}}
...
\begin{figure}[htbp]
\begin{center}
\scalebox{0.7}{\includegraphics{emp-am41-surr.eps}}
...
%##############################################################
\label{pfns}
The Madland-Nix or Los Alamos model~\cite{Madland:1982a} constitutes the basis for the evaluation
...
The Los Alamos (LA) model is described at length in Ref.~\cite{Madland:1982a},
...
Weisskopf statistical evaporation theory~\cite{Weisskopf:1937a} is used to explain the emission of 
...
\begin{itemize}
\item Nuclear reaction model code EMPIRE~\cite{empire:2007},
...
parameters~\cite{RIPL-3}. The code is also suitable for massive
...
to the Bayes theorem as shown in Ref.~\cite{Muir:89}.
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{Y89-covar-plots/plot-dcs.eps}
...
\item [{PFNTKE}] Used in prompt fission neutron (PFN) calculations.
                 ...
                 fission energy release (or measured TKE).\\
                 The relative uncertainty of the scaling factor in \% could be given by I1.
                
\item [{PFNALP}] Used in prompt fission neutron (PFN) calculations.
                 The default parameter $\alpha$ ($\alpha_0=1$ for Madland-Nix (LA) model and
                 $\alpha_0~0.9$ for Kornilov parameterization) will be multiplied by VAL (default: 1.).
...
\item [{PFNRAT}] Used in prompt fission neutron (PFN) calculations.
                 The default parameter $r=T_f^L/T_f^H$ ($r_0=1$ for Madland-Nix (LA) model and
                 $r_0=1.248$ for Kornilov parameterization) will be multiplied by VAL (default: 1.).
...
\noindent The  {*}-inp.fis file must be deleted manually if one of the following changes has been done in main input:
\begin{itemize}
...
\noindent The {*}-inp.fis file is divided into sections corresponding to each fissioning
...
and uses the new deformations which are printed in *-fiss.out but not in *-inp.fis. Therefore
...
\end{itemize}
The next lines contain the same information presented in Example 1. 
...
----------------------------------------
 ...
 Nr.parabolas =3      Nr.wells=1
   ...
   5.000    .900   5.670    .600   2.000   1.000
 ...
 ======================================================================
    Number  of  discrete states at hump 1 = 1
 Kdis  Pidis   Edis    homega
  .0    1      .000     .900
    Number  of  discrete states at hump 2 = 1
 Kdis  Pidis   Edis    homega
  .0    1      .000     .600
    Number  of  discrete states in well 3 = 1
 Kdis  Pidis   Edis    homega
  .0    1      .000    1.000
 
 ======================================================================
  Quantities used only if FISDEN=1 to calculate LD at  saddles
 ======================================================================
              Asym shellcorr Ushif    gamm  atilf/atil   Ecf     VIB1/2   VIBdt    VIBnor
  Barrier 1    2    2.600     .300     .600    1.000     .000    1.000     .100    1.000
  Barrier 2    3     .140     .300     .600    1.000     .000    1.000     .100    1.000
 
 ======================================================================
   Coefficients used only if FISDEN=2 to adjust HFB LD at saddles 
 ======================================================================
             Asym    Delta    alpha   Norm
 Barrier  1    2     .000     .000    1.000
 Barrier  2    3     .000     .000    1.000
...
\begin{figure}[tb]
 \centering
 \includegraphics[width=\columnwidth]{tot-Vv-shift.eps}
