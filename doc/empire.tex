...
= 0 Transmission coefficients calculated during the first run are stored, and reused
    in subsequent EMPIRE runs (default),
       
> 0 Transmission coefficients are calculated independently for each run; no reusage
    is allowed. This option is useful to calculate some quantities that are only used
    if TL are not already present (e.g. Bass fusion barrier in HI induced reactions).
    However, this potion slows down the execution of the code in subsequent runs up
    to a factor of 10 times (additional time is needed to calculate TLs again; reading
    them is much faster.
