...
A projectile can be a photons a nucleon, a light or
...
and DWBA (ECIS06), Multi-step Direct (ORION + TRISTAN), NVWY Multi-step
...
2001 and replaced with the ECIS03 version in EMPIRE-3.1. Symmetric
...
Complete EMPIRE execution consists of the following operation sequences,
which are described in more detail below:

\begin{itemize}
\item "runE" sequence prepares data and runs the main calculation.
...
\item Files from a previous run are deleted.

\item EMPIRE is executed; case related files are moved to the appropriate
  directories.

\item If the experimental data in computational C4 format are not available,
  they are copied or generated from the EXFOR file.
\end{itemize}
...
\item FIXUP is executed to reconstruct the (n,p) and (n,alpha) reactions
  ...
  is needed for easier visualisation of the results. The input file
...
\item ENDRES is executed (conditionally, if the file *-res.endf is present
  ...
  file. 
  The lower energy of the elastiv angular distribution is
...
\item STANEF is executed to update the dictionary section in the new
  ENDF file. The input file is generated within the script.
\end{itemize}
...
*.checkr, *.fizcon and *.psyche for the corresponding checking code.


\subsection{"process" sequence}

The Pre-Pro codes and some of the codes from the ENDVER package are
executed to prepare the data for plotting. The following codes and
scripts are executed on the ENDF file:

\begin{itemize}
\item LINEAR is used to linearise any cross sections, which are not be
