...
class MF31(MF_base):
    ...
    def writeMF31(self,filename):
        ...
        while len(tmplist) >= 6:
            fout.write( super(MF31,self).writeENDFline( tuple(tmplist[:6]), self.MAT, self.MF, self.MT, lineN ) )
            lineN += 1
            tmplist = tmplist[6:]
        fout.write( super(MF31,self).writeENDFline( tuple(tmplist), self.MAT, self.MF, self.MT, lineN ) )
