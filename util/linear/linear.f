...
C-----FORM FACTORS AND SCATTERING FUNCTIONS?
...
C       3456
      DATA PROGDOC/
     1 ' ***************** Program LINEAR (VERSION 2010-1) ***********',
...
C-----OUTPUT MINIMUM DATA OF INTEREST.
      CALL OUT9(XCMIN,PROGDOC1(27,2),3)
...
C-----ASCENDING ORDER).
   ...
   20 IZA = C1H
      CALL ZAHOL(IZA,ZABCD)
      WRITE(OUTP,270) ZABCD,MATH,MFH,MTH,N2IN,(NBT(I),INT(I),I=1,N1)
      WRITE(*   ,270) ZABCD,MATH,MFH,MTH,N2IN,(NBT(I),INT(I),I=1,N1)
      CALL ENDIT
C
...
C-----ORIGINAL ENERGY POINTS.
      IF(MFH.NE.3.OR.NOTHIN.GT.0) GO TO 70
c-----SKIP LEADING CROSS SECTION = 0 POINTS - WARNING < 0 O.K.
      IF(YOUT(1).NE.0.0D+0) GO TO 60
   40 KSTART = KSTART + 1
...
C-----CROSS SECTION IF THERE IS NOT ONE THERE ALREADY.
   60 IF(XOUT(1).LE.ESTART.OR.YOUT(1).EQ.0.0) GO TO 70
      XOUT(2)=XOUT(1)
      YOUT(2)=YOUT(1)
      YOUT(1)=0.0
      N2CORE=2
...
C-----SET UP LOOP OVER INTERPOLATION RANGES.
   70 NR2=KSTART   ! 10/05/18 - CHANGED TO SKIP = 0 NEAR START
      DO 260 IR=1,N1
...
C-----ELIMINATE DUPLICATE POINTS.
      IF(XOUT(N2CORE).NE.XOUT(N2CORE-2)) GO TO 90
...
C-----NOT FOR MF = 9 AND 10
     ...
     1   XOUT(N2CORE).GE.ETHERMAL) GO TO 120
...
C-----CORE SPACE FOR ONE MORE POINT REQUIRED. IF NO ROOM IN CORE, THIN
C-----AND UNLOAD ONE PAGE OF POINTS TO SCRATCH.
  120 IF(N2CORE.GE.NPAGE2) CALL THINIT
      N2CORE=N2CORE+1
      N2P1=N2CORE+1
      N2P2=N2P1+1
      XOUT(N2CORE)=XEND
...
C-----NO THERMAL FOR MF = 9 OR 10
      IF(MFH.EQ.9.OR.MFH.EQ.10) GO TO 160
      IF(XTOP.GT.ETHERMAL.AND.XOUT(N2CORE).LT.ETHERMAL)
     1 XTOP=ETHERMAL
  160 IF(XTOP.GT.2.0D+00*XOUT(N2CORE)) XTOP=2.0D+00*XOUT(N2CORE)
      IF(XTOP.GE.XEND) GO TO 170
...
C     INTEREST.
C
      IF(DABS(YN2).LE.XCMIN.AND.DABS(YN2P2).LE.XCMIN) GO TO 230
C
C     DEFINE ENERGY AT MIDDLE OF INTERVAL.
C
C-----INTYPE IS NOW 3, 4, 5 OR 6.
  180 IF(INTYPE-4) 200,190,200
C-----LINEAR ENERGY.
  190 XN2P1=HALF*(XN2+XN2P2)
      GO TO 210
C-----LOG ENERGY.
  200 XN2P1=DSQRT(XN2*XN2P2)
C-----ROUND MIDPOINT.
  210 CALL INCORE9(XN2P1)
C
...
C-----SPACING ONLY KEEP THE TWO ENDS OF THE INTERVAL.
      IF(XN2P1.LE.XN2.OR.XN2P1.GE.XN2P2) GO TO 230
...
C-----KEEP THE TWO ENDS OF THE INTERVAL.
      IF(DABS(YN2).LT.XCMIN.AND.DABS(YN2P2).LT.XCMIN) GO TO 230
C
...
C-----IN ORDER TO ALLOW ACCURATE BACKWARD THINNING BEFORE OUTPUT.
      IF(DABS(YN2P1-YAPROX).LE.DABS(YN2P1*ERRXC3)) GO TO 220
C
...
C-----CORE SPACE FOR ONE MORE POINT REQUIRED. IF NO ROOM IN CORE, THIN
C-----AND UNLOAD ONE PAGE OF POINTS TO SCRATCH.
  230 IF(N2CORE.GE.NPAGE2) CALL THINIT
...
C-----MOVE TO NEXT INTERVAL.
  240 IF(XOUT(N2CORE).LT.XEND) GO TO 170
C-----IF RANGE SAVED, RESTORE POINT AND RESET FLAG
      IF(IMSAVED.LE.0) GO TO 250
      IMSAVED=0
      XEND=XSAVE
      YEND=YSAVE
      GO TO 150
...
C-----REACTION WITH A NEGATIVE Q-VALUE.
      IF(IM6.GT.0.AND.Q.LT.0.0) WRITE(OUTP,280)
      RETURN
  270 FORMAT(2X,10A1,I5,I4,I4/
     1 '  Error in Interpolation Law---Execution Terminated'/
     2 '  N2 =',I6/'   NBT  INT'/(I6,I5))
  280 FORMAT(19X,'WARNING - the Above Section Uses Interpolation Law 6'/
...
C-----SECTION.
   ...
   40 CALL OUT9(TEMP,FIELD(1,1),  3)
      CALL OUT9(Q   ,FIELD(1,2),  3)
...
C-----CONVERT MINIMUM CROSS SECTION FROM CHARACTER TO FLOATING POINT.
   20 CALL IN9(XCMIN,FIELD(1,1))
   30 IF(MODGET.LE.0) MODGET=0
   ...
   50 IF(NOTHIN.EQ.0) WRITE(OUTP,360)
...
C
      ...
      WRITE(   *,110) NAMEIN
  110 FORMAT(//' ERROR - Opening ENDF/B data file'/1X,A72//)
      CALL ENDIT
      ENDIF
C
...
C
      ...
      READ(INP,320,END=180,ERR=180) ENER3(1),ER3(1)
      GO TO 190
  180 ISTAT1   = 1
      ENER3(1) = 0.0
      ER3(1)   = 0.0
  190 CALL OUT9(ENER3(1),FIELD(1,1),  3)
      IF(ER3(1).LE.0.0) GO TO 200
      PERCNT=100.0*ER3(1)
      CALL OUT9(ER3(1),FIELD(1,2),  3)
  ...
  200 ER3(1)=ERRMIN
      ...
      IF(ENER3(1).GT.0.0) GO TO 210
      MAXER3=2
      GO TO 250
  210 DO 240 MAXER3=2,21
      IF(ISTAT1.EQ.1) GO TO 250
      READ(INP,320,END=250,ERR=250) ENER3(MAXER3),ER3(MAXER3)
      IF(ENER3(MAXER3).LE.0.0.AND.ER3(MAXER3).LE.0.0) GO TO 250
      CALL OUT9(ENER3(MAXER3),FIELD(1,1),  3)
      IF(ER3(MAXER3).LE.0.0) GO TO 220
      PERCNT=100.0*ER3(MAXER3)
      CALL OUT9(ER3(MAXER3),FIELD(1,2),  3)
      WRITE(OUTP,430) ((FIELD(M,I),M=1,11),I=1,2),PERCNT
      WRITE(*   ,430) ((FIELD(M,I),M=1,11),I=1,2),PERCNT
      GO TO 230
  220 ER3(MAXER3)=ERRMIN
      PERCNT=100.0*ERRMIN
      CALL OUT9(ER3(MAXER3),FIELD(1,2),  3)
      WRITE(OUTP,440) ((FIELD(M,I),M=1,11),I=1,2),PERCNT
      WRITE(*   ,440) ((FIELD(M,I),M=1,11),I=1,2),PERCNT
  230 IF(ENER3(MAXER3).LT.ENER3(MAXER3-1)) GO TO 290
  240 CONTINUE
      GO TO 270
...
C-----ERROR LAW ENERGIES NOT IN ASCENDING ORDER.
  290 WRITE(OUTP,480)
      WRITE(*   ,480)
      GO TO 280
  300 FORMAT(2I11,11A1,I11)
  310 FORMAT(I6,I2,I3,I6,I2,I3)
  320 FORMAT(2E11.4)
  330 FORMAT(
