...
C       3456
      DATA PROGDOC/
     1 ' ***************** Program LINEAR (VERSION 2007-1) ***********',
...
C-----ASCENDING ORDER).
   ...
   20 IZA = C1H
      CALL ZAHOL(IZA,ZABCD)
      WRITE(OUTP,240) ZABCD,MATH,MFH,MTH,N2IN,(NBT(I),INT(I),I=1,N1)
      WRITE(*   ,240) ZABCD,MATH,MFH,MTH,N2IN,(NBT(I),INT(I),I=1,N1)
      CALL ENDIT
C
...
C-----NOT FOR MF = 9 AND 10
      IF(MFH.EQ.9.OR.MFH.EQ.10) GO TO 90
      IF(XEND        .LE.ETHERMAL.OR.
     1   XOUT(N2CORE).GE.ETHERMAL) GO TO 90
...
C-----CORE SPACE FOR ONE MORE POINT REQUIRED. IF NO ROOM IN CORE, THIN
C-----AND UNLOAD ONE PAGE OF POINTS TO SCRATCH.
   90 IF(N2CORE.GE.NPAGE2) CALL THINIT
      N2CORE=N2CORE+1
      N2P1=N2CORE+1
      N2P2=N2P1+1
      XOUT(N2CORE)=XEND
...
C-----NO THERMAL FOR MF = 9 OR 10
      IF(MFH.EQ.9.OR.MFH.EQ.10) GO TO 130
      IF(XTOP.GT.ETHERMAL.AND.XOUT(N2CORE).LT.ETHERMAL)
     1 XTOP=ETHERMAL
  130 IF(XTOP.GT.2.0D+00*XOUT(N2CORE)) XTOP=2.0D+00*XOUT(N2CORE)
      IF(XTOP.GE.XEND) GO TO 140
...
C     INTEREST.
C
      IF(DABS(YN2).LE.XCMIN.AND.DABS(YN2P2).LE.XCMIN) GO TO 200
C
C     DEFINE ENERGY AT MIDDLE OF INTERVAL.
C
C-----INTYPE IS NOW 3, 4, 5 OR 6.
  150 IF(INTYPE-4) 170,160,170
C-----LINEAR ENERGY.
  160 XN2P1=HALF*(XN2+XN2P2)
      GO TO 180
C-----LOG ENERGY.
  170 XN2P1=DSQRT(XN2*XN2P2)
C-----ROUND MIDPOINT.
  180 CALL INCORE9(XN2P1)
C
...
C-----SPACING ONLY KEEP THE TWO ENDS OF THE INTERVAL.
      IF(XN2P1.LE.XN2.OR.XN2P1.GE.XN2P2) GO TO 200
...
C-----KEEP THE TWO ENDS OF THE INTERVAL.
      IF(DABS(YN2).LT.XCMIN.AND.DABS(YN2P2).LT.XCMIN) GO TO 200
C
...
C-----IN ORDER TO ALLOW ACCURATE BACKWARD THINNING BEFORE OUTPUT.
      IF(DABS(YN2P1-YAPROX).LE.DABS(YN2P1*ERRXC3)) GO TO 190
C
...
C-----CORE SPACE FOR ONE MORE POINT REQUIRED. IF NO ROOM IN CORE, THIN
C-----AND UNLOAD ONE PAGE OF POINTS TO SCRATCH.
  200 IF(N2CORE.GE.NPAGE2) CALL THINIT
...
C-----MOVE TO NEXT INTERVAL.
  210 IF(XOUT(N2CORE).LT.XEND) GO TO 140
C-----IF RANGE SAVED, RESTORE POINT AND RESET FLAG
      IF(IMSAVED.LE.0) GO TO 220
      IMSAVED=0
      XEND=XSAVE
      YEND=YSAVE
      GO TO 120
...
C-----REACTION WITH A NEGATIVE Q-VALUE.
      IF(IM6.GT.0.AND.Q.LT.0.0) WRITE(OUTP,250)
      RETURN
  240 FORMAT(2X,10A1,I5,I4,I4/
     1 '  Error in Interpolation Law---Execution Terminated'/
     2 '  N2 =',I6/'   NBT  INT'/(I6,I5))
  250 FORMAT(19X,'WARNING - the Above Section Uses Interpolation Law 6'/
...
C-----PRINT FINAL FILENAMES
  ...
  100 FORMAT(2X,78('-')/
     1 '  ENDF/B Input and Output Data Filenames'/2X,A60/
     2 2X,A60)
C
...
C
      ...
      WRITE(   *,110) NAMEIN
  110 FORMAT(//' ERROR - Opening ENDF/B data file'/1X,A60//)
      CALL ENDIT
      ENDIF
C
