...
C
      ...
      REAL*8 SCALE
      DIMENSION IEXP(NW),IPARM(NP),NENRG(NR)
...
C
      ...
      DATA NTOTAL,ISEQNO,KCOVEX,CHITOT,SCALE/ 0, 0, 0, 0.0, 1.0/
      DATA PNAME / NP*'            '/
C
      READ(5,2000) TITLE
      READ(5,2015) NREAC,NPARM,KCTL1,KCTL2,KCOVEX,SCALE,EMIN0,EMAX0
      ...
      ELSE
         ...
         DO 110 I=1,NPARM
            ...
            IF(XX.EQ.0.) THEN
               ...
               WRITE(0,2160) I,PNAME(I),P0(I),PRIP(K),P1(I),PE(I)
            ELSE
               YY=PE(I)/XX*100.
               WRITE(6,2170) I,PNAME(I),P0(I),PRIP(K),P1(I),YY
               WRITE(0,2170) I,PNAME(I),P0(I),PRIP(K),P1(I),YY
            END IF
  110    CONTINUE
         WRITE(21,'(11(1PE12.5))') (PE(I)/P1(I),I=1,NPARM)
      END IF
...
C***************************************
   ...
   59       X1=X1+A(I,J)*PD(J)
         S0(I)=S0(I)+X1
   58 CONTINUE
C CHECKING FOR PARAMETERS AND CROSS SECTIONS
      DO I=1,NPARM
         IF (P1(I).LT.0.0) THEN
            WRITE(0,*) 'PARAMETER ',I,' IS NEGATIVE'
         ENDIF 
      ENDDO
      DO I=1,NENRG
         IF (S0(I).LT.0.0) THEN
            WRITE(0,*) 'CROSS SECTIONS < 0!!!!'
         ENDIF
      ENDDO
C
C***************************************
...
CP      DO 611 I=1,NDATA
CP         ISTART=I*(I-1)/2
CP         WRITE(56,600) X(I),Y(I),(V(ISTART+J),J=1,I)
CP 611     CONTINUE
CP
      DO 310 I=1,NDATA
         IF(Z(I) .LT. 0.0) THEN
...
C FOR COVARIANCE MATRIX OF PARAMETER (INPUT FORMAT)
         DO 49 I=1,NPARM
            ISTART=(I*(I-1))/2
            DO 48 J=1,I
               W(I,J)=REAL(X0(ISTART+J)/(PE(I)*PE(J)))
   48       CONTINUE     
   49    CONTINUE
         ...
         DO 47 I=1,NPARM
            DO 46 J=1,NPARM
               W(I,J)=W(J,I)
 46            CONTINUE
 47       WRITE(21,'(20(F6.3))')(W(I,J),J=1,NPARM) 
C
      ELSE
         DO 50 I=1,NPARM
            ISTART=(I*(I-1))/2
