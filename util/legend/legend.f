...
C     TO CONVERT FROM ONE SYSTEM TO THE OTHER). A MAXIMUM OF 30 LEGENDRE
...
C     ONLY USE COEFFICIENTS UP TO P8). IF OVER 30 NON-ZERO COEFFICIENTS
C     ARE READ ONLY THE FIRST 30 WILL BE USED.
C
...
C     DISITRIBUTION IS CONSTRUCTED IN THE SYSTEM IN WHICH THE TABULATED
C     DATA ARE GIVEN (I.E., CM OR LAB - NO ATTEMPT IS MADE TO CONVERT
C     FROM ONE SYSTEM TO THE OTHER). A MAXIMUM OF 20000 POINTS IS ALLOWE
...
C                    LIMITS ARE 11 TO 20000 POINTS)
...
C                    ALLOWABLE NUMBER OF POINTS = 20000.
...
C=======================================================================
      ...
      CHARACTER*1 NAME1,NAME2,NAME3,NAME4,BLANK,FIELD6
      CHARACTER*4 CARD
      ...
      COMMON/COPC/CARD(17)
      COMMON/LEGCON/FLMULT(30),PCON1(30),PCON2(30)
      COMMON/WHERE/E,IZALST,MATLST,MTLST,NE,JE
      COMMON/TYPES/TYPED(3)
      COMMON/FLAGS4/IMTABS,IMLEGS,IMFIX,IMVAR
      COMMON/TABLET/ETAB1(100),ETAB2(100),MATAB1(100),MATAB2(100),
     1 MFTAB1(100),MFTAB2(100),MTAB1(100),MTAB2(100),
     2 SIGTAB(100),DFLTAB(100),ITAB
      COMMON/NAMEX/NAME1(60),NAME2(60)
      COMMON/FIELDC/FIELD6(11,6)
      DIMENSION NAME3(8),NAME4(12)
...
C
      READ(INP,200) ERROK,NPTMU,IMTABS,IMLEGS,IMFIX,IMVAR
C-----INSURE PARAMETERS ARE IN LEGAL RANGE. IF NOT USE DEFAULT VALUES.
      IF(ERROK.LE.0.0) ERROK=ERRMIN
...
C-----WRITE TITLE FOR OUTPUT LISTING.
      ...
      IF(IMVAR.EQ.0) WRITE(OUTP,400)
      IF(IMVAR.EQ.1) WRITE(OUTP,410)
C
...
C-----INPUT DATA.
      READ(INP,210) NAME1
      DO 20 I=1,60
      IF(NAME1(I).NE.BLANK) GO TO 40
   20 CONTINUE
      DO 30 I=1,8
   30 NAME1(I)=NAME3(I)
C-----OUTPUT DATA.
   40 READ(INP,210) NAME2
      DO 50 I=1,60
      IF(NAME2(I).NE.BLANK) GO TO 70
   50 CONTINUE
      DO 60 I=1,12
   60 NAME2(I)=NAME4(I)
   70 WRITE(OUTP,350) NAME1,NAME2
      WRITE(*   ,350) NAME1,NAME2
C
...
C-----MINIMUM REQUESTED CROSS SECTION AND CHANGE IN COEFFICIENTS.
      ...
      DO 100 ITAB=1,100
C-----READ NEXT REQUEST.
      READ(INP,220) MATAB1(ITAB),MFTAB1(ITAB),MTAB1(ITAB),MATAB2(ITAB),
     1 MFTAB2(ITAB),MTAB2(ITAB),ETAB1(ITAB),ETAB2(ITAB),SIGTAB(ITAB),
     2 DFLTAB(ITAB)
...
C-----ASSIGN STANDARD VALUES.
   80 IF(MATAB2(ITAB).LT.MATAB1(ITAB)) MATAB2(ITAB)=MATAB1(ITAB)
...
C-----OUTPUT INTERPRETATION OF CARD.
   90 CALL OUT9(ETAB1( ITAB),FIELD6(1,1),3)
      CALL OUT9(ETAB2( ITAB),FIELD6(1,2),3)
      CALL OUT9(SIGTAB(ITAB),FIELD6(1,3),0)
      WRITE(OUTP,360) MATAB1(ITAB),MFTAB1(ITAB),MTAB1(ITAB),
     1 MATAB2(ITAB),MFTAB2(ITAB),MTAB2(ITAB),
     2 ((FIELD6(M,J),M=1,11),J=1,3),DFLTAB(ITAB)
      IF(IEND.GT.0) GO TO 120
  100 CONTINUE
      ITAB=100
      GO TO 120
  110 ITAB=ITAB-1
...
C-----COPY TAPE LABEL.
      CALL COPYL
      WRITE(OUTP,370) CARD,MATH
      WRITE(*   ,370) CARD,MATH
      LASTMAT=-10000
C-----COPY UP TO HOLLERITH (MF/MT=1/451) OR ANGULAR DISTRIBUTIONS (MF=4)
  130 CALL CONTIL
      IF(MATH.LT.0) GO TO 190
      IF(MATH.EQ.LASTMAT) GO TO 140
...
C-----CHECK FOR HOLLERITH SECTION.
  140 IF(MFH.NE.1.OR.MTH.NE.451) GO TO 150
      CALL CONTOL
      CALL FILE1
      CALL COPYS
      GO TO 130
C-----CHECK FOR ANGULAR DISTRIBUTIONS.
  150 IF(MFH-4) 160,170,160
...
C-----PRINT TOTALS AND TERMINATE.
      WRITE(OUTP,390) TYPED
      WRITE(*   ,390) TYPED
      GO TO 180
  200 FORMAT(E11.4,5I11)
  210 FORMAT(60A1)
  220 FORMAT(I6,I2,I3,I6,I2,I3,4E11.4)
  230 FORMAT(' Linearized ENDF/B Angular Distributions (LEGEND 2000-1)'/
     1 1X,78('-'))
  ...
  350 FORMAT(1X,78('-')/
     ...
     2 1X,60A1)
  360 FORMAT(I6,I4,I5,I6,I4,I5,33A1,F11.5)
  370 FORMAT(1X,78('-')/
  ...
  390 FORMAT(1X,78('-')/' Summary of Data as Read'/1X,78('-')/
     1 ' Isotropic Sections-------------------',I5/
     2 ' Energies with Legendre Coefficients--',I5/
     3 ' Energies with Tabulations------------',I5)
  400 FORMAT(' Coefficient Tests-----------Off')
  410 FORMAT(' Coefficient Tests-----------On')
      END
...
C       3456
      DATA PROGDOC/
     1 ' ***************** Program LEGEND (VERSION 2000-1) ***********',
...
C=======================================================================
      ...
      COMMON/OUTTAB/N2OUT
      COMMON/MAXIE/ERRMAX,ERROK,ERRUSE,LOOPE
      COMMON/KEEPER/FLKEEP(30,3),EKEEP(3),ELAST4,LKEEP(3)
      COMMON/LEGS/FL(30),FLOLD(30),NEWFL(30),DFLCHN,DFLMAX,LEGIN,LEGUSE,
     1 LOOPL,LOOPPC,KPASS
      COMMON/WHERE/E,IZALST,MATLST,MTLST,NE,JE
      COMMON/TYPES/TYPED(3)
      COMMON/FLAGS4/IMTABS,IMLEGS,IMFIX,IMVAR
      COMMON/NBTINT/NBT(100),INT(100)
      COMMON/FIELDC/FIELD6(11,6)
      COMMON XMUIN(20000),PTIN(20000),XMUOUT(20000),PTOUT(20000)
C
...
C
C-----IF LTT IS NOT 0, 1 OR 2 PRINT ERROR MESSAGE AND COPY SECTION.
      IF(LTT.GE.0.AND.LTT.LE.2) GO TO 30
      WRITE(OUTP,270) IZA,MATH,MTH
      GO TO 90
C-----DETERMINE REPRESENTATION (0=ISOTROPIC, 1=LEGENDRE, 2=TABULATED)
   30 IF(LTT-1) 40,50,70
C-----ANGULAR DISTRIBUTIONS ARE ISOTROPIC. COPY SECTION.
   40 TYPED(1)=TYPED(1)+1
      WRITE(OUTP,280) IZALST,MATLST,MTLST
      GO TO 90
C-----LEGENDRE COEFFICIENTS. IF REQUESTED COPY LEGENDRE DATA.
   50 IF(IMLEGS) 60,60,100
   60 WRITE(OUTP,290) IZALST,MATLST,MTLST
      TYPED(1)=TYPED(1)+1
      GO TO 90
C-----TABULATED DATA. IF REQUESTED COPY TABULATED DATA.
   70 IF(IMTABS) 80,80,110
   80 WRITE(OUTP,290) IZALST,MATLST,MTLST
      TYPED(2)=TYPED(2)+1
C-----COPY SECTION.
   90 CALL CONTOL
      CALL COPYS
...
C-----ZERO AT LOW ENERGY.
      IF(LTT.NE.1) GO TO 160
      DO 140 L=1,N1
      IF(INT(L).GE.4) GO TO 150
  140 CONTINUE
      GO TO 160
  150 WRITE(OUTP,300)
...
C     NOT SPAN THE COSINE RASGE -1.0 TO 1.0.
C
C-----OUTPUT MESSAGE IF REQUESTED ACCURACY WAS NOT ACHIEVED.
      IF(LOOPE.LE.0) GO TO 190
      CALL OUT9(ERROK,FIELD6(1,1),3)
      WRITE(OUTP,240) (FIELD6(M,1),M=1,11)
C-----CHECK FOR COSINES NOT IN ASCENDING ORDER.
  190 COSLST=-1.0
      DO 200 J=1,N2OUT
      IF(XMUOUT(J).GE.COSLST) GO TO 200
      WRITE(OUTP,250)
      GO TO 210
  200 COSLST=XMUOUT(J)
C-----CHECK FOR COSINES SPANNING -1.0 TO 1.0
  210 IF(DABS(XMUOUT(1)+1.0).LE.0.001) XMUOUT(1)=-1.0
      IF(DABS(XMUOUT(N2OUT)-1.0).LE.0.001) XMUOUT(N2OUT)=1.0
      IF(DABS(XMUOUT(1)+1.0).GT.0.001.OR.DABS(XMUOUT(N2OUT)-1.0)
     1 .GT.0.001) WRITE(OUTP,260)
C-----END OF ENERGY LOOP.
  220 CONTINUE
C-----COPY SECTION END LINE.
      CALL COPYS
C-----END OF FILE 4 SECTION.
  230 RETURN
  240 FORMAT(1X,78('-')/' WARNING...Allowable Error=',11A1/
     1 1X,78('-'))
  250 FORMAT(1X,78('-')/' WARNING...Cosines NOT in Ascending Order'/
     1 1X,78('-'))
  260 FORMAT(1X,78('-')/' WARNING...Cosines Do NOT Span -1 to +1'/
     1 1X,78('-'))
  270 FORMAT(I7,I5,I4,' LTT=',I6,' Expect 0, 1 or 2...Section Copied.')
  280 FORMAT(I7,I5,I4,' Isotropic...Copied')
  290 FORMAT(I7,I5,I4,' Legendre Coefficients...Copied')
  300 FORMAT(I7,I5,I4,' WARNING - Log Interpolation for Legendre'/
...
C     BE 20000 OR FEWER POINTS. ALMOST ALL ANGULAR DISTRIBUTIONS CAN BE
C     REPRESENTED BY 20000 OR FEWER POINTS. IF THE ALLOWABLE ERROR CAUSE
C     MORE THAN 20000 POINTS TO BE GENERATED THE ALLOWABLE ERROR WILL BE
...
C     REPRESENTED BY 20000 OR FEWER POINTS. WHENEVER THE ALLOWABLE ERROR
...
C-----INSURE THAT ANGULAR DISTRIBUTION WILL FIT IN CORE.
   30 IF(N2IN.LE.20000) GO TO 40
      WRITE(OUTP,280) IZALST,MATLST,MTLST,N2IN
      CALL ENDIT
...
C-----OUTPUT ANGULAR DISTRIBUTION.
  ...
  280 FORMAT(I7,I5,I4,
     1 ' Angular Distribution Contains Over 20000 Points'/
...
C-----PRINT WARNING IF COEFFICIENT STARTS AT A NON-ZERO VALUE.
      ...
      LOUT=1
      CALL OUT9(FLKEEP(L,3),FIELD6(1,1),0)
...
C-----CHECK FOR ASCENDING ENERGIES.
      ...
      CALL OUT9(ELAST4,FIELD6(1,1),0)
      CALL OUT9(E     ,FIELD6(1,2),0)
...
C-----CONVERT COEFFICIENTS TO OUTPUT FORM.
      CALL OUT9(FLKEEP(L,2),FIELD6(1,3),0)
      CALL OUT9(FLKEEP(L,3),FIELD6(1,4),0)
...
C=======================================================================
      ...
      COMMON/LEGS/FL(30),FLOLD(30),NEWFL(30),DFLCHN,DFLMAX,LEGIN,LEGUSE,
     1 LOOPL,LOOPPC,KPASS
      COMMON/ADDIN/SUM,SUMN(30),PN(30)
      COMMON/OUTTAB/N2OUT
      COMMON/MAXIE/ERRMAX,ERROK,ERRUSE,LOOPE
      COMMON XMUIN(20000),PTIN(20000),XMUOUT(20000),PTOUT(20000)
...
C     COSINE=XMU.
C
C     RESULTS ARE RETURNED IN /ADDIN/
C
...
C=======================================================================
C
C     THIN (X,Y) DATA ASSUMING LINAR INTERPOLATION BETWEEN POINTS.
C
...
C=======================================================================
      ...
      EQUIVALENCE (XMUOUT(1),X(1)),(PTOUT(1),Y(1))
C
...
C
   20 IF(Y(M)*Y(MM1).GE.0.0) GO TO 30
...
C-----INITIALIZE NON-NEGATIVE POINT COUNT. SEARCH FOR NEGATIVE POINTS.
      ...
      IMNEG=0
      DO 10 I=1,N2OUT
      IF(PTOUT(I).LE.0.0) IMNEG=IMNEG+1
...
C=======================================================================
     ...
     1 LOOPL,LOOPPC,KPASS
      ...
      COMMON/LEGCON/FLMULT(30),PCON1(30),PCON2(30)
      COMMON/MINY/SIGMIN
      DIMENSION VALUES(30),LVALUE(30),DFL(30)
C
...
C-----DEFINE ALL FILE NAMES.
      OPEN(INP,FILE='LEGEND.INP',STATUS='OLD')
