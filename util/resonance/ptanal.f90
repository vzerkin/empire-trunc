program ptanal
    !
    !     written by J.H.Chang (as ANAL)
    !     modified by S.Y.Oh (PTANAL) March 30, 1999
    !     modified by Y.S.Cho (PTANAL) Feb 06, 2007 (for EMPIRE)
    !     July 26,2006    linux output for d0, s0 agreed with alpha results.
    !      However, linux version gave ridiculous  result for *.fit outputs
    !     Tom  burrows and mike herman found out that variable dummy was not
    !     dimensioned in the alpha version which nevertheless gave correct values
    !      ******************************************************************
    !     read averaged BNL325 file and perform analyses
    !       - calculate strength functions, level spacings
    !       - assign L and J for unidentified resonances using Bayesian
    !       - convert CFMTA format to ENDF-6 format
    !
    !     For unknown Gg value, ggavg is assinged
    !     recognize flag at 128 col. as ggflag(1,)
    !     ggflag(2,) is used for determined spin
    !
    !     Input files : ptanal.inp
    !                   resonance data from various files
    !     Output files: ptanal.lis  - standard output
    !                   endfa.txt   - ENDF-6 format MF=2 in MLBW
    !                                 Only up to L=1 for Resolved Res. Region
    !                   ptdistx.dat - reduced width distribution of given data
    !                   ptdistx.fit - reduced width distribution of fitted
    !                      x=blank : all resonances
    !                      x=0,1   : s- and p-wave, respectively
    !cho                ptdist.gp   - GNUPLOT file
    !     note: Modification is necessary for using SAS file as an input.
    !         : This calculates d-wave probability ONLY FOR INFORMATION.
    !
    parameter (mres = 1000)
    ...
    !cho
    !                 adjust(1:1) : type of adjustment
    !                 adjust(2:5) : index of resonance which will be adjusted
    !                 adjust(7:10) : number of total positive resonances which will be adjusted
    !                 adjust(11:11) : sign of adjustment
    !                 adjust(12:13) : number of positive resonance energies at which cross sections
    !                                 will be evaluated
    !                 adjust(14:17) : number of extra positive resonances which will be included
    !                                 in ENDF file
    !cho
    ...
    !     awt = atomic weight
    !     sf0 = s-wave strength function [1E-4]
    !     iset = set number selected among sf1 values to determine p-wave
    !     sf1(3) = 3 trial set of p-wave strength function
    !     sf2(3) = 3 trial set of d-wave strength function ; OBSOLETE
    !     D0 = avg. level spacing for s-wave [eV]
    !     D1 = avg. level spacing for p-wave [eV] ; OBSOLETE
    !     ecut = upper energy limit for analyses [eV]
    !            recommended: energy of the last res. + 2*max(Gn,Gg)
    !     zam = ZZAAA
    !     spin = spin of the target
    !     ggavg(2) = average gamma width for s- and p-wave [meV]
    !     gncut(2) = weak resonance cutoff width, gGn^l [meV]
    !                Resonances having widths smaller than the values would
    !                not be taken into account for the analyses.
    !     gncuth(2)= too strong resonance cutoff width, gGn^l [meV]
    !                Resonances having widths larger than the values would
    !                not be taken into account for the analyses.
    !        Note: Very minor effect is observed using this option.
    !     mat = MAT number in ENDF format
    !     inflag = input flag: read inflag*gGn & inflag*gGn0 from BNL325.TXT ; OBSOLETE
    !            Of usual, inflag=2 for odd and =1 for even nuclide, respectively
    !cho  inflag is no longer used. It is now provided in the resonance parameter table
    !cho  abun = fractional abundance
    !cho  ap = scattering radius (in units of 1e-12 cm !!!)
    !
    adjust = ''
    rad = 1.35
    iset = 1
    ...
    700  format(/' <<<<< PTANAL >>>>>')
    call readres
    !cho  sas related routines are deleted
    !cho  call readsas('sas.txt')
    call pwave
    call ptdist(-1)
    call ptdist(0)
    call ptdist(1)
...
subroutine readinp
    ...
    !caleb Said suggests using arithmetic mean for d-wave gamma width:
...
!.....
subroutine readres
    !     read resonance parameter file in CFMTA format
    parameter (mres = 1000)
    character fname*1024, line*196
    !cho  character ggflag*1
    character*1 ggflag, gnflag, aaflag
    common/rps/ nres, rp(8, mres), SJ(mres), LJ(mres), ggflag(2, mres), &
            gnflag(2, mres), aaflag(2, mres)
    common/dat/ rad, awt, sf0, iset, sf1(3), D0, sf2(3), D1, ecut, ap, dap, &
    ...
    common/adj/ npos, nidx, nplus, adjust
    character adjust*17
    !cho  character srp*2,jflag*1,lflag*1,gflag*1,ajxch*3,lwch*1
    character jflag*1, lflag*1, ajxch*3, lwch*1, gflag*3, nflag*3, &
            aflag*3
    !
    !cho  modified to read newly introduced resonance parameter table
    !     instead of bnl325.txt
    !
    ...
    do i = 1, mres
        ...
        read(line, 2000, err = 150) iza, e0, de0, jflag, ajx, lflag, lw, &
                nflag, ggn, dggn, gflag, gg, dgg, aflag, area
        ...
        !      read resonance parameter table
        !cho   read(2,1000,end=120) srp,e0,de0,
        !cho 1  ajx,jflag,lw,lflag,ggn,dggn,gg,dgg, area
        !1000  format(7x,a2,f9.3,f6.1,1x,1x,f3.1,a1,i1,a1,16x,f8.3,f7.2,1x,
        !cho 1  f8.3,f7.2,1x,16x,f8.3,25x,a1)
        !
        !cho   if(srp.ne.'RP') then
        !cho    print *,' Should be RP at',e0
        !cho    stop
        !cho   endif
        !cho   if(ajx.eq.0.0.or.lw.eq.0) then
        !cho     backspace 2
        !cho     read(2,2002) ajxch,lwch
        !2002    format(26x,a3,1x,a1)
        !cho     if(ajxch.ne.'   '.and.jflag.eq.' ') jflag='C'
        !cho     if(lwch.ne.' '.and.lflag.eq.' ') lflag='C'
        !cho   end if
        !cho   read(2,2010) srp,gflag
        !2010  format(7x,a2,16x,7x,97x,a1)
        !cho   if(srp.ne.'R2') then
        !cho    print *,' Should be R2 at',e0
        !cho    stop
        !cho   endif
        ...
        rp(1, i) = e0
        rp(2, i) = de0
        !cho   rp(3,i)=ggn/inflag
        !cho   rp(4,i)=dggn/inflag
        rp(3, i) = ggn
        rp(4, i) = dggn
        rp(5, i) = gg
        rp(6, i) = dgg
        !caleb use kernel to adjust gG_n?
        rp(7, i) = area
        !       rp(7,i)=0
        rp(8, i) = 0
        SJ(i) = -1
        !oh       if(jflag.eq.'F') SJ(i)=ajx
        if(ajx.ne.0..or.jflag.ne.' ') SJ(i) = ajx
        if(jflag.eq.'A') SJ(i) = -1
        LJ(i) = -1
        !oh       if(lflag.eq.'F') LJ(i)=lw
        if(lw.ne.0.or.lflag.ne.' ') LJ(i) = lw
        if(lflag.eq.'A') LJ(i) = -1
        !cho   ggflag(1,i)=gflag
        !cho   ggflag(2,i)=' '
        ...
        if((gnflag(1, i).eq.'C'.or.gnflag(1, i).eq.'D'.or.&
                gnflag(1, i).eq.'D').and.abun.eq.0) then
    ...
    if (adjust(1 : 1).ne.' ') then
        !      ptanla.unc is for writing uncertainty of resonance parameter to be adjusted
        ...
        !       if (adjust(11:11).eq.'+') then
        !        rp(1,nidx)=rp(1,nidx)+rp(2,nidx)
        !       else
        !        rp(1,nidx)=rp(1,nidx)-rp(2,nidx)
        !       endif
        !      elseif (adjust(1:1).eq.'N') then
        !       if (adjust(11:11).eq.'+') then
        !        rp(3,nidx)=rp(3,nidx)+rp(4,nidx)
        !       else
        !        rp(3,nidx)=rp(3,nidx)-rp(4,nidx)
        !       endif
        !      elseif (adjust(1:1).eq.'G') then
        !       if (adjust(11:11).eq.'+') then
        !        rp(5,nidx-1)=rp(5,nidx)+rp(6,nidx)
        !       else
        !        rp(5,nidx-1)=rp(5,nidx)-rp(6,nidx)
        !       endif
        !      elseif (adjust(1:1).eq.'F') then
        !      endif
        !      if (nplus+npos+4.lt.nres) then
        !       nres=nplus+npos+4
        !      endif
    endif
    return
...
subroutine gamavg
    !     compute average Gg
    !oh   weighted by 1/variance
    parameter (mres = 1000)
    !cho  character ggflag*1
    character*1 ggflag, gnflag, aaflag
    common/rps/ nres, rp(8, mres), SJ(mres), LJ(mres), ggflag(2, mres), &
            gnflag(2, mres), aaflag(2, mres)
    !cho  add capability of computing average Gg for each partial wave
    gsum = 0.
    gsum0 = 0.
    gsum1 = 0.
    g2sum = 0.
...
subroutine pwave
    ...
    !cho
    !2100 format(' Input data'//'  ZZAAA = ',f6.0/'  Awt = ',f9.3/
    !    *       '  Upper energy of Resolved Region = ',f9.2,'  eV'/
    !    1       '  Average level spacing, D0       =    ',f7.3,' eV'/
    !    2       '  s-wave strength function        =    ',4pf7.3,' E-4',0p/
    !    3       '  Weak res. gGn^l cutoffs [meV]   =   ',2f8.3/
    !    4       '  Avg. gamma widths [meV]         =   ',2f8.3//)
    2100 format(' Input data'//'  ZZAAA = ', f6.0/'  Awt = ', f10.3/&
            '  Upper energy of Resolved Region = ', f10.3, ' eV'/&
            '  Average level spacing, D0       = ', f10.3, ' eV'/&
            '  s-wave strength function        =    ', 4pf7.3, ' E-4', 0p/&
            '  Weak res. gGn^l cutoffs [meV]   = ', 3f10.3/&
            '  Avg. gamma widths [meV]         = ', 3f10.3//)
    !oh      write(7,2160) sf1
    !oh 2160 format(43x,'p-wave Probabilities'/
    !oh     1  11x,'E (eV)',3x,'gGn (meV)',5x,'L',3x,4p3f10.4)
    write(7, 2160) iset, sf1
    2160 format(' Calculated Probabilities'//&
            '  iset=', i2, ' among p-wave STFs of S_1 =', 4p3f10.4, ' E-4'//&
            11x, 'E (eV)', 3x, 'gGn (meV)'&
            , 5x, 'L', 5x, 'p-wave prob. for each S_1    d-prob.'/)
    do n = 1, nres
        lw = 0
        En = rp(1, n)
        !oh       gGn=0.5*rp(3,n)
        gGn = rp(3, n)
        !cho   assumes g factor if not included
        ...
        if(gGn.eq.0.0) then
            mk = '?'
            !  avg. stat. factor, g=0.5 assumed : OBSOLETE
        ...
        do is = 1, 3
            ...
            p1s1 = p1 * sf1(is)
            bb = p1s1 / p0s0
            !c        x = gGn/(2*D0)*sqrt(1/En)*(1/p1s1-1/p0s0)
            x = gGn / (2 * D0) * sqrt(1 / En) * (corr / p1s1 - 1 / p0s0)
            !c        yzero=2.*bb/(1.-bb)*x
            !c        ccc=sqrt(3.14159/2.*yzero/bb)
            !c        if(spin.lt.0.001) corr=1.
            !c        if(spin.gt.0.499.and.spin.lt.0.501) corr=1./(0.6667+0.3333*ccc)
            !c        if(spin.gt.0.999) corr=1./(0.5+0.5*ccc)
            Pp(is) = 0.
            if(x.lt.50.0)&
        ...
        !
        !      Calculate d-wave probability: Just for information !!!
        !
        d1d2 = 5. / 3.
        ...
        !c       if(lw.eq.1.and.Pd.gt.0.5) lw=2
        !
        !      check if the others agree
        do is = 1, iss
            if((Pp(iset) - 0.5) * (Pp(is) - 0.5).le.0.0) mk = '*'
        enddo
        !c       write(7,1000) n,mk,En,1000*gGn,lw,(Pp(is),is=1,iss)
        !c 1000  format(i5,1x,a1,f10.3,f12.3,2x,i4,3f10.2)
        write(7, 1000) n, mk, En, 1000 * gGn, lw, (Pp(is), is = 1, iss), Pd
        1000  format(i5, 1x, a1, f10.3, f12.3, 2x, i4, 4f10.2)
        !      check if new assignment agrees with old one
    ...
    1095 format(/5x, '*:  Assignment is not certain.'/&
            5x, '?:  gGn is not given, so we assume from gGnGg/G.')
    return
end
!.....
subroutine ptdist(lwave)
    !     Porter-Tomas distribution
    parameter (mres = 1000)
    !cho  character*1 ggflag
    character*1 ggflag, gnflag, aaflag
    common/rps/ nres, rp(8, mres), SJ(mres), LJ(mres), ggflag(2, mres), &
    ...
    j = 0
    !cho  skip the leading negative energies
    nneg = 0
    do n = 1, nres
    ...
    20 do n = 1 + nneg, nres
        ...
        if(gGn.eq.0.0) then
            !oh avg. stat. factor g=0.5 assumed
            facg = rp(7, n) / (0.5 * 0.001 * ggavg(1))
            if(facg.gt.0.99) facg = 0.99
            gGn = rp(7, n) / (1 - facg)
        endif
    ...
    100 jres = j
    if(lwave.ge.0)&
            print *, jres, ' res. found for ', wav(lwave + 1), '-wave'
    ...
    if(lwave.eq.0) then
        A(2) = 0.05
        !cho   replaced by the following formula
        !cho   A(2)=sf0*D0
    elseif(lwave.eq.1) then
        A(2) = 0.2
    endif
    !     skip small gGn^l resonances with c(j).lt.gncut
    !     and  large gGn^l resonances with c(j).gt.gncuth
    ib = 1
    if(lwave.ge.0) then
        do j = 1, jres
            ib = j
            !oh        if(x(j).gt.gncut(lwave+1)) go to 200
    ...
    !
    !   Fit sqrt(gGn^l) vs. Nres
    !   A(1)=Nres-fit, A(2)=sqrt(<2*gGn^l>)
    !
    !c      call mrqfit(x(ib),c(ib),csig(ib),jres-ib+1, A,covar)
    call mrqfit(x(ib), c(ib), csig(ib), jres - ib + 1 - ibh, A, covar, jres)
    !
    Dsp = ecut / A(1)
    !oh      var=ecut/(A(1)-sqrt(covar(1,1))) - Dsp
    errfit = Dsp / A(1) * sqrt(covar(1, 1))
    !   statistical error = <D>*0.5*sqrt(1/N)
    errtot = Dsp * sqrt(0.25 / jres)
    errtot = sqrt(errtot * errtot + errfit * errfit)
    write(7, 2200) Dsp, errtot, errfit
    !cho
    !2200 format(5x,' <D>     =',f8.3,' +-',f8.3,' eV  (including error',
    ...
    if(lwave.ge.0) then
        ...
        strsd = 4 * A(1) * A(1) * A(2) * A(2) * covar(2, 2)&
                + 4 * A(1) * A(2)**3 * covar(1, 2) + A(2)**4 * covar(1, 1)
        strsd = 10000. * ttt * sqrt(strsd)
        ! statistical error = <S>*sqrt(2/N+0.25/N)
        errtot = str * sqrt(2.25 / jres)
        errtot = sqrt(errtot * errtot + strsd * strsd)
        write(7, 2220) Str, errtot, strsd
        !cho
        !2220  format(5x,' Str.ftn.=',f8.3,' +-',f8.3,' E-4 (including error',
...
!.....
!c      subroutine mrqfit(x,c,csig,jres, A,covar,nres)
subroutine mrqfit(x, c, csig, jres, A, covar, nnres)
    ...
    ia(2) = 1
    alamda = -1
    !TWB      call mrqmin(x,c,csig,jres,A,ia,2,covar,alpha,2,
    !TWB     1     chisq,porter,alamda)
    ...
    !cho
    !1400 format(5x,' chi_sq  =',f8.3,4x,'(',f8.3,' =chi_sq/N)')
...
subroutine spina
    ...
    do lw = 1, 3
        ...
        !      count number of res. for the wave
        lws = 0
        do k = 1, npr
            nass(k) = 0
        enddo
        do n = 1, nres
            ...
            if(LJ(n) + 1.eq.lw) then
                ...
                if(sjn.ge.0.0) then
                    !oh          write(7,2000) SJ(n),rp(1,n)
                    !oh 2000     format(4x,' * Pre-assigned J =',f4.1,' at',f8.2,' eV')
                    k = SJ(n) - spj + 1
        ...
        !oh       write(7,1200) 'g            =',
        !oh     1       ((2*(spj+k-1)+1)/(2.0*(2*spin+1)),k=1,npr)
        !oh  210  write(7,2100) lws,(nass(k),k=1,npr)
        !oh 2100  format(/'   There are ',i4,' resonances.'/
        !oh     1         '   Nos. of res. with pre-assigned J are:',6i4//)
        210  write(7, 1220) 'No. Pre-asgn. J =', (nass(k), k = 1, npr)
        1220  format(3x, a17, 6(6x, i4))
        !      find accumulated probability now
        if(lws.gt.0) then
            opr = 0
            allx = lws
            do k = 1, npr
...
subroutine ljprob(spi, L, npr, pr, spj)
    ...
    !     npr: number of choice
    !     pr:  probability
    !     spj: minimum j-spin corresponding to pr(1)
    data eps/0.0001/
    isp2 = 2 * spi + eps
    if(L.gt.0) go to 100
    ...
    pr(6) = (spi + 3) / (6 * spi + 3)
    go to 500
    !cho  300 print *,' *** LJPROB *** High wave',i2,' is not programmed yet'
...
subroutine endf
    ...
    !     we write dummy records for PSY code
    !oh      write(3,1100) 'Dummy'
    !heck ENDF description file
    !oh      open(4,file='endf.desc',status='old',err=85)
    !oh      read(4,1100) txt,mat
    !oh      nseq=1
    !oh      write(3,1100) txt,mat,1,451,nseq
    !oh      do n=1,1000
    !oh       read(4,1100,end=80) txt
    !oh       nseq=nseq+1
    !oh       write(3,1100) txt,mat,1,451,nseq
    !oh      enddo
    !oh   80 close(4)
    ...
    mth = 451
    !oh      do i=1,3
    !oh       write(3,1100) '   ',mat,mfh,mth
    !oh      enddo
    !oh      write(3,1000) ' ',' ',0,0,1,1,mat,mfh,mth
    !
    ...
    !
    write(3, 1200)&
            ' zsymam    ', ' ', 'RES. EVAL. ', ' ', ' ', '1999mmdd   '&
            , mat, mfh, mth
    write(3, 1200)&
            '***********', '***********', ' ', ' ', ' ', ' ', mat, mfh, mth
    write(3, 1200)&
            ' RESONANCE ', 'PARAMETER  ', ' ', ' ', ' ', ' ', mat, mfh, mth
    ...
    write(3, 1000) s1, s2, 0, 0, 1, 0, mat, mfh, mth
    !     spicify MLBW and energy range
    call r2str(s1, 11, 6, 1.0e-5)
    call r2str(s2, 11, 6, ecut)
    write(3, 1000) s1, s2, 1, 2, 0, 0, mat, mfh, mth
    !     find maximum L value and count number of resonances
    do lw = 1, 5
        nr(lw) = 0
    enddo
    lm = 0

    !cho   cut the resonance energies for sensitivity calculation
    if (adjust(1 : 1).ne.' ') then
    ...
    do lw = 1, lm
        ...
        do n = 1, nres
            ...
            if((En.le.ecut).and.(LJ(n).eq.lwave)) then
                ...
                if(ggflag(2, n).eq.'?') then
                    !         recompute since spin is changed
                    gg = rp(5, n)
                ...
                if(gGn.eq.0.0) then
                    ...
                    if (ggflag(1, n).eq.'A') gg = rp(5, n) / gfact
                    if (ggflag(1, n).eq.'C') gg = rp(5, n) / 2 / gfact
                    !cho      cause problem if average gamma width is not given
                    if (ggavg(lw).gt.0.0) then
                        ...
                        2030       format(5x, a10, ': gGn is not given. Based on', &
                                ' capture area and avg. Gg, gGn =', f7.4)
                        ehead = '    '
                    endif
                endif
                gn = gGn / gfact
                    ...
                    if(rp(7, n).gt.0.0) then
                        ...
                        if(gn - rp(7, n) / gfact.gt.1.e-4) then
                            ...
                            2085       format(5x, a10, ': gGn changed from ', f9.6, ' eV.', &
                                    ' Based on capture area and avg. Gg, gGn =', f9.6, ' eV')
                            ehead = '     '
                            gGn = xgGn
                            gn = gGn / gfact
                        endif
                ...
                !         if (rp(2,n).eq.0.0.and.rp(1,n).le.100e+3) then
                !           dEn=abs(0.001*En)
                !         elseif (rp(2,n).eq.0.0) then
                ...
                !         dgg=1.5*dgg
                !
                !cho  uncertainty of resonance parameter to be adjusted is written
                !     to ptanal.unc and the corresponding parameter is adjusted
        ...
        2200  format(10x, '<Gg>  =', f10.3, ' meV ', &
                '(', f10.3, ' = simple avg. from', i3, ' res. with given Gg)'//)
    ...
    !
    !     resonance parameters and their uncertainties are written
    !
    do n = 1, nres
    ...
    enddo
    !
    !     correlation coefficients are assigned and
    !     values between -10^-3 and 10^-3 are dropped according to the rule
    !
    !     corr : correlation between Gn and Gg
    !     corr = -0.99
    !     corr = 0.5
    !     corr = 0.0
    !     do i=1,nnn
    ...
    !     enddo
    !
    !     assigning of correlations should be done here
    ...
    nm = 0
    !     compute number of lines for correlation coefficients
    do i = 1, nnn
        j = 1
        do while (j.lt.i)
            if (correff(j, i).ne.0) then
                ...
                nm = nm + 1
                j = j + len
                !            j=j+1      ! gives incorrect value for nm
            else
                j = j + 1
            endif
        enddo
    ...
    do i = 1, nnn
        ...
        do while (j.lt.i)
            if (correff(j, i).ne.0) then
                len = min(13, i - j)
                do n = 1, len
                    !     multiply by 10^ndigit (=3 for our case) and convert it to integer
...
function rspin(L, spi)
    common/ran/ iran
    data eps/0.0001/
    !     determine resonance spin according to equal space spin
    !     need to initalize random number by calling
    !     iran=-1
    !     do x=ran2(iran) enddo
    isp2 = spi * 2 + eps
    !     s-wave
    if(L.gt.0) go to 100
...
subroutine r2str(str, n, m, x)
    ...
    !     write as many digits as possible
...
END
!.....
!TWB      SUBROUTINE mrqmin(x,y,sig,ndata,a,ia,ma,covar,alpha,nca,chisq,
!TWB     *funcs,alamda)
SUBROUTINE mrqmin(x, y, sig, ndata, a, ia, ma, covar, alpha, nca, chisq, &
        alamda)
    INTEGER ma, nca, ndata, ia(ma), MMAX
    !TWB      REAL alamda,chisq,funcs,a(ma),alpha(nca,nca),covar(nca,nca),
    ...
    if(alamda.lt.0.)then
        ...
        11      continue
        alamda = 0.001
        !TWB        call mrqcof(x,y,sig,ndata,a,ia,ma,alpha,beta,nca,chisq,funcs)
    ...
    do 14 j = 1, mfit
        ...
        da(j) = beta(j)
    14    continue
    !cho  just skip the fitting when error occures in gaussj
    ...
    15    continue
    !TWB      call mrqcof(x,y,sig,ndata,atry,ia,ma,covar,da,nca,chisq,funcs)
...
END
!.....
!TWB      SUBROUTINE mrqcof(x,y,sig,ndata,a,ia,ma,alpha,beta,nalp,chisq,
!TWB     *funcs)
SUBROUTINE mrqcof(x, y, sig, ndata, a, ia, ma, alpha, beta, nalp, chisq)
    ...
    chisq = 0.
    do 16 i = 1, ndata
        !TWB        call funcs(x(i),a,ymod,dyda,ma)
...
SUBROUTINE gaussj(a, n, np, b, m, mp, ierr)
    ...
    do 22 i = 1, n
        ...
        indxr(i) = irow
        indxc(i) = icol
        !cho    just skip the current processing when error occures
...
FUNCTION gammp(a, x)
    ...
    !dub  REAL*8 gammcf,gamser
    !dub  REAL gln
    if(x.lt.0..or.a.le.0.) call pause ('bad arguments in gammp')
    if(x.lt.a + 1.)then
        call gser(gamser, a, x, gln)
        gammp = gamser
        !dub    print *,'gamser=',gamser
    else
        call gcf(gammcf, a, x, gln)
        gammp = 1. - gammcf
        !dub    print *,'gammcf=',gammcf
    endif
    return
END
!.....
...
SUBROUTINE gser(gamser, a, x, gln)
    ...
    !dub1     gamser=sumx*dexp(-x+a*dlog(x*1.0D0)-gln)
    return
END
!  (C) Copr. 1986-92 Numerical Recipes Software <=s='%.
SUBROUTINE gcf(gammcf, a, x, gln)
    ...
    !dub1     gammcf=dexp(-x+a*dlog(x*1.0D0)-gln)*h
    return
END
!  (C) Copr. 1986-92 Numerical Recipes Software <=s='%.
FUNCTION gammln(xx)
...
subroutine mkgplot
    ...
    write(9, 1000) 'plot "ptdist.dat" title ', &
            '"Experimental data (combined)" \', &
            '     with points pt 4 ps 0.5 lt ', icolor(1), ', \'
    write(9, 1000) '     "ptdist.fit" title ', &
            '"Porter-Thomas fit (combined)" \', &
            '     with line lt ', icolor(2), ', \'
    write(9, 1000) '     "ptdist0.dat" title ', &
            '"Experimental data (s-wave)" \', &
            '     with points pt 4 ps 0.5 lt ', icolor(3), ', \'
    write(9, 1000) '     "ptdist0.fit" title ', &
            '"Porter-Thomas fit (s-wave)" \', &
            '     with line lt ', icolor(4), ', \'
    write(9, 1000) '     "ptdist1.dat" title ', &
            '"Experimental data (p-wave)" \', &
            '     with points pt 4 ps 0.5 lt ', icolor(5), ', \'
    write(9, 1000) '     "ptdist1.fit" title ', &
            '"Porter-Thomas fit (p-wave)" \', &
            '     with line lt ', icolor(6), ''
    close(9)
    irt = system("gnuplot ptdist.gp")
    return
end
!.....
! cmattoon: the pause statement is obsolete in f90, this subroutine
! gives equivalent function
! NOT useful if we plan on parallelizing however.
subroutine pause(message)
    character (len = *) message
    !     intent(in) message
    print *, message
    !     now pause, wait for input:
    read (*, *)
    return
end
