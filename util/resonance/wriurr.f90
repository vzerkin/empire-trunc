program wriurr
    !
    !     MODIFIED BY Y.S.CHO (Jan 31, 2007)
    !     APENDING RESONANCE PARAMETERS TO ENDFR.TXT
    !     INPUT FILES = WRIURRX.INP
    !                   ENDFR.TXT
    !     OUTPUT FILES = WRIURR.LIS (STANDARD OUTPUT)
    !                    ENDFU.TXT
    !
    ...
    nseq = 0
    call wrcont(0.0, 0.0, 0, 0, 0, 0)
    !cho  the following two routines are inserted for file 32 (covariances)
    call rd32151
    call wurrcov
    mt = 0
    nseq = 99999
...
subroutine wurr
    ...
    data wv/'s', 'p', 'd'/
    !
    !     sf(3)  = strength functions for s-, p-, and -d waves [1E-4]
    !     gg(3)  = average gamma widths
    !     ds0(3) = level spacings [eV] ; usually need ds0(1) only.
    !              If ds0(2) or ds0(3) is given, write energy-independent D & Gn^l.
    !     e0     = lowest energy for URR [eV]
    !     ne     = number of energy intervals ; OBSOLETE
    !     de(300)= energy intervals [eV]
    !     bn     = binding energy [MeV]
    !     pair   = pairing energy [MeV]
    !     dena   = level density parameter [MeV^-1]
    !     disp   = spin dispersion parameter, sigma
    !     icon(3)= If .eq.1, energy-dependent D_l,J & Gn^l_J by Mughabghab's formula
    !                        (but still constant strength function)
    !              Otherwise, energy-independent D and Gn^l
    !cho  gpow   = power related to computation of gamma width
    !
    write(7, 2000) awri, spin
    2000 format(//5x, 'Writing URR section with AWRI=', f10.5, &
            5x, 'Target spin=', f7.1)
    print 2000, awri, spin
    !cho  compute a default pairing energy
    iz = int(za / 1000)
    ia = za - iz * 1000 + 1
    in = ia - iz
    ...
    !cho  print *, 'z=',iz,' n=',in,' a=',ia,' pair=',pair
    !cho
    gpow = 2.5
    do ll = 1, 3
        sf(ll) = 0.0
    ...
    read(4, urr)
    close(4)
    !  count max-wave from input value of sf
    do ll = 1, 3
    ...
    2045 format(/7x, 'neutron separation energy', f7.3, ' MeV'&
            /7x, 'pairing energy           ', f6.2, '  MeV'&
            /7x, 'level density parameter  ', f6.2, ' /MeV'&
            /7x, 'spin dispersion parameter', f6.2)
    !
    ! For cases with only (2J+1) dependence (without considering spin dispersion)
    !
    dratio(1) = 1.
    dratio(2) = 3.
    dratio(3) = 5.
    ...
    do n = 1, 300
        ...
        ne = n + 1
    enddo
    !cho 200  write(7,2050) ne,e0,euner
    ...
    if(abs(e0 - endres).gt.0.01) then
        ...
        2059  format(' *** End of RRR (', 1pe10.3, ' ) do not match', &
                ' with begin of URR (', 1pe10.3, ' )')
    endif
    !     Unresolved parameters with LRF=2 (parameters are energy dept if icon=1)
    ...
    do ll = 1, NLS
        L = ll - 1
        !      compute number of resonance spins
        ...
        if(ds0(ll).eq.0.0) then
            !         if(icon(ll).eq.1) dratio(ll)=gsum(ll)/gsum(1)
        ...
        !
        do J = 1, NJS
            !       interpolation law is lin-lin(INT=2)
            ne6 = 6 * ne + 6
            ...
            do n = 1, ne
                ...
                ds = dsp
                !
                !     dsj= ds * gsum/gfdisp(J)     ! considering spin dispersion parameter
                ...
                gn = 1.0e-4 * sf(ll) * dsj
                !cho  introduce new variable "gpow"
                ggr = gg(ll) * ((bn - pair + ener / 1.0e6) / (bn - pair))**gpow
                !cho  call wrlist(ener,dsj,0.0,gn,0.001*gg(ll),0.0)
...
subroutine rd1451
    ...
    if(nwd.eq.0) then
        !      consider if 32 cols are blanks and col 33 is 1 then nxc section
        nwdr = 0
        blk32 = '                                '
        do n = 1, 3000
            read(1, 1000) txt
...
subroutine rd32151
    ...
    !cho NER=1 for test
    170 call wrcont(za, 1.0, 0, lfw, 2, 0)
    ! 170 call wrcont(za,1.0,0,lfw,1,0)
    !     Handle for resolved energy region
    read(1, 1100) el, eh, lru, lrf, nro, naps
    write(7, 2500) el, eh
    2500 format(//2x, 'Resolved res. region:', 1pe11.3, ' -', e11.3, ' eV')
