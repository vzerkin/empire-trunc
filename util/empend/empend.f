...
C-M     interval on the EMPIRE output file. The dubintervals
...
C-M
C-
      PARAMETER   (MXE=100,MXT=200,MXM=20,MXR=80000,MXI=400)
...
C* Write the ENDF file-4 data
      MT4=2
  420 CALL WRIMF4(LOU,IWO(MTH),QQI,NXS,MT6,RWO(LA)
...
C* Read and check the energy
  201 READ (REC(51:60),994) E
      E = E*1.E6
      IF(NEN.LE.0 .OR. E.GT.EIN(NEN)) GO TO 206
      WRITE(LTT,902) ' EMPEND WARNING - Double energy point eV',E
C* Skip the double energy point
...
C* Next product nucleus data
  210 READ (REC(20:29),802) JZ,CH,JA
      JZA=JZ*1000+JA
      IF(JZA  .EQ. IZA  ) READ (REC(37:46),994) AWR
C* Test for radiative capture cross section
      IF(JZA  .EQ. IZA+1) GO TO 300
C* Test for discrete levels inelastic scattering cross section
      IF(JZA  .EQ. IZA     ) THEN
                             MT0=50
                             GO TO 350
                             END IF
C* Test for discrete levels (n,p) cross section
      IF(JZA  .EQ. IZA-1000) THEN
                             MT0=600
                             GO TO 350
                             END IF
C* Test for discrete levels (n,a) cross section
      IF(JZA  .EQ. IZA-2003) THEN
                             MT0=800
                             GO TO 350
                             END IF
C* Test for (n,2n) cross section
      IF(JZA  .EQ. IZA-1   ) THEN
                             MT =   16
                             GO TO 310
                             END IF
C* Test for (n,3n) cross section
      IF(JZA  .EQ. IZA-2   ) THEN
                             MT =   17
                             GO TO 310
                             END IF
C* Test for (n,na) cross section
      IF(JZA  .EQ. IZA-2004) THEN
                             MT =   22
                             GO TO 310
                             END IF
C* Test for (n,2na) cross section
      IF(JZA  .EQ. IZA-2005) THEN
                             MT =   24
                             GO TO 310
                             END IF
C* Test for (n,3na) cross section
      IF(JZA  .EQ. IZA-2006) THEN
                             MT =   25
                             GO TO 310
                             END IF
C* Test for (n,np) cross section
      IF(JZA  .EQ. IZA-1001) THEN
                             MT =   28
                             GO TO 310
                             END IF
C* Test for (n,2np) cross section
      IF(JZA  .EQ. IZA-1002) THEN
                             MT =   41
                             GO TO 310
                             END IF
C* Test for (n,3np) cross section
      IF(JZA  .EQ. IZA-1003) THEN
                             MT =   42
                             GO TO 310
                             END IF
C* Test for (n,npa) cross section
      IF(JZA  .EQ. IZA-3005) THEN
                             MT =   45
                             GO TO 310
                             END IF
C* Test for (n,a) cross section
      IF(JZA  .EQ. IZA-1001) THEN
                             MT =  107
                             GO TO 310
                             END IF
C* Test for (n,2a) cross section
      IF(JZA  .EQ. IZA-4007) THEN
                             MT =  108
                             GO TO 310
                             END IF
C* Test for (n,2p) cross section
      IF(JZA  .EQ. IZA-1999) THEN
                             MT =  111
                             GO TO 310
                             END IF
C* Test for (n,pa) cross section
      IF(JZA  .EQ. IZA-3004) THEN
                             MT =  112
                             GO TO 310
                             END IF
C* For decay products with non-zero x-sect. print warning
...
C* Reconstruct Q values from MT and the binding energies
      ...
      DO 318 I=1,IMT
      KZA=IZB(I)
C* Neutron and proton (and alpha) emission
      IF(KZA.EQ.IZA  .AND.(MT.EQ.28.OR.MT.EQ.45) ) GO TO 316
C* Neutron and alpha emission
      IF(KZA.EQ.IZA  .AND.(MT.EQ.22) ) GO TO 317
C* Two neutrons emission (and a particle)
      IF(KZA.EQ.IZA  .AND.(MT.EQ.16.OR.MT.EQ.24.OR.MT.EQ.41)) GO TO 315
C* Three neutrons emission (and a particle)
      IF(KZA.EQ.IZA-1.AND.(MT.EQ.17.OR.MT.EQ.25.OR.MT.EQ.42)) GO TO 315
C* Two neutrons and alpha emission
      IF(KZA.EQ.IZA-1.AND.(MT.EQ.24) ) GO TO 317
C* Three neutrons and alpha emission
      IF(KZA.EQ.IZA-2.AND.(MT.EQ.25) ) GO TO 317
C* Two neutrons and proton emission
      IF(KZA.EQ.IZA-1.AND.(MT.EQ.41) ) GO TO 316
C* Three neutrons and proton emission
      IF(KZA.EQ.IZA-2.AND.(MT.EQ.42) ) GO TO 316
C* Neutron, proton and alpha emission
      IF(KZA.EQ.IZA-1000.AND.(MT.EQ. 45) ) GO TO 317
C* Two alpha emission
      IF(KZA.EQ.IZA+1   .AND.(MT.EQ.108) ) GO TO 317
      IF(KZA.EQ.IZA-2003.AND.(MT.EQ.108) ) GO TO 317
C* Two proton emission
      IF(KZA.EQ.IZA+1   .AND.(MT.EQ.111) ) GO TO 316
      IF(KZA.EQ.IZA-999 .AND.(MT.EQ.111) ) GO TO 316
C* Proton and alpha emission
      IF(KZA.EQ.IZA+1   .AND.(MT.EQ.112) ) GO TO 316
      IF(KZA.EQ.IZA-999 .AND.(MT.EQ.112) ) GO TO 317
      GO TO 318
C* Subtract binding energy of the neutron
  315 QQM(IXS)=QQM(IXS)-1.E6*BEN(1,I)
      GO TO 318
C* Subtract binding energy of the proton
  316 QQM(IXS)=QQM(IXS)-1.E6*BEN(2,I)
      GO TO 318
C* Subtract binding energy of the alpha particle
  317 QQM(IXS)=QQM(IXS)-1.E6*BEN(3,I)
  318 CONTINUE
      QQI(IXS)=QQM(IXS)
  320 XSC(NEN,IXS)=X*1.E-3
      GO TO 110
C*
C* Process discrete levels - inelastic, (n,p), (n,a)
  350 READ (LIN,891)
      READ (LIN,891) REC
...
C* Positioned to read discrete levels
      ...
      READ (LIN,891)
      XI=0.
  352 READ (LIN,805) IL,EL,X
      XI=XI+X
      MT=MT0-1+IL
      IF(MT.EQ.50) GO TO 352
      IF(IL.LE.0 ) GO TO 351
C     IF(X .LE.0 ) GO TO 352
      IF(NXS.LE.0) GO TO 354
      DO 353 I=1,NXS
      IXS=I
      IF(MTH(I).EQ.MT) GO TO 360
  353 CONTINUE
  354 NXS=NXS+1
      IF(NXS.GT.MXT) STOP 'EMPEND ERROR - MXT limit exceeded'
      IXS=NXS
...
C* Reconstruct Q values from MT and the binding energies
  ...
  358 CONTINUE
      QQI(IXS)=QQM(IXS)-1.E6*EL
      QM =QQM(IXS)
      QI =QQI(IXS)
  360 XSC(NEN,IXS)=X*1.E-3
      GO TO 352
...
C* Positioned to read continuum cross section (subtract discrete levels)
  390 continue
      READ (REC,803) XC
      X =XC-XI
      IF(XC.GT.0 .AND. ABS(X/XC).LT.2.E-5) X=0.
      IF(MT0.EQ.50 ) MT=91
      IF(MT0.EQ.600) MT=649
      IF(MT0.EQ.800) MT=849
      IF(NXS.LE.0  ) GO TO 394
      DO 393 I=1,NXS
      IXS=I
      IF(MTH(I).EQ.MT) GO TO 396
  393 CONTINUE
  394 NXS=NXS+1
      IF(NXS.GT.MXT) STOP 'EMPEND ERROR - MXT limit exceeded'
      IXS=NXS
...
C* Number of input angles MXA and number of particles per react. MXP
      PARAMETER    (MXA=80, MXP=6, MXZ=160)
...
C* Read the energy/angle distribution data
  600 IF(MT6.LT.0 .AND.
     1  (REC(15:22).EQ.'recoils' .OR.
     2   REC(15:22).EQ.'gammas ')    ) GO TO 110
      MT =0
      E0 =EE
      JT6=ABS(MT6)
C* Identify the reaction and assign the MT number
...
C* Case: Reaction already defined - check for matching
  620 IF(MTH(IT).NE.MT) GO TO 110
      IF(REC(15:22).EQ.POUT(IK)) GO TO 624
      DO 622 JK=1,NK
      IF(REC(15:22).EQ.POUT(JK)) GO TO 110
  622 CONTINUE
      NK=NK+1
...
C* Save the energy and the cross section
      ...
      ELSE
        IF(EIS(NE6).NE.ETH) THEN
           WRITE(LTT,995) ' EMPEND ERROR - Inconsistent E-mesh MF 6'
           WRITE(LER,995) ' EMPEND ERROR - Inconsistent E-mesh MF 6'
...
C-Purpose: Write cross section (file-3) data in ENDF-6 format
...
C* Select MT numbers in ascending order
      MT =999
      DO 302 J=1,NXS
      IF(MTH(J).LT. 0) GO TO 302
      IF(MTH(J).GT.MT) GO TO 302
      IT =J
      MT =MTH(IT)
  302 CONTINUE
...
C* Define the output grid for spline interpolated function
      ...
      RWO(LX)=EE
      DO 324 J=2,NEN
      DE=(EIN(J)-EIN(J-1))/FLOAT(NEP)
  323 IF(EE.GE.EIN(J) .OR. NE1.GE.NEO) GO TO 324
          EE=MIN(EIN(J),EE+DE)
          IF(ABS(EIN(J)-EE).LT.0.2*DE) EE=EIN(J)
          NE1=NE1+1
          RWO(LX-1+NE1)=EE
          GO TO 323
  324 CONTINUE
      RWO(LX-1+NE1)=EIN(NEN)
C* Log-lin interpolate from threshold
  ...
  326 IF(RWO(JY).GT.SMALL) GO TO 330
     ...
     1 +(RWO(JY+1)-RWO(JY))*(RWO(KX)-RWO(JX))/(RWO(JX+1)-RWO(JX))
      ...
      JE=JE-1
      GO TO 326
C* Spline fit log of the cross section
  330 F1=0.
      F2=0.
...
C* Write file MF4 angular distributions (first outgoing particle)
   22 MF =4
      ...
      IF(MT.EQ.2) THEN
        EOU=0
      ELSE
        EOU=MAX(-QQI(IT)*(AWR+1.)/AWR , EI2 )
   24   NW =RWO(L1+2)+0.1
        L1 =L1+4+NW
        EI1=EI2
        EI2=RWO(L1)
        IF(EI1.LT.EOU .AND. EI2.LT.EOU) THEN
          JE=JE-1
          GO TO 24
        END IF
      END IF
...
C* Trap zero-determinant
      ...
      QQ(I)=RWO(I)
   21 CONTINUE
C* Check the absolute difference between input and calculated points
C* Check for negative distributions ( < -EPZER0 )
   22 ERR=0.
      ENE=0.
      EPZERO=1.E-10
      DO 24 IP=1,NP
