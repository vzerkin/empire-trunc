...
! *                    1. CHECKR CONVERTED TO FORTRAN-77
...
! *                    4. MF IS ALLOWED IN SUBLIBRARY NSUB
...
! *                    4. RESONANCE REGION CHANGES OF MAY 1988 CSEWG
! *      VERSION 6.5   APRIL 1989  C.L.DUNFORD
! *                    1. CORRECTIONS TO VERSION 6.4
! *                    2. NEW FORMATS FOR FILES 32, 34, 35 AND 40
! *                    3. CHECK FOR CORRECT MAT NUMBER FOR ENDF/B
! *                    4. ADD LB=8 FOR FILE 33
! *      VERSION 6.6   JUNE 1990  C.L.DUNFORD
! *                    1. CORRECTIONS TO VERSION 6.5
! *                    2. CHECK MAT VS ZA FOR COMPOUNDS
! *                    3. PHOTON INTERACTION FORMAT EXTENTIONS
! *                    4. LOG STORAGE OF S(ALPHA,BETA)
! *      VERSION 6.7   JUNE 1991  C.L.DUNFORD
! *                    1. CORRECTIONS TO VERSION 6.6
! *                    2. ADDITION OF PION PRODUCTION MTS
...
! *                    2. INCREASED NUMBER OF SECTIONS ALLOWED
! *      VERSION 6.10  NOVEMBER 1995 C.L.DUNFORD
! *                    1. ADDED RECOGNITION OF MAT NO. FOR S-METHANE
! *                    2. ADDED RECOGNITION OF NEW MT'S,11,44,45,117
! *                    3. ALLOW 12 RESONANCE REGION ENERGY RANGES.
! *                    4. CORRECTED INTERPOLATION CHECK TO ALLOW INT=6
! *                       FOR CHARGED PARTICLE PROJECTILE IN FILE 6
...
! *                    4. ALLOW LCT=3 IN MF=6
! *                    5. IMPLEMENT REVISED SEQUENCING SYSTEM
! *                    6. ADD NEW 451 FIELDS NREL AND EMAX
! *                    7. NEW LTT=3 REPRESENTATION IN FILE 4
! *                    8. INCREASED LIMIT ON FILE 6 SUBSECTIONS TO 1000
! *      VERSION 6.12  FEBRUARY 2001     C.L.DUNFORD
! *                    1. CODE MEETS F90 FIXED FORMAT STANDARD
! *                    2. PC VERSION USING DIGITIAL VISUAL FORTRAN
! *                    3. MOVE NREL TO L1 FIELD FROM L2 FIELD
! *                    4. OPTIONALY USE A DASH TO ENTER A MAT NUMBER
! *                       RANGE
! *                    5. ADDED JEF MAT NUMBER CHECK
! *                    6. ALLOW 5000 RESONANCES PER L-VALUE
! *                    7. CHECK THAT FOR A GIVEN MT, NK IS THE SAME IN
! *                       MF=14 AS IN MF=12 OR MF=13
...
! *                    3. SUPPRESS SEQUENCE AND DIRECTORY ERROR CHECKING
! *                        WHEN FILE NOT PROCESSED BY STANEF.
! *                    4. CORRECTED LIMIT TESTS FOR FILE 6 LAWS 1 AND 5
! *                    5. FORMAT MODIFICATIONS FOR RADIOACTIVE PRODUCTS
! *                        IN FILES 8, 9 AMD 10
! *                    6. WINDOWS GRAPHICAL INTERFACE ADDED USING
! *                       DIGITAL VISUAL FORTRAN
! *                    7. UNIX GRAPHICAL INTERFACE ADDED USING
...
!
      INTEGER(KIND=4), PARAMETER :: NIRMAX=20   ! INTERPOLATION REGIONS
      INTEGER(KIND=4), PARAMETER :: NPTSMAX=50000 ! POINTS IN A TAB 1
      INTEGER(KIND=4), PARAMETER :: NPTS2MAX=2000 ! POINTS IN A TAB 2
!
!     ALLOWED INTERPOLATION SCHEMES
!
      INTEGER(KIND=4), PARAMETER :: INTMAX=5
!
!     CONTENTS OF FIRST RECORD AND INTERPOLATION TABLE FOR A TAB1 RECORD
!
      INTEGER(KIND=4) L1,L2,NR,NP
      INTEGER(KIND=4), DIMENSION(NIRMAX) :: NBT,JNT
      REAL(KIND=4)    C1,C2
!
!     CONTENTS OF FIRST RECORD AND INTERPOLATION TABLE FOR A TAB2 RECORD
!
      INTEGER(KIND=4) L12,L22,NR2,NP2
      INTEGER(KIND=4), DIMENSION(NIRMAX) :: NBT2,JNT2
      REAL(KIND=4)    C12,C22
!
...
!
      ...
      REAL(KIND=4)    C1L,C2L
      REAL(KIND=4), DIMENSION(4) :: BIN,BIN1
!
!     POSSIBLE DATA REPETITION RATES ON A LIST RECORD
!
      INTEGER(KIND=4), PARAMETER :: NREP6 = 6,NREP12 = 12,NREP18=18
!
...
!       SAID TO BE EQUAL
!
      REAL(KIND=4), PARAMETER ::  EPS = .00001
!
!     NXC   NUMBER OF SECTIONS ENCOUNTERED
!     NXC0  NUMBER OF SECTIONS IN THE DIRECTORY
!
      INTEGER(KIND=4), PARAMETER :: NSECMAX=350
      INTEGER(KIND=4) NXC,NXC0
      INTEGER(KIND=4), DIMENSION(NSECMAX,2):: INDX
!
!     SET SOME IMPORTANT LIMITS
!
!          FILE 1 COMMENTS
      INTEGER(KIND=4), PARAMETER :: NCDMI=3,NCDMA=999999 !COMMENTS
!          FILE 1 NUBAR
      INTEGER(KIND=4), PARAMETER :: NCOMAX=4   ! NUBAR COEFFICIENTS
!          FILE 2 RESONANCE DATA
      INTEGER(KIND=4), PARAMETER :: NISMAX=20  ! NUMBER OF ISOTOPES
      INTEGER(KIND=4), PARAMETER :: NERM6=12,NERM5=2 ! RESONANCE RANGES
      INTEGER(KIND=4), PARAMETER :: LRFM6=6,LRFM5=2 ! ALLOWED REPRES
      INTEGER(KIND=4), PARAMETER :: NSCLMAX=50  ! SCATTERING LENGTHS
      INTEGER(KIND=4), PARAMETER :: NLSMAX=4   ! RESONANCE L-VALUES
      INTEGER(KIND=4), PARAMETER :: NLSCMAX=20   ! L-VALUES FOR DSIGMA
      INTEGER(KIND=4), PARAMETER :: NJSMAX=6   ! RESONANCE J-VALUES
      INTEGER(KIND=4), PARAMETER :: NRESMAX=5000 ! RESONANCES PER L
      INTEGER(KIND=4), PARAMETER :: NGREMAX=1,NFREMAX=1,NIREMAX=4,      &       
     &              NCREMAX=4      ! MAXIMUM OF REACTIONS PER TYPE
      INTEGER(KIND=4), PARAMETER :: URNEMAX = 250 !UR ENERGY POINTS
!          FILE 4 SECONDARY ANGULAR DATA
      INTEGER(KIND=4), PARAMETER :: NES4MAX=1500 ! NUMBER OF E(INC)
      INTEGER(KIND=4), PARAMETER :: NLEGMAX=64  ! LEGENDRE COEFS
      INTEGER(KIND=4), PARAMETER :: NANGMAX=101 ! ANGLE POINTS
!          FILE 5 SECONDARY ENERGY DATA
      INTEGER(KIND=4), PARAMETER :: NSUBSMAX=100!  SUBSECTION LIMIT
      INTEGER(KIND=4), PARAMETER :: NES5MAX=200 ! NUMBER OF E(INC)
      INTEGER(KIND=4), PARAMETER :: NEDISMAX=1000 ! ENERGY POINTS
!          FILE 6 SECONDARY ENERGY-ANGLE DATA
      INTEGER(KIND=4), PARAMETER :: NSUBS6MAX=1000! SUBSECTION LIMIT
!          FILE 7 THERMAL SCATTERING LAW
      INTEGER(KIND=4), PARAMETER :: NPSAMAX=3     ! NONPRINCIPAL ATOMS
      INTEGER(KIND=4), PARAMETER :: NSMTMAX=100   ! SUBSEQUENT TEMPS
!          FILE 8 FISSION YIELDS AND DECAY DATA
      INTEGER(KIND=4), PARAMETER :: NFPMAX=2500   ! NUMBER OF YIELDS
      INTEGER(KIND=4), PARAMETER :: NDSTMAX=100 ! PRODUCT STATES
!          FILES 12 AND 13 PHOTON PRODUCTION
      INTEGER(KIND=4), PARAMETER :: NPHMAX=1000 ! DISCRETE PHOTONS
!
      END MODULE CHECKR_DEF
!
!***********************************************************************
!
      MODULE CHECKR_MDC
...
!
      USE CHECKR_DEF, ONLY : INPAR,ILENP
!
      IMPLICIT NONE
!
      INPAR = ' '
...
!+++MDC+++
!...DVF, LWI
!/      USE CHECKR_DEF, ONLY : TEXT
!/      USE CHECKR_WIN_DEF, ONLY :IRERUN
!---MDC---
!
      IMPLICIT NONE
!
...
!+++MDC+++
!...DVF, LWI
!/      USE CHECKR_DEF, ONLY : MATO,MFO,MTO
!/      USE CHECKR_WIN_DEF, ONLY :IRERUN
!---MDC---
!
      IMPLICIT NONE
...
!...VMS, ANS, WIN, UNX
      PROGRAM CHECKR
!
!     MAIN PROGRAM FOR NON-GRAPHICAL MODES
!
      USE CHECKR_DEF, ONLY : IOUT
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) ISUCCESS
!
      CALL CHECKR_RUN(ISUCCESS)
!
!     TERMINATE JOB
!
...
!***********************************************************************
!
      SUBROUTINE CHECKR_RUN(ISUCCESS)
!
...
! **********************************************************************
!
      USE CHECKR_DEF, ONLY : IOUT,NOUT,JIN
      USE CHECKR_DEF, ONLY : VERSION
      USE CHECKR_DEF, ONLY : I_DATA
      USE CHECKR_DEF, ONLY : IFIN,IONEPASS
      USE CHECKR_DEF, ONLY : MATO,MFO,MTO
      USE CHECKR_DEF, ONLY : MAT,MF,MT
      USE CHECKR_DEF, ONLY : NSUB
      USE CHECKR_DEF, ONLY : LRP,LFI
      USE CHECKR_DEF, ONLY : NXC
      USE CHECKR_DEF, ONLY : INDX
      USE CHECKR_DEF, ONLY : MTLP
      USE CHECKR_DEF, ONLY : I452
      USE CHECKR_DEF, ONLY : N12S
      USE CHECKR_DEF, ONLY : NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      USE CHECKR_MDC, ONLY : IMDC
      USE CHECKR_MDC, ONLY : GET_FROM_CLINE,OUT_STATUS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) ISUCCESS
...
!
      IF(MAT.NE.MATO)  THEN  ! NEW MATERIAL
         ...
         N12S = 0
         MFO = 0
         WRITE(NOUT,'(A/4X,A,I5)')  '1','CHECK MATERIAL',MATO
         WRITE(NOUT,'(20X,A)')                                          &       
      ...
      IF(MF.NE.MFO)   THEN           ! NEW FILE
         ...
         ELSE
            IF(IFL2.EQ.0.AND.LRP.GE.0)  THEN
               WRITE(NOUT,'(/1X,A/1X,A)')  DASHES,'FILE  2'
               WRITE(NOUT,'(4X,A)')  'SECTION  151'
...
!     NEW SECTION
!
   35 WRITE(NOUT,'(4X,A,I3)')  'SECTION ',MT
      MTO = MT
!
...
!
      USE CHECKR_DEF, ONLY : NIN,NOUT,IOUT,JOUT,JIN
      USE CHECKR_DEF, ONLY : VERSION
      USE CHECKR_DEF, ONLY : I_DATA
      USE CHECKR_DEF, ONLY : INPAR,ILENP
      USE CHECKR_DEF, ONLY : IFIN,IONEPASS
      USE CHECKR_DEF, ONLY : MATO,MFO,MTO
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : IERX
!
      USE CHECKR_MDC, ONLY : IMDC,TFMT,OSTATUS
!
      IMPLICIT NONE
!
...
!
     ...
     &            '1',' PROGRAM CHECKR VERSION ',VERSION,               &       
     &            'Run on ',DATE()
      END IF
      WRITE(NOUT,'(2A)')                                                &       
     &   ' Input File Specification------------------------',           &       
...
!
      USE CHECKR_DEF, ONLY : NIN,IOUT
      USE CHECKR_DEF, ONLY : I_DATA
!
      USE CHECKR_MDC, ONLY : TFMT
!
      IMPLICIT NONE
!
...
!
      USE CHECKR_DEF, ONLY : JIN,NOUT,IOUT
      USE CHECKR_DEF, ONLY : I_DATA
      USE CHECKR_DEF, ONLY : MAT,MF,MT
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : TLABEL,LABEL
      USE CHECKR_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ,NSEQ,NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      USE CHECKR_MDC, ONLY : IMDC
...
!
      USE CHECKR_DEF, ONLY : NSUB,NFOR
      USE CHECKR_DEF, ONLY : MTO
      USE CHECKR_DEF, ONLY : MF
      USE CHECKR_DEF, ONLY : ZA,AWR
      USE CHECKR_DEF, ONLY : LRP
      USE CHECKR_DEF, ONLY : C1H,C2H
      USE CHECKR_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) ISUBTP
!
...
!
      ...
      END IF
      WRITE(NOUT,'(6X,A,I3,1X,A)') 'FILE',MF,EMESS
!
      RETURN
...
!
      USE CHECKR_DEF, ONLY : JIN,NOUT
      USE CHECKR_DEF, ONLY : IFIN
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : MATO,MFO,MTO
      USE CHECKR_DEF, ONLY : MAT,MF,MT
      USE CHECKR_DEF, ONLY : MTP
      USE CHECKR_DEF, ONLY : NSEQ,ISEQ,ASEQ
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      WRITE(NOUT,'(6X,A,I3,A,I4,A,I6,A,I6)') 'MF=',MFO,' MT=',MTO,      &       
...
!
      USE CHECKR_DEF, ONLY : NSUB
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : I452
      USE CHECKR_DEF, ONLY : NCOMAX
      USE CHECKR_DEF, ONLY : L2H,NPL,N2L
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NUREP
!
      INTEGER(KIND=4), PARAMETER :: LNUMAX=2,LNUMAXS4=1
!
!     MF = 1, MT = 451
!
...
!
      USE CHECKR_DEF, ONLY : IOUT,NOUT
      USE CHECKR_DEF, ONLY : NLIB,NVER,NSUB,NMOD,NFOR
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MAT
      USE CHECKR_DEF, ONLY : ZA,AWR,STA,LIS,LISO,AWI
      USE CHECKR_DEF, ONLY : LRP,LFI,LDRV
      USE CHECKR_DEF, ONLY : NXC,NXC0
      USE CHECKR_DEF, ONLY : INDX
      USE CHECKR_DEF, ONLY : NCDMI,NCDMA
      USE CHECKR_DEF, ONLY : NSECMAX
      USE CHECKR_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : TEXT
      USE CHECKR_DEF, ONLY : NSEQP1,NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      USE CHECKR_MDC, ONLY : IMDC
      USE CHECKR_MDC, ONLY : CURRENT_MAT
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IZ,IA,ISTA
      INTEGER(KIND=4) NREL
      INTEGER(KIND=4) NCD,NID
      INTEGER(KIND=4) MATCHK,MATASS,MATASS_JEF
      INTEGER(KIND=4) JPART,JSUB,JTYPE
      INTEGER(KIND=4) IEF
      INTEGER(KIND=4) MFT,MFTP
      INTEGER(KIND=4) N1
      INTEGER(KIND=4) K,N,NC
      REAL(KIND=4) EMAX,TEMP
!
      INTEGER(KIND=4), PARAMETER :: NPARTS = 8
      INTEGER(KIND=4), DIMENSION(NPARTS) :: IPART
      DATA IPART/0,1,11,1001,1002,1003,2003,2004/
!
...
!
      IF(NFOR.GE.6) THEN
         IF((NLIB.EQ.0.OR.NLIB.EQ.4)) THEN
            MATCHK = MATASS(IZ,IA,LISO)
         ELSE IF(NLIB.EQ.2) THEN
            MATCHK = MATASS_JEF(IZ,IA,LISO)
         ELSE
            GO TO 10
         END IF
...
!***********************************************************************
!
      INTEGER(KIND=4) FUNCTION MATASS(IZ,IA,LIS0)
!
...
!
      ...
      INTEGER(KIND=4) N
!
      INTEGER(KIND=4), PARAMETER :: NELS = 103
      INTEGER(KIND=4), DIMENSION (NELS) :: IMEAN
      DATA IMEAN/                                                       &       
...
!
      INTEGER(KIND=4), DIMENSION(4) :: ISP
      DATA ISP /9920,9945,9965,9980/
!
      INTEGER(KIND=4), PARAMETER :: NCOMPS = 18
      INTEGER(KIND=4), DIMENSION(NCOMPS) :: ZACOMP
      DATA ZACOMP/101,102,103,107,111,112,113,126,127,128,              &       
...
!***********************************************************************
!
      INTEGER(KIND=4) FUNCTION MATASS_JEF(IZ,IA,LIS0)
!
!     FUNCTION TO CONVERT A  CHARGE NUMBER, A MASS NUMBER AND A
...
!
      INTEGER(KIND=4), PARAMETER :: NELS = 103
      INTEGER(KIND=4), DIMENSION (NELS) :: IMEAN
      DATA IMEAN/1,3,6,9,10,12,14,16,19,20,23,24,27,28,31,32,           &       
...
!
      USE CHECKR_DEF, ONLY : ZA
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : LRP
      USE CHECKR_DEF, ONLY : NISMAX,NERM6,NERM5
      USE CHECKR_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
...
!
      ...
      INTEGER(KIND=4) MLRP
      INTEGER(KIND=4) JPOT
      INTEGER(KIND=4) LFW,LRU
      INTEGER(KIND=4) NER,NERM
      INTEGER(KIND=4) N,NI
!
...
!
      USE CHECKR_DEF, ONLY : NLIB,NFOR
      USE CHECKR_DEF, ONLY : NLSMAX,NLSCMAX,NSCLMAX
      USE CHECKR_DEF, ONLY : LRFM5,LRFM6
      USE CHECKR_DEF, ONLY : C2H,N1H,N2H,L1H,L2H
      USE CHECKR_DEF, ONLY : NP
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LRF,LRFM
      INTEGER(KIND=4) NRO
      INTEGER(KIND=4) NAPS,NAPSM
      INTEGER(KIND=4) NLS,NLSC
      REAL(KIND=4) AP
!
      LRF = L2H
!
...
!
      USE CHECKR_DEF, ONLY : NRESMAX
      USE CHECKR_DEF, ONLY : C2L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NLS,LRF
!
      INTEGER(KIND=4) NRS,NUM
      INTEGER(KIND=4) NL
!
      INTEGER(KIND=4), PARAMETER :: NREP=6
!
      DO NL=1,NLS
         CALL RDLIST
...
!
      USE CHECKR_DEF, ONLY : NLIB
      USE CHECKR_DEF, ONLY : NREP6,NREP12
      USE CHECKR_DEF, ONLY : NJSMAX,NRESMAX
      USE CHECKR_DEF, ONLY : N1H,L1L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      USE CHECKR_DEF, ONLY : NLSMAX,NRESMAX
      USE CHECKR_DEF, ONLY : NGREMAX,NFREMAX,NIREMAX,NCREMAX
      USE CHECKR_DEF, ONLY : NREP12
      USE CHECKR_DEF, ONLY : L1H,L2H,N1H,N2H,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : BIN1
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      INTEGER(KIND=4) NLS
!
      INTEGER(KIND=4) NSS,NJS,NLSJ
      INTEGER(KIND=4) NGRE,NFRE,NIRE,NCRE,NMTRE
      INTEGER(KIND=4) NCR,LIL,IJI
      INTEGER(KIND=4) L
      INTEGER(KIND=4) LBK,LPS
      INTEGER(KIND=4) NUM
      INTEGER(KIND=4) NL,NS,NJ
!
      INTEGER(KIND=4), PARAMETER :: NMTRES=4
      INTEGER(KIND=4), DIMENSION(NMTRES) :: MTRE
!
!     HYBRID R-FUNCTION
!
      CALL RDCONT
...
!
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : NLSMAX,NJSMAX,URNEMAX
      USE CHECKR_DEF, ONLY : NREP6
      USE CHECKR_DEF, ONLY : L1H,L2H,N1H,NPL,N2L
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      INTEGER(KIND=4) LRF
      INTEGER(KIND=4) LSSF,LSSFC
      INTEGER(KIND=4) NLS,NJS
      INTEGER(KIND=4) NL,NJ
      INTEGER(KIND=4) NE,NET
      INTEGER(KIND=4) NUM
!
      INTEGER(KIND=4), PARAMETER :: LSSFC6 = 1,LSSFC5 = 0
!
...
!
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LFW,LRF
!
      INTEGER(KIND=4) NER
      INTEGER(KIND=4) NLS
!
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NIRE,NCRE
      INTEGER(KIND=4), DIMENSION(*) :: MTRE
!
      INTEGER(KIND=4) NMTRE
      INTEGER(KIND=4) NFND
      INTEGER(KIND=4) ICHK
      INTEGER(KIND=4) IGRP
      INTEGER(KIND=4) IOPRE,IOCUR
      INTEGER(KIND=4) IMTERR
      INTEGER(KIND=4) I,J
!
      INTEGER(KIND=4), DIMENSION(4) :: MTERR
      INTEGER(KIND=4), DIMENSION(2) :: MTSUM
      INTEGER(KIND=4), PARAMETER :: NMTS = 9
      INTEGER(KIND=4), DIMENSION (NMTS)::  MTSFND,VALMTS,GRPMTS
      DATA VALMTS/51,52,53,54,103,104,105,106,107/,GRPMTS/4*1,5*2/
!
!     INITIALIZE
!
      NFND = 0
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : NLEGMAX,NES4MAX,NANGMAX
      USE CHECKR_DEF, ONLY : C2,NP,NP2
      USE CHECKR_DEF, ONLY : L1H,L2H,C2L,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LVT,LTT,LCT
      INTEGER(KIND=4) NM,NPLT
      INTEGER(KIND=4) LI
      INTEGER(KIND=4) NE
      INTEGER(KIND=4) N
!
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MT
      USE CHECKR_DEF, ONLY : NSUB
      USE CHECKR_DEF, ONLY : NSUBSMAX,NES5MAX,NEDISMAX
      USE CHECKR_DEF, ONLY : N1H
      USE CHECKR_DEF, ONLY : L2,NP,NP2
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      ...
      INTEGER(KIND=4) N,NM
!
      INTEGER(KIND=4), PARAMETER :: LFMAX=12
!
!     TEST FOR A VALID MT NUMBER
!
      CALL TESTFT(MTO,MFO)
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : NXC0
      USE CHECKR_DEF, ONLY : L2H,N1H,L1L,L2L,NPL,N2L,L12,L22
      USE CHECKR_DEF, ONLY : L2,NP,NP2
      USE CHECKR_DEF, ONLY : NSUBS6MAX,NES5MAX,NLEGMAX,NANGMAX,NPTSMAX
      USE CHECKR_DEF, ONLY : NEDISMAX
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MFM1,MFM2,ISET
      INTEGER(KIND=4) LCT
      INTEGER(KIND=4) NK
      INTEGER(KIND=4) LAW
      INTEGER(KIND=4) LANG,LEP,NEP,NEPM
      INTEGER(KIND=4) NA,ND,NAT
      INTEGER(KIND=4) NE,NPM
      INTEGER(KIND=4) LTP,LIDP,NL
      INTEGER(KIND=4) NW,NWT
      INTEGER(KIND=4) NPSX
      INTEGER(KIND=4) NMU
      INTEGER(KIND=4) I,N,NU
!
      INTEGER(KIND=4), PARAMETER :: LAWMAX=7
      INTEGER(KIND=4), PARAMETER :: LANGMAX=15,LEPMAX=5
      INTEGER(KIND=4), PARAMETER :: NPSXMAX=20
!
!     TEST FOR A VALID MT NUMBER
!
      CALL TESTFT(MTO,MFO)
...
!
      ...
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MT
      USE CHECKR_DEF, ONLY : NPSAMAX,NSMTMAX,INTMAX
      USE CHECKR_DEF, ONLY : NREP6
      USE CHECKR_DEF, ONLY : L1,C1,C2,NP,NP2
      USE CHECKR_DEF, ONLY : L1H,L2H,N1H,C1L,C2L,L1L,NPL,N2L
      USE CHECKR_DEF, ONLY : BIN
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      INTEGER(KIND=4) LTHR
      INTEGER(KIND=4) LAT,LASYM,LLN,NS
      INTEGER(KIND=4) NUM
      INTEGER(KIND=4) NB
      INTEGER(KIND=4) LT
...
!
      USE CHECKR_DEF, ONLY : MTO,MFO
      USE CHECKR_DEF, ONLY : MT
      USE CHECKR_DEF, ONLY : LIS,LISO
      USE CHECKR_DEF, ONLY : C1H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : C1L,C2L,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : NFPMAX,INTMAX,NDSTMAX
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      INTEGER(KIND=4) LEP1
      INTEGER(KIND=4) NFP,NUM
      INTEGER(KIND=4) NS,NO
      INTEGER(KIND=4) LFSO,LFSP
      INTEGER(KIND=4) MATPR
      INTEGER(KIND=4) LMF,NND
      INTEGER(KIND=4) K,N
      REAL(KIND=4) ZAP,ELFS
!
!     TEST FOR A VALID MT NUMBER
!
      CALL TESTFT(MTO,MFO)
...
!
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : LIS,LISO,STA
      USE CHECKR_DEF, ONLY : NREP6
      USE CHECKR_DEF, ONLY : L2
      USE CHECKR_DEF, ONLY : L1H,L2H,N2H,C2L,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      ...
      INTEGER(KIND=4) NDKT
      INTEGER(KIND=4) NUM
      INTEGER(KIND=4) ISTP,IS
      INTEGER(KIND=4) LCON,LCOV
      INTEGER(KIND=4) NER
      INTEGER(KIND=4) LB,NPP
      INTEGER(KIND=4) N,NN
!
      REAL(KIND=4) STYPE
!
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : LISO
      USE CHECKR_DEF, ONLY : NXC0
      USE CHECKR_DEF, ONLY : NDSTMAX
      USE CHECKR_DEF, ONLY : L1,L2
      USE CHECKR_DEF, ONLY : C1H,L1H,L2H,N1H
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MFM1,ISET,INAT
      INTEGER(KIND=4) NS,LFSO,LFSP,IZAP,IZAPP
      INTEGER(KIND=4) N
!
!     TEST FOR A VALID MT NUMBER
!
...
!
      ...
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : NXC0
      USE CHECKR_DEF, ONLY : NPHMAX,NEDISMAX
      USE CHECKR_DEF, ONLY : C1,L1,L2,NP
      USE CHECKR_DEF, ONLY : L1H,N1H,L2H,L1L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      INTEGER(KIND=4) LO
      INTEGER(KIND=4) MFM1,ISET
      INTEGER(KIND=4) NK
      INTEGER(KIND=4) LP,LF
      INTEGER(KIND=4) LG,NS
      INTEGER(KIND=4) NUM
      INTEGER(KIND=4) J2
!
!     TEST FOR A VALID MT NUMBER
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NK,IPATH
!
      INTEGER(KIND=4) N
      INTEGER(KIND=4), PARAMETER :: NMTMAX=NSECMAX
      INTEGER(KIND=4), DIMENSION(NMTMAX) :: MTS,NKS
!
      IF(IPATH.EQ.0) THEN
         N12S = N12S + 1
         MTS(N12S) = MT
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : NPHMAX,NEDISMAX,NLEGMAX,NANGMAX
      USE CHECKR_DEF, ONLY : NP,NP2,NR,NR2
      USE CHECKR_DEF, ONLY : L1H,L2H,N1H,N2H,NPL
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      INTEGER(KIND=4) MFM1,MFM2,ISET
      INTEGER(KIND=4) NK,NI
      INTEGER(KIND=4) LI,LTT
      INTEGER(KIND=4) NE
      INTEGER(KIND=4) N,M
!
!     TEST FOR A VALID MT NUMBER
...
!
      USE CHECKR_DEF, ONLY : MTO,MFO
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : NES5MAX,NEDISMAX
      USE CHECKR_DEF, ONLY : L2,NP,NP2
      USE CHECKR_DEF, ONLY : N1H
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : NSUBS6MAX,INTMAX,NES5MAX,NLEGMAX,NANGMAX
      USE CHECKR_DEF, ONLY : L2,NP2
      USE CHECKR_DEF, ONLY : N1H,L12,L22,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
      USE CHECKR_DEF, ONLY : NSEQP1
!
...
!
      INTEGER(KIND=4) NK
      INTEGER(KIND=4) LAW
      INTEGER(KIND=4) LANG,LEP
      INTEGER(KIND=4) NE,NEP
      INTEGER(KIND=4) NW,NL,NWT
      INTEGER(KIND=4) ND,NA
      INTEGER(KIND=4) NAT
      INTEGER(KIND=4) I,N
!
!     TEST FOR A VALID MT NUMBER
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : NSUBS6MAX
      USE CHECKR_DEF, ONLY : N1H,C1L,NPL,N2L
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
...
!
      ...
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : ZA
      USE CHECKR_DEF, ONLY : NISMAX
      USE CHECKR_DEF, ONLY : NERM5,NERM6
      USE CHECKR_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
...
!
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : NLSMAX,NRESMAX
      USE CHECKR_DEF, ONLY : NREP18
      USE CHECKR_DEF, ONLY : LRFM5,LRFM6
      USE CHECKR_DEF, ONLY : C2H,L1H,L2H,N1H,N2H,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LRF,LRFM,LSSF,NRO,NAPS,NAPSM,NIT
      INTEGER(KIND=4) NLS,NRS,NUM,NSRS,NLRS
      INTEGER(KIND=4) MPAR,MPARM,LCOMP
      INTEGER(KIND=4) NRB,NCOUNT,NVS,IDP,IDPM
      INTEGER(KIND=4) LB,LT,NE,NE2,NPM,NTT,NEC
      INTEGER(KIND=4) I1,I3,NL
      REAL(KIND=4) AP
!
      LRF = L2H
!
...
!
      USE CHECKR_DEF, ONLY : NREP6
      USE CHECKR_DEF, ONLY : NLSMAX,NJSMAX,URNEMAX
      USE CHECKR_DEF, ONLY : L1H,L2H,N1H,L1L,N2L,NPL
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LRF,LSSF,NLS,NJS,NE,NET
      INTEGER(KIND=4) NCOUNT,MPAR,NPAR,NPART,NTT
      INTEGER(KIND=4) NL,NJ
!
      LRF = L2H
...
!
      ...
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : MTLP
      USE CHECKR_DEF, ONLY : NSUBSMAX
      USE CHECKR_DEF, ONLY : L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : NSEQP
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
...
!
      INTEGER(KIND=4) ISET
      INTEGER(KIND=4) MTL
      INTEGER(KIND=4) NL,NC,NI
      INTEGER(KIND=4) MAT1,MAT1B,MT1,MT1B
      INTEGER(KIND=4) IE
      INTEGER(KIND=4) I1
!
!     TEST FOR A VALID MT NUMBER
...
!
      INTEGER(KIND=4) ISET
      INTEGER(KIND=4) NMT1
      INTEGER(KIND=4) LEZERO
      INTEGER(KIND=4) NSEQPT
      INTEGER(KIND=4) MAT1,MT1,MT1B
      INTEGER(KIND=4) NL,NL1,NSS
      INTEGER(KIND=4) LTT
      INTEGER(KIND=4) IE
      INTEGER(KIND=4) NI
      INTEGER(KIND=4) LL,LLP,LL1,LL1P
...
!
      USE CHECKR_DEF, ONLY : MFO,MTO
      USE CHECKR_DEF, ONLY : NSUBSMAX
      USE CHECKR_DEF, ONLY : N1H,C1L,C2L,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LS,LB
      INTEGER(KIND=4) NK
      INTEGER(KIND=4) NE,NTT
      INTEGER(KIND=4) I1
      REAL(KIND=4) EL,EU
!
!     TEST FOR A VALID MT NUMBER
...
!
      INTEGER(KIND=4) LS,LB
      INTEGER(KIND=4) NS,NL
      INTEGER(KIND=4) NC,NI
      INTEGER(KIND=4) LFS,LFSP
      INTEGER(KIND=4) MAT1,MF1,MT1,LFS1
      INTEGER(KIND=4) MAT1B,MF1B,MT1B,LFS1B
      INTEGER(KIND=4) IE,ISET
      INTEGER(KIND=4) I1,N
!
!     TEST FOR A VALID MT NUMBER
!
...
!
      USE CHECKR_DEF, ONLY : MAT,MT
      USE CHECKR_DEF, ONLY : L2H,L1L
      USE CHECKR_DEF, ONLY : NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
!
      IMPLICIT NONE
...
!
      USE CHECKR_DEF, ONLY : MF,MT
      USE CHECKR_DEF, ONLY : NES5MAX
      USE CHECKR_DEF, ONLY : L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MAT1I,MT1I
!
      INTEGER(KIND=4) LB,LBM
      INTEGER(KIND=4) LT
      INTEGER(KIND=4) NE,NTT
      INTEGER(KIND=4) NE2,NEC
      INTEGER(KIND=4) NPM,NTY
!
...
!
      USE CHECKR_DEF, ONLY : JIN
      USE CHECKR_DEF, ONLY : IFIELD,TEXT
      USE CHECKR_DEF, ONLY : MATP,MFP,MTP
      USE CHECKR_DEF, ONLY : NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      ...
      USE CHECKR_DEF, ONLY : MATP,MFP,MTP
      USE CHECKR_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ,NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      ...
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : MATP,MFP,MTP
      USE CHECKR_DEF, ONLY : LDRV
      USE CHECKR_DEF, ONLY : NBT,JNT
      USE CHECKR_DEF, ONLY : NIRMAX,NPTSMAX
      USE CHECKR_DEF, ONLY : C1,C2,L1,L2,NR,NP
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ,NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
...
!
      ...
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : MATP,MFP,MTP
      USE CHECKR_DEF, ONLY : NBT2,JNT2
      USE CHECKR_DEF, ONLY : NIRMAX,NPTS2MAX
      USE CHECKR_DEF, ONLY : NR2,NP2
      USE CHECKR_DEF, ONLY : C12,C22,L12,L22
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ,NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IBMOD
!
      INTEGER(KIND=4) ICMOD
      INTEGER(KIND=4) NI,NF
      INTEGER(KIND=4) I,N
!
!     READ IN CONT-LIKE RECORD
...
!
      ...
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : MATP,MFP,MTP
      USE CHECKR_DEF, ONLY : NPTSMAX
      USE CHECKR_DEF, ONLY : NBT2,JNT2
      USE CHECKR_DEF, ONLY : C1L,C2L,L1L,L2L,NPL,N2L
      USE CHECKR_DEF, ONLY : BIN,BIN1
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ,NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
...
!
      ...
      USE CHECKR_DEF, ONLY : NFOR
      USE CHECKR_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE CHECKR_DEF, ONLY : ASEQ,ISEQ,NSEQ,NSEQP,NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
...
!
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : IBR
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) L1T,L2T,N1T,N2T
      INTEGER(KIND=4) MATT,MFT,MTT
      INTEGER(KIND=4) NSEQT
!
      ...
      REAL(KIND=4) C1T,C2T
!
      INTEGER(KIND=4), DIMENSION(4) :: L
      REAL(KIND=4), DIMENSION(2) :: C
!
...
!
      INTEGER(KIND=4) MATT,MFT,MTT
      INTEGER(KIND=4) NSEQT
      INTEGER(KIND=4), DIMENSION(3) :: NBTT,INTT
!
      CHARACTER(LEN=11) RFIELD
      INTEGER(KIND=4) KK,K
!
      INTEGER(KIND=4), DIMENSION(6) :: L
!
!     PROCESS INTEGER FIELDS
!
      DO K=1,6
...
!
      USE CHECKR_DEF, ONLY : IFIELD
      USE CHECKR_DEF, ONLY : IBR1
      USE CHECKR_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MATR,MFR,MTR
      INTEGER(KIND=4) NSEQR
      INTEGER(KIND=4) IFMT
!
...
!
      USE CHECKR_DEF, ONLY : NFOR,NSUB
      USE CHECKR_DEF, ONLY : MF
      USE CHECKR_DEF, ONLY : NIRMAX,INTMAX
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NRT,NPT,IBMOD
      INTEGER(KIND=4) NBTT(NIRMAX),JNTT(NIRMAX)
      INTEGER(KIND=4) JNTMIN,JNTMAX
!
      INTEGER(KIND=4) JJNT,KNT
      INTEGER(KIND=4) M
!
...
!
      USE CHECKR_DEF, ONLY : MATP,MFP,MTP
      USE CHECKR_DEF, ONLY : MAT,MF,MT
      USE CHECKR_DEF, ONLY : ASEQ,NSEQ,NSEQP
      USE CHECKR_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
...
!
      USE CHECKR_DEF, ONLY : NFOR,NSUB,LDRV
      USE CHECKR_DEF, ONLY : NSEQP1
      USE CHECKR_DEF, ONLY : EMESS
      USE CHECKR_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MTT0,MFT0
!
      INTEGER(KIND=4) MTT,MFT
      INTEGER(KIND=4) MTCAT
      INTEGER(KIND=4) MT3
      INTEGER(KIND=4) IDX,IPAR
      INTEGER(KIND=4) IPART
      INTEGER(KIND=4) IEVAL
      INTEGER(KIND=4) N
      INTEGER(KIND=4) GET_MTCAT
!
      INTEGER(KIND=4), PARAMETER:: NPARTS = 7
      INTEGER(KIND=4), DIMENSION(NPARTS) :: IPZA,IELAS
      DATA IPZA/0,1,1001,1002,1003,2003,2004/
      DATA IELAS/0,50,600,650,700,750,800/
!
!     INITIALIZE
!
      MTCAT = 0
...
!***********************************************************************
!
      INTEGER FUNCTION GET_MTCAT(MTT,MT3)
!
...
!
!     SET THE FINAL MT CATEGORY
!
  100 GET_MTCAT = MTCAT
!
      RETURN
      END FUNCTION GET_MTCAT
!
!***********************************************************************
!
...
!
      USE CHECKR_DEF, ONLY : NFOR,NSUB
      USE CHECKR_DEF, ONLY : L1H
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MTT,MT3,MFT
      INTEGER(KIND=4) MTCAT
      INTEGER(KIND=4) IEVAL
!
      INTEGER(KIND=4) ISUBTP
...
!
      IF(JSEQ.NE.0) THEN
         IF(ASEQ.NE.' ') THEN
            WRITE(NOUT,'(6X,2A,I6)')  EMESS(1:49),'SEQUENCE NUMBER',JSEQ
         ELSE
            WRITE(NOUT,'(6X,2A,I6)')  EMESS(1:49),'RECORD NUMBER',ISEQ
         END IF
      ELSE
         IF(EMESS.EQ.' ') THEN
            NEMES = 1
