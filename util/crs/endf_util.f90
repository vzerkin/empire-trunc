module endf_util
    ...
    real*8, parameter, public :: pi = 3.1415926535897932384626
    real*8, parameter, public :: twopi = 2.D0*pi

    real*8, parameter :: dlx = 1.0D-06   ! allowed difference bewteen eval & TAB1 points
    ...
    ! x is outside the range of the table (within dlx). Before calling this
    ...
    if(.not.associated(tb)) then
        terpa = 0.D0
        return
    endif

10  if((tb%dat(ip)%x - x) > dlx) then

        if((tb%dat(ip-1)%x - x) <= dlx) then
