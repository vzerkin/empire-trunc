...
C-M  - PLOTC4 list file.
C-M  - ENDF source file.
C-M  - EXFOR source file in computational format.
C-M  - Index number from the PLOTC4 list file to be processed.
C-M    NOTE: Several entries may be chosen, terminated by blank.
C-M
C-M  Files used:
...
C* Define the PLOTC4 list file
   12 WRITE(LTT,91) '$Enter the PLOTC4 list filename      : '
      READ (LKB,91) FLNM
...
C* Define the EXFOR file in computational format
   22 WRITE(LTT,91) '$Enter the C4 file of EXFOR data      : '
      READ (LKB,91) FLNM
...
C...  IF(NEN.LE.1) GO TO 40
      WRITE(LTT,96) ' Total number of ENDF files entered     ',NEN
      DO 32 I=1,NEN
