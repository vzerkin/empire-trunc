...
C-M
C-M     CALL DXSEND(LEF,ZA0,ZAP,MF0,MT0,KEA,EIN,PAR,EPS,ENR,DXS
C-M    1           ,RWO,NEN,MEN,MRW,LTT)
C-M
...
C-M  IER  - Error flag, which is zero on exit if data assembly is
...
C ----INITIALIZE ENDF/B AND EXFOR POINT COUNTS AND ENDF/B MAT NUMBER.
      ...
      IEX=0
      MAT=0
C ----DEFINE LAST REFERENCE AS OTHERS.
      DO 10 J=1,9
...
C...  ZAP=1
      ...
      ELSE IF(MFIN.EQ.6) THEN
        ...
        END IF
      END IF
C...      IF(IDOUB.EQ.2) THEN
C...        KEA=2
C...        IF(MFIN.EQ.6) THEN
C...          PAR=FIELD4(1)
C...          DEG=ACOS(PAR)*180/3.1415926
C...        END IF
C...      ELSE
C...        KEA=1
C...        IF(MFIN.EQ.6) PAR=FIELD4(3)
C...      END IF
...
C ----RANGE.
      ...
      IF(MF.EQ.154) MF=7
      IF(MF.EQ.801) MF=8
      IF(MF.LT.1.OR.MF.GT.8) GO TO 60
...
C
C     FIND NEXT SECTION OF REQUESTED ENDF/B CROSS SECTIONS (MF=3).
C
      PARAMETER (MXPNT=90000)
      INTEGER OUTP
      COMMON/UNITS/INP,OUTP,ITAPE1,ITAPE2
...
C ----FIND BEGINNING OF NEXT SECTION.
   10 READ(ITAPE2,1010) C1,C2,L1,L2,N1,N2,MAT,MF,MT
      IF(MT) 20,20,30
   20 IF(MAT) 110,10,10
C ----DEFINE EVALUATION MOD. FROM FIRST CARD OF HOLLERITH SECTION.
   30 IF(MAT.EQ.MATLST) GO TO 40
      MATLST=MAT
      MODIZA=0
      IF(MF.EQ.1.AND.MT.EQ.451) MODIZA=N2
C ----SEARCH FOR SECTION OF FILE 3.
   40 AWR=C2
...
C ----PROCESS FILE MF3 DATA
   78 READ(ITAPE2,1010) C1,C2,L1,L2,N1,N2,MAT,MF,MT
...
C
      ...
      IF(MF.EQ.3) GO TO 20
      IF(ENEX.LT.ELGET(IGET).OR.ENEX.GT.EHGET(IGET)) GO TO 10
      GO TO 20
...
C ----IF NOT CROSS SECTION PRECEED DATA TYPE BY ENERGY.
  100 CALL EUNIT(ENEX,ZMULT,ZUNIT,IZUNIT,4,IPTZ,1)
      JMFBCD=IMFBCD
      CALL LEFTY(MFBCD,IMFBCD,KMFBCD,JMFBCD)
...
C
   50 IF(XNOW.GT.XREAL(2)) IMON=0
      KTOP=1
      YNOW=((XNOW-XREAL(2))*YLAST+(XREAL(2)-XLAST)*YNOW)/(XNOW-XLAST)
      XNOW=XREAL(2)
      IF(YNOW.LE.YREAL(2)) GO TO 60
      IMON=0
      XNOW=((YNOW-YREAL(2))*XLAST+(YREAL(2)-YLAST)*XNOW)/(YNOW-YLAST)
      YNOW=YREAL(2)
      GO TO 110
   60 IF(YNOW.GE.YREAL(1)) GO TO 110
      IMON=0
      XNOW=((YNOW-YREAL(1))*XLAST+(YREAL(1)-YLAST)*XNOW)/(YNOW-YLAST)
      YNOW=YREAL(1)
      GO TO 110
C
...
C
      ...
      IF(MFSAVX.LT.3.OR.MFSAVX.GT.8) GO TO 10
...
C
   30 IF(NZATAB.LE.0) GO TO 50
     ...
     3 IZATRY.NE.MZATAB(9,I)) GO TO 40
      ...
      IF(FIELDI(1).EQ.EXTRA(I)) GO TO 60
   40 CONTINUE
...
C
   70 IF(NZATAB.LE.0) GO TO 630
      ...
      KZATAB=KZATAB+1
      DO 80 K=1,9
   80 MZATAB(K,KZATAB)=MZATAB(K,I)
      EXTRA(KZATAB)=EXTRA(I)
   90 CONTINUE
      NZATAB=KZATAB
...
C
      ...
      IF(MFX.EQ.154) MFSAVX=7
      IF(MFX.EQ.801) MFSAVX=8
      IF(MFSAVX.LT.1.OR.MFSAVX.GT.8) GO TO 190
C
...
C ----LOCATION FORWARD IN TABLE.
  590 IF(NZATAB.LE.1.OR.KZATAB.EQ.NZATAB) GO TO 620
      ...
      DO 610 I=II,NZATAB
      DO 600 K=1,9
  600 MZATAB(K,I-1)=MZATAB(K,I)
  610 EXTRA(I-1)=EXTRA(I)
      KZATAB=KZATAB-1
  620 NZATAB=NZATAB-1
