...
C* Find particle emission reactions
      ...
      IZAP=ZAP+0.1
      IF(IZAP.EQ.1) THEN
C* Neutron emission: prepare reaction list for 
   20   MF=3
        MT=0
...
C* Proton emission: prepare reaction list for 
   24   MF=3
        MT=0
        CALL FINDMT(LEF,ZA0,ZA,AW,L1,L2,N1,N2,MAT,MF,MT,IER)
        IF(IER.NE.0) GO TO 50
...
C* Select contributiong reactions
     ...
     &     (MT.GE.115 .AND. MT.LE.116)) THEN
          LOOP=LOOP+1
          LST(LOOP)=MT
        END IF
        GO TO 24
      ELSE IF(IZAP.EQ.2004) THEN
C* Alpha emission: prepare reaction list for 
   34   MF=3
        MT=0
        CALL FINDMT(LEF,ZA0,ZA,AW,L1,L2,N1,N2,MAT,MF,MT,IER)
        IF(IER.NE.0) GO TO 50
...
C* Find the energy/angle distribution data
   40 MF =0
      MT =MT0
      CALL FINDMT(LEF,ZA0,ZA,AWR,L1,L2,N1,N2,MAT,MF,MT,IER)
      IF(MF.GT.6 .AND. MF.NE.26) THEN
        PRINT *,'WARNING - No differential data for MT',MT0
        GO TO 90
      END IF
C* Case - No MF4/5/6 (possible only for two-body scattering)
      IF(IER.NE.0 .AND.
     1  (MT0.NE.2 .AND. (MT0.LT.50 .OR. MT0.GT.90) ) ) GO TO 90
C* If file MF 6 data jump to file MF 6 processing
      IF(MF.EQ.6) GO TO 60
C* Process MF 4 data
...
C* If not the right particle try next one
          CALL RDTAB2(LEF,C1,C2,L1,L2,NR,NE,NBT,INR,IER)
          DO 52 IE=1,NE
          CALL RDLIST(LEF,C1,C2,L1,L2,N1,N2,RWO(LXE),IER)
   52     CONTINUE
...
C* For each incident energy read the outg.particle energy distribution
     ...
     1      PRINT *,'WARNING - Multiple Eout int.rng. MF 5, MT',MT
          ...
          END IF
        ELSE
          CALL RDLIST(LEF,C1,EI2,L1,L2,N1,NF,RWO(LXE),IER)
