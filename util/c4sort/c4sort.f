...
C-M
C-M  The following reactions are considered "Standards":
C-M    Li-6(n,t)
C-M    B-10(n,a)
C-M    Au-197(n,g)
C-M    U-235(n,f)
C-M  If ratio data are defined with a standard reaction in the
C-M  numerator, the reaction ratio is redefined with the standard
C-M  in the denominator.
C-M
C-M  Note:
...
C* Files and logical file unit numbers
     ...
     8     FLS2/'C4SORT.TM2'/
     9     FLS3/'C4SORT.TM3'/
      DATA LIN,LC4,LOU,LES,LKB,LTT,LSC,LS2,LS3
     &    /  1,  2,  3, -4,  5,  6,  7,  8,  9 /
...
C* C4SORT Input exists -Process the input file
   12 OPEN(UNIT=LIN,FILE=FLIN,STATUS='OLD')
      IER=0
      NALI=MXAL
...
C* Input instructions processed - start processing the C4 file
   15 OPEN (UNIT=LC4,FILE=FLC4,STATUS='OLD')
...
C* Convert discrete gamma production cross sections to levels
        OPEN (UNIT=LS3,FILE=FLS3,STATUS='UNKNOWN')
C*
        IZA1=-1
        MT1 =-1
        NER = 0
...
C*      -- Skip comments
        ...
        READ (REC(41:58),'(2F9.0)',ERR=16) XSR,DXS
        READ (REC(59:76),'(2F9.0)',ERR=16) RMT,RZA
C*      -- Skip undefined/unknown reactions
        IF(MT.LT.1 .OR.MT.GT.9999) GO TO 16
C*      -- Check Ratio data
        IF(MF.EQ.203) THEN
C*        -- Check if the numerator is a standard --> force denominator
          IF( (IZAT.EQ. 3006 .AND. MT.EQ.103) .OR.
     &        (IZAT.EQ. 5010 .AND. MT.EQ.107) .OR.
     &        (IZAT.EQ.79197 .AND. MT.EQ.102) .or.
     &        (IZAT.EQ.92235 .AND. MT.EQ. 18) ) THEN
            DMY =RMT
            MT  =NINT(RMT)
            RMT =DMY
            DMY =RZA
...
C*        -- Check specifically for Cf-252 sf spectrum in the numerator
          READ(REC(1:19),902,ERR=16) KIN,KZA,KF,KMT
...
C...
          ...
          END DO
        END IF
C* Check for discrete-level gamma-production data
        IF(MF.EQ.3 .AND. MT.GE.9000 .AND. RZA.LT.1 .AND.
     &     REC(77:85).NE.'         ' .AND. REC(95:97).EQ.' E2') THEN
          IF(NDG.EQ.0) THEN
C*          -- On first encounter save parameters
            NDG=1
            HRSD=REC( 59: 67)
            HEJC=REC( 68: 76)
            HEXF=REC(123:132)
            REWIND LS3
          END IF
          IF(HRSD.EQ.REC( 59: 67) .AND.
     &       HEJC.EQ.REC( 68: 76) .AND.
     &       HEXF.EQ.REC(123:132)) THEN
            WRITE(LS3,901) REC
          ELSE
            CALL GLTOEL(LS3,LS2,DPATH,LPATH)
            NDG=0
          END IF
        ELSE
C*        -- If reaction changed, add discrete level data to LS2
          IF(NDG.EQ.1) THEN
            CALL GLTOEL(LS3,LS2,DPATH,LPATH)
            NDG=0
          END IF
...
C*    -- Special treatment for correct positioning of mu-bar
      ...
      IF(ENTI(12:19).EQ.' 1549000') ENTI(12:19)='   3 254'
...
c...
      ...
      WRITE(LTT,921) ' Total No. of records written         : ',JLN
      CLOSE(UNIT=LC4)
C*
C* Sort the entries
C*
      WRITE(LTT,921) ' '
      WRITE(LTT,921) ' Begin sorting the indices              '
...
C-V  12/06 Implement roundoff correction
      CHARACTER*9  CH9
      CHARACTER*15 C15
C*    -- Increment by fraction 1e-7 to ensure correct roundoff
      WRITE(C15,'(1P,E15.8)') FLT*(1+1.E-7)
      L=15
      ...
      RETURN
      END
      SUBROUTINE GLTOEL(LIN,LOU,DPATH,LPATH)
C-Title  : Subroutine GLTOEL
C-Purpose: Convert discrete-level gamma production to level cross sect.
C-Description:
C-D  The gamma-production data for the given residual are read into
C-D  the work arrays. The level energies are read from the levels-file.
C-D  The cross section for the excitation of a particular discrete level
C-D  are reconstructed by considering the direct contribution (transition
C-D  directly into the ground state) and the contributions from
C-D  transitions into lower discrete levels. If the incident energy is
C-D  above the threshold for a higher level, the contribution from the
C-D  transitions from this level to the current is subtracted.
C-D
C-D  NOTE:
C-D  The discrete-level cross section is registered only if gammas for
C-D  all intermediate levels are identified. No provision is made for
C-D  cases where the transition probability between two levels is truly
C-D  zero. The procedure is robust for the first discrete level, but
C-D  may result in missed data for the higher levels. At present, the
C-D  coding limits the number of levels considered to less than NLV.
C-D
C-D  Parameters
C-D    LIN   Logical file unit containing the source data
C-D    LOU   Logical file unit to which the level cross sections are
C-D          written
C-D    DPATH Character string defining the path to the "levels" files
C-D    LPATH Length of the DPATH string
C-D
C-External: read_levels_RIPL
C-
      CHARACTER*80  DPATH
      CHARACTER*132 REC
...
C* Criterion for discriminating level energies (eV)
      DATA LVLEPS/ 5000 /
C-F Identify the residual nucleus
      REWIND LIN
      READ (LIN,130,END=100) REC
      READ (REC,'(5X,2I3)') IZ0,IA0
      IZA0=IZ0*1000+IA0
      IF(REC(59:67).NE.'         ') THEN
        READ(REC(59:67),'(F9.0)') ZA
        IZA=ZA
        IZ =IZA/1000
        IA =IZA-1000*IZ
      ELSE
...
C...  PRINT *,NLV
C...
C*    -- Considel only the levels below level NLV
      NLV   =MIN(NLV,4)
C*    -- Approximate atomic mass ratio by IA (only for E_threshold)
      AWR=IA
C*    -- Skip ground-state level, covert to eV, estimate E_threshold
      DO L=1,NLV
        ELV(L)=ELV(L+1)*1.D6
        ETH(L)=ELV(L)*(AWR+1)/AWR
      END DO
C*    -- Maximum incident particle energy to be considered
      EMX=ELV(NLV)
C-F Start reading the discrete gamma-production cross section data
      REWIND LIN
      NPR=0
   20 NP1=NPR+1
      READ (LIN,130,END=30) REC
      READ (REC,140) IZAT,EN(NP1),DE(NP1),CS(NP1),DC(NP1)
     &              ,ZAR,EJC,EL1,EL2
      IZAR=ZAR
      IF(IZAR.EQ.0) IZAR=IZA0
C...
C...  PRINT *,IZAT,IZA0,IZAR,IZA,ZAR,EJC,EL1,EL2
C...
      IF(IZAT.NE.IZA0) THEN
        PRINT *,'GLTOEL WARNING - Entry skipped - target changed ',IZAT
        GO TO 20
      END IF
      IF(IZAR.NE.IZA) THEN
        PRINT *,'GLTOEL WARNING - Entry skipped - resid. changed ',IZAR
        GO TO 20
      END IF
      IF(EJC.GE.1) THEN
        PRINT *,'GLTOEL WARNING - Entry skipped - eject. not g',EJC
        GO TO 20
      END IF
      IF(EL2.GT.0 .AND. NINT(EL1).NE.NINT(EL2)) GO TO 20
      EL(NP1)=EL1
      NPR=NP1
      IF(NP1.LT.MXPR) GO TO 20
      STOP 'GLTOEL ERROR - MXPR limit exceeded'
C*
C*    All gamma-production cross sections read
   30 CONTINUE
c...
C...  print *,'npr',npr
c...
      IL =0
C-F Loop over levels
   40 IL = IL+1
      DO L=1,NLV
        DLV(L)=ABS(ELV(L)-ELV(IL))
      END DO
C*    Find the first energy point
      EI =0
C*    Loop to next energy
   50 IX =0
      DO I=1,NPR
        IF(EN(I).GT.EI) THEN
          EI=EN(I)
        ...
        END IF
      END DO
C*    --Last energy processed, proceed to next level
      IF(IL.GE.NLV) GO TO 100
      GO TO 40
C*
C* Sum contributions to the current level IL
   60 CONTINUE
C...
C...  print *,'Energy',i,ei
C...
...
C*      -- Skip data above the threshold for the highest level
        IF(EN(I).GE.EMX) CYCLE
C*      -- Matching incident energy found
C...
C...    PRINT *,'    ',IL,I,ELV(IL),EL(I)
C...
        IF( ABS(NINT(ELV(IL)-EL(I))).LT.LVLEPS) THEN
C*        -- Add contribution to ground-state
          XS=XS+CS(I)
          DX=DX+DC(I)
          IX=IX+1
          NX=IL
C*        -- Check for threshold of higher levels
          DO WHILE (EI.GT.ETH(NX+1) .AND. NX+1.LT.NLV)
            NX=NX+1
          END DO
c...
C...      PRINT *,'Direct IL,I,EL,ELV,NX',IL,I,ELV(IL),EL(I),XS,NX
C...
        END IF
        DO L=1,NLV
C...
C...      PRINT *,'    ',L,I,DLV(L),EL(I)
C...
          IF(NINT(ABS(DLV(L)-EL(I))).LT.LVLEPS) THEN
C*          -- Gamma matching level found
            IF(L.GT.IL) THEN
C*            -- Subtract feeding from higher levels
              XS=XS-CS(I)
              DX=DX+DC(I)
            ELSE
C*            -- Add feeding to intermediate levels
              XS=XS+CS(I)
              DX=DX+DC(I)
            END IF
            IX=IX+1
C...
C...        PRINT *,'  Indirect L,I,DLV,EL',L,I,DLV(L),EL(I),XS
C...
          END IF
        END DO
      END DO
C*
      IF(IX.EQ.NX) THEN
c...
C...    PRINT *,'Final',IL,EI,ELV(IL),XS,DX,IX,NX
C...    PRINT *,' '
c...
        IF     (IZA.EQ.IZA0) THEN
          MT= 51
        ELSE IF(IZA.EQ.IZA0+1-1001) THEN
          MT=601
        ELSE IF(IZA.EQ.IZA0+1-2004) THEN
          MT=801
        ELSE
          GO TO 50
        END IF
        WRITE(REC(16:19),'(I4)') 51
        CALL CH9PCK(EI,REC(23:31))
        IF(DI.NE.0) THEN
          CALL CH9PCK(DI,REC(32:40))
        ELSE
          REC(32:40)='         '
        END IF
        CALL CH9PCK(XS,REC(41:49))
        CALL CH9PCK(DX,REC(50:58))
        REC(59:76)='                  '
        ELIL=ELV(IL)
        CALL CH9PCK(ELIL,REC(77:85))
        REC(86:97)='         LVL'
C*      -- Place '+' at the end of the author
        DO I=98,132
          IF(REC(I:I).EQ.'(') THEN
            I2=MAX(1,I-2)
C*          -- Found open bracket before the year
   62       IF(REC(I2:I2).EQ.' ') THEN
              I2=I2-1
              GO TO 62
            END IF
            REC(I2+1:I2+1)='+'
