...
C
      ...
      logical lcovar(Nreact,Nreact),lpositive
...
C    &      nuc_print+6,SYMbe(0), int(Z(0)), SYMb(0), int(A(0))
          ...
          if(A(0).gt.220) Nnucd=11  ! neglecting all charged=particle emissions for heavy nuclei
...
C    &         (preaction(nnuc),nnuc=1,min(nuc_print,max_prn))
          read(10,'(13X,(95A12))')(REAction(ir),ir=1,NNUcd)
...
C           WRITE(41,'(G10.5,1x,1P,20E12.5)') EINl, ..
...
C
C         Skipping those MC samples not covering the full energy range
C                  or giving a reading error
C
          read(10,'(10x,90E12.5)',ERR=200,END=200) (rndvec(ir,i),ir=1,Nnucd)
