...
! *                    1. PSYCHE CONVERTED TO FORTRAN-77
! *                    2. NEW ENDF-6 FORMATS FILES 1-27 ADDED
! *                    3. ENERGY BALANCE TEST FOR FILE 6 ADDED
...
! *                    2. RESTORE REICH-MOORE TO ENDF-5 FORMAT
! *                    3. PROCESS MORE THAN ONE INPUT FILE
! *                    4. FORMAT CHANGES OF MAY 1987 EXCEPT
! *                        GENERALIZED R-MATRIX AND MULTIPLE
! *                        RESOLVED RESONANCE REGIONS
! *      VERSION 6.4   MAY 1988  C.L. DUNFORD
! *                    1. CORRECTIONS TO VERSION 6.3
! *                    2. MORE THAN ONE RESOLVED RESONANCE REGION
! *                    3. IMPLEMENT HYBRID R-FUNCTION
! *                    4. CHANGES TO RESONANCE REGION MAY 1988 CSEWG
! *      VERSION 6.5   APRIL 1989
! *                    1. CORRECTIONS TO VERSION 6.4
! *                    2. RECOGNIZE FILES 34, 35 AND 40
...
! *                    2. CORRECTION TO REICH-MOORE MODULE FOR
! *                        POTENTIAL SCATTERING IN REDUNDANT J CHANNELS
...
! *                    1. CORRECTIONS TO VERSION 6.8
! *                    2. HANDLE MORE THAN 500 ANGULAR DISTRIBUTIONS
! *                    3. CORRECT R-MATRIX POTENTIAL SCATTERING
! *      VERSION 6.10  NOVEMBER 1995  C.L.DUNFORD
! *                    1. ALLOWED CODE TO HANDLE UP TO 150 SECTIONS IN A
! *                        FILE
! *      VERSION 6.11  APRIL 1998   C.L. DUNFORD
! *                    1. ALLOW 1500 ANGULAR DISTRIBUTIONS
! *                    2. ALLOW 50000 POINTS IN TAB AND LIST RECORDS
! *                    3. ALLOW UP TO 64 LEGENDRE COEFFICIENTS
! *                    4. INCLUDE ONLY RESONANCES WITHIN THE BOUNDS OF
! *                        AN ENERGY REGION IN CALCULATING AVERAGES
! *                    5. CORRECT ENERGY BALANCE TEST
! *                    6. NEW LTT=3 REPRESENTATION IN FILE 4
! *      VERSION 6.12  FEBRUARY 2001     C.L.DUNFORD
! *                    1. IMPLEMENT RECOMMENDED PHYSICAL CONSTANTS
! *                    2. ACCOUNT FOR 2 POSSIBLE CHANNEL SPINS IN
! *                       REICH-MOORE WHEN I AND L NOT EQUAL TO ZERO
! *                    3. IMPLEMENT "$" OUTPUT CONTROL ON UNIX
! *                    4. CREATE THREE INTERACTIVE VERSIONS AND ONE
! *                       BATCH VERSION
! *                    5. DO NOT CHECK Q-VALUE FOR A NATURAL ELEMENT
! *                       AS DEFINITION IS VAGUE
...
!
      ...
      INTEGER(KIND=4), PARAMETER :: ISCR=22,JSCR=23,KSCR=24,LSCR=25
!
...
!
      ...
      INTEGER(KIND=4), DIMENSION(INTABMAX) :: NTERP,INTERP
!
...
!     DATA TABLE
!
      REAL(KIND=4), DIMENSION(50000) :: X,Y
!
...
!
      INTEGER(KIND=4) LNU,NRC,NPT
      INTEGER(KIND=4), DIMENSION(INTABMAX) :: ITERP,JTERP
      REAL(KIND=4), DIMENSION(4) :: POLI
      REAL(KIND=4), DIMENSION(5000) :: ETERP,ANUTRP
!
!     ISOTOPES IN RESONANCE REGION
!
      INTEGER(KIND=4), PARAMETER :: NISMAX=20
      INTEGER(KIND=4) NIS
      INTEGER(KIND=4), DIMENSION(NISMAX) :: NERS
      REAL(KIND=4), DIMENSION(NISMAX) :: ZAIS,ABNS,SPINS
      INTEGER(KIND=4) NISN,NISMAT
      INTEGER(KIND=4), DIMENSION(NISMAX) :: IAIS
      REAL(KIND=4), DIMENSION(NISMAX) :: ZAIMAT,ABNIS,SPINIS
!
!     ENERGY REGION BOUNDARIES
!
      REAL(KIND=4) E1,E2
      INTEGER(KIND=4), PARAMETER :: NREGMAX=14
      INTEGER(KIND=4), DIMENSION(NREGMAX) :: IRES
      REAL(KIND=4), DIMENSION(NREGMAX) :: EMIDLE
      INTEGER(KIND=4) IUNR
      INTEGER(KIND=4), PARAMETER :: NBOUNDMAX=NREGMAX+3
      INTEGER(KIND=4) NBOUND
      REAL(KIND=4), DIMENSION(NBOUNDMAX) :: RBOUND
!
!     ENERGY DEPENDENT SCATTERING RADIUS EXPANSION ARRAYS
!
      INTEGER(KIND=4), DIMENSION(NREGMAX) :: NRED,NPED
      INTEGER(KIND=4), DIMENSION(INTABMAX,NREGMAX) ::  NBTED,JNTED
      REAL(KIND=4), DIMENSION(100,NREGMAX) :: EP,APED
!
!     RESONANCE PARAMETER STORAGE
!
      INTEGER(KIND=4) IPT
      REAL(KIND=4), DIMENSION(40000) :: RESPAR
      REAL(KIND=4), DIMENSION(500) :: CONSA,SFA
!
!     ARRAY STORING Q-VALUES FROM FILE 3 FOR LATER TESTS
!
      INTEGER(KIND=4), PARAMETER ::  SZMT3=150
      INTEGER(KIND=4) NMT3
      INTEGER(KIND=4), DIMENSION(SZMT3) :: MT3
      REAL(KIND=4), DIMENSION(SZMT3) :: QIVAL,QVAL
      REAL(KIND=4), PARAMETER :: QUNK= 7.777E+07
!
!     LEGENDRE COEFFICIENTS
!
      INTEGER(KIND=4) NCOEF
      INTEGER(KIND=4), PARAMETER :: NCOEFMAX=64
      REAL(KIND=4) ENERGY
      REAL(KIND=4), DIMENSION(NCOEFMAX) :: F
      INTEGER(KIND=4) NMOM
      REAL(KIND=4), DIMENSION(NCOEFMAX) :: MOMENT
      REAL(KIND=4), DIMENSION(33) :: MEAN
      REAL(KIND=4), DIMENSION(32) :: VAR
      INTEGER(KIND=4) NPOWS
      INTEGER(KIND=4), PARAMETER :: NCOEFMAX1=NCOEFMAX+1
      REAL(KIND=4), DIMENSION(NCOEFMAX1,NCOEFMAX1) :: W
!
!     SECTIONS INVOLVED IN ENERGY BALANCE CALCULATIONS
!
      INTEGER(KIND=4) MT1213
      INTEGER(KIND=4), DIMENSION(10) :: MTSCR
      INTEGER(KIND=4) NMTBAR
      INTEGER(KIND=4), DIMENSION(50) :: MTBAR
      INTEGER(KIND=4), PARAMETER :: NSECMAX=350
      INTEGER(KIND=4) MTTOT  ! SECTIONS WITH PHOTONS
      INTEGER(KIND=4), DIMENSION(NSECMAX) :: MTDCT
      INTEGER(KIND=4), PARAMETER :: NSMF5=NSECMAX
      INTEGER(KIND=4) MF5   ! SECTIONS IN MF5 WITH PHOTONS
      INTEGER(KIND=4), DIMENSION(NSMF5) :: MT5
      INTEGER(KIND=4), PARAMETER :: NBSMAX=50
      INTEGER(KIND=4) NBS,NBSN  ! SECTIONS IN MF13
      INTEGER(KIND=4), DIMENSION(NBSMAX) :: MTS
      REAL(KIND=4), DIMENSION(1500) :: EGRID,AEBAR,EINT,PKINT
      REAL(KIND=4), DIMENSION(1500) :: EBAV,YLD
!
      REAL(KIND=4), PARAMETER :: BIGNO=1.0E+20
      REAL(KIND=4), PARAMETER :: PI=3.14159265
      REAL(KIND=4), PARAMETER :: CROC=2.196807E-03
      REAL(KIND=4), PARAMETER :: ANEUTR=1.008664904
      REAL(KIND=4), PARAMETER :: THERMA=0.0253
      REAL(KIND=4), PARAMETER :: EPI6=1.0E-06
      REAL(KIND=4), PARAMETER :: EPI4=1.0E-04
      REAL(KIND=4), PARAMETER :: EPI3=1.0E-03
      REAL(KIND=4), PARAMETER :: EPI2=1.0E-02
      REAL(KIND=4), PARAMETER :: EPI1=1.0E-01
!
      INTEGER(KIND=4), PARAMETER :: NSMASS=23
      INTEGER(KIND=4), DIMENSION(NSMASS) :: IZAB
      DATA IZAB/1,1001,1002,1003,2003,2004,                             &       
     &  3006,  3007,  4009,  5010,  7014,                               &       
     & 20048, 22046, 22049, 24054, 56136,                               &       
     & 56137, 57139, 60142, 66156, 72174,                               &       
     & 72180, 82208/
      REAL(KIND=4), DIMENSION(NSMASS) :: AMASSB
      DATA AMASSB/  8071.43,                                            &       
     &   7289.03,  13135.84,  14949.94,  14931.32,   2424.93,           &       
     &  14087.28,  14908.22,  11348.02,  12051.77,   2863.44,           &       
     & -44216.52, -44122.72, -45329.01, -56931.33, -87870.41,           &       
     & -87732.55, -87230.47, -85948.72, -70526.78, -55829.55,           &       
     & -49778.51, -21758.95/
!
!     ERROR MESSAGE TEXT
!
...
!
      USE PSYCHE_DEF, ONLY : INPAR,ILENP
!
      IMPLICIT NONE
!
      INPAR = ' '
...
!+++MDC+++
!...DVF, LWI
!/      USE PSYCHE_DEF, ONLY : TEXT
!/      USE PSYCHE_WIN_DEF, ONLY : IRERUN
!---MDC---
!
      IMPLICIT NONE
!
...
!+++MDC+++
!...DVF, LWI
!/      USE PSYCHE_DEF, ONLY : MATO,MFO,MTO
!/      USE PSYCHE_WIN_DEF, ONLY : IRERUN
!---MDC---
!
      IMPLICIT NONE
...
!
      USE PSYCHE_DEF, ONLY : IOUT
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) ISUCCESS
!
      CALL PSYCHE_RUN(ISUCCESS)
!
!     TERMINATE JOB
!
...
!***********************************************************************
!
      SUBROUTINE PSYCHE_RUN(ISUCCESS)
!
      USE PSYCHE_DEF, ONLY : IOUT,NOUT,JIN
      USE PSYCHE_DEF, ONLY : ISCR,JSCR,KSCR,LSCR
      USE PSYCHE_DEF, ONLY : VERSION
      USE PSYCHE_DEF, ONLY : I_DATA
      USE PSYCHE_DEF, ONLY : IFIN,IONEPASS
      USE PSYCHE_DEF, ONLY : NSUB
      USE PSYCHE_DEF, ONLY : MATO,MFO,MTO
      USE PSYCHE_DEF, ONLY : MAT,MF,MT
      USE PSYCHE_DEF, ONLY : LRP
      USE PSYCHE_DEF, ONLY : NMT3
      USE PSYCHE_DEF, ONLY : MT1213,NMTBAR,MTTOT,MF5
      USE PSYCHE_DEF, ONLY : NBS,NBSN,MTS
      USE PSYCHE_DEF, ONLY : NSEQ,NSEQP,NSEQP1
      USE PSYCHE_DEF, ONLY : EMESS
      USE PSYCHE_DEF, ONLY : IERX
!
      USE PSYCHE_MDC, ONLY : IMDC
      USE PSYCHE_MDC, ONLY : GET_FROM_CLINE,OUT_STATUS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) ISUCCESS
...
!
      USE PSYCHE_DEF, ONLY : NIN,IOUT,JOUT,NOUT,JIN
      USE PSYCHE_DEF, ONLY : ISCR,JSCR,KSCR,LSCR
      USE PSYCHE_DEF, ONLY : VERSION
      USE PSYCHE_DEF, ONLY : I_DATA
      USE PSYCHE_DEF, ONLY : INPAR,ILENP
      ...
      USE PSYCHE_DEF, ONLY : IERX
!
      USE PSYCHE_MDC, ONLY : IMDC,TFMT,OSTATUS
!
      IMPLICIT NONE
!
...
!
     ...
     &            '1',' PROGRAM PSYCHE VERSION ',VERSION,               &       
     &            'Run on ',DATE()
      END IF
      WRITE(NOUT,'(2A)')                                                &       
     &   ' Input File Specification------------------------',           &       
...
!
      USE PSYCHE_DEF, ONLY : NIN,IOUT
      USE PSYCHE_DEF, ONLY : I_DATA
!
      USE PSYCHE_MDC, ONLY : TFMT
!
      IMPLICIT NONE
!
...
!
      USE PSYCHE_DEF, ONLY : JIN,IOUT,NOUT
      USE PSYCHE_DEF, ONLY : I_DATA
      USE PSYCHE_DEF, ONLY : MAT,MF,MT
      USE PSYCHE_DEF, ONLY : LABEL,TLABEL
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE PSYCHE_DEF, ONLY : NSEQ,NSEQP
      USE PSYCHE_DEF, ONLY : EMESS
      USE PSYCHE_DEF, ONLY : IERX
!
      USE PSYCHE_MDC, ONLY : IMDC
...
!     CHECK FILE 1 DATA
!
      USE PSYCHE_DEF, ONLY : IOUT,NOUT
      USE PSYCHE_DEF, ONLY : NLIB,NVER,NSUB,NMOD,NFOR
      USE PSYCHE_DEF, ONLY : ZA,AWR,AWI,ELIS,LIS,LISO
      USE PSYCHE_DEF, ONLY : LRP,LFI
      USE PSYCHE_DEF, ONLY : MAT,MF,MT
      USE PSYCHE_DEF, ONLY : ENMAX
      USE PSYCHE_DEF, ONLY : NSECMAX
      USE PSYCHE_DEF, ONLY : LNU,NRC,NPT
      USE PSYCHE_DEF, ONLY : ITERP,JTERP
      USE PSYCHE_DEF, ONLY : POLI
      USE PSYCHE_DEF, ONLY : ETERP,ANUTRP
      USE PSYCHE_DEF, ONLY : MTTOT,MTDCT
      USE PSYCHE_DEF, ONLY : NBSMAX,NBS,MTS
      USE PSYCHE_DEF, ONLY : NR,NP
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : X,Y
      USE PSYCHE_DEF, ONLY : TEXT
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H,NPL
      USE PSYCHE_DEF, ONLY : EMESS
      USE PSYCHE_DEF, ONLY : IERX
!
      USE PSYCHE_MDC, ONLY : IMDC
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NCD,NXC
      INTEGER(KIND=4) NID
      INTEGER(KIND=4) NC,I,J,N,NN
!
      IERX = 0
!
...
!********READ IN COMMENT RECORDS
         ...
         DO NC=1,NCD
            ...
            IF(NC.LE.NID)   THEN
               ...
               IF(IOUT.NE.NOUT)   WRITE(NOUT,'(6X,A)')   TEXT
            END IF
         END DO
!********PROCESS DIRECTORY
         NXC = N2H
...
!     CHECK FILE 2 DATA
!
      USE PSYCHE_DEF, ONLY : ISCR
      USE PSYCHE_DEF, ONLY : ZA,AWR
      USE PSYCHE_DEF, ONLY : ENMIN,ENMAX
      USE PSYCHE_DEF, ONLY : E1,E2
      USE PSYCHE_DEF, ONLY : NIS,NERS,ZAIS,ABNS,SPINS
      USE PSYCHE_DEF, ONLY : NISN
      USE PSYCHE_DEF, ONLY : IRES,EMIDLE,IUNR
      USE PSYCHE_DEF, ONLY : NBOUND,RBOUND
      USE PSYCHE_DEF, ONLY : NRED,NPED
      USE PSYCHE_DEF, ONLY : IPT
      USE PSYCHE_DEF, ONLY : EPI4
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LFW,LRU,LRF
      INTEGER(KIND=4) NER,NRO,NAPS
      INTEGER(KIND=4) IZ,IA
      INTEGER(KIND=4) NI,NRE,ILI
      REAL(KIND=4) EL,EU
      REAL(KIND=4) AWREL,DIFF
      REAL(KIND=4), EXTERNAL :: AWRN
!
...
!
      USE PSYCHE_DEF, ONLY : JIN
      USE PSYCHE_DEF, ONLY : SPINS
      USE PSYCHE_DEF, ONLY : NRED,NPED,NBTED,JNTED,EP,APED
      USE PSYCHE_DEF, ONLY : IPT,RESPAR
      USE PSYCHE_DEF, ONLY : CROC,ANEUTR
      USE PSYCHE_DEF, ONLY : NR,NP
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : X,Y
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IA,NI,NAPS,NRO,LRF,NRE
      REAL(KIND=4) EL,EU
!
      INTEGER(KIND=4) LRX,LI,LBK
      INTEGER(KIND=4) NX,NCRE
      INTEGER(KIND=4) L,NLS,NJS,NSS,NRS,NLJ,NLSJ,LPS
      INTEGER(KIND=4) IRP,IPTU,IBEG,IRBEG,IBO
      INTEGER(KIND=4) NC,NOFF
      INTEGER(KIND=4) N,NN,III,NL,NJ,NS,LIL,NCR
      REAL(KIND=4) APL,AWRI,ROC,RHOC,SER,PER,ROCED,ER,AER,RHO
      REAL(KIND=4) AP,AS,AC,ACP
      REAL(KIND=4) SPI,R,AJ,QX
!
...
!
      ...
      USE PSYCHE_DEF, ONLY : SPINS
      USE PSYCHE_DEF, ONLY : IPT,RESPAR
      USE PSYCHE_DEF, ONLY : ANEUTR
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IA,NI,LFW,LRF
!
      INTEGER(KIND=4) LSSF
      INTEGER(KIND=4) L,NRS,NLS,NJS,MUF
      INTEGER(KIND=4) IRP,NINT,NE,NC
      INTEGER(KIND=4) NL,NJ,J
      REAL(KIND=4) A,AWRI,AJ,SPI,R
!
!
...
!
      USE PSYCHE_DEF, ONLY : NOUT
      USE PSYCHE_DEF, ONLY : CROC
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IA,L,NN,NRS,LRF
      REAL(KIND=4) AWRI,R,AJ,SPI,EL,EU
      REAL(KIND=4), DIMENSION(NN) :: A
!
      INTEGER(KIND=4) NREP,NRSA,INOW
      INTEGER(KIND=4) N
      REAL(KIND=4) DE,FNRS,DNRS
      REAL(KIND=4) GDEN,ROC,GNOEST,GGH,GG2
      REAL(KIND=4) GTBAR,GGBAR,GFBAR,GGLOG,GNOBAR,GNOLOG,GFLOG,SF,D
      REAL(KIND=4) E,FL,RHO,SE,PE,GNO,AJC,GG,GF
      REAL(KIND=4) ENUG,DELNUG,ENUF,DELNUF,ENUN,DELNUN,YIE
!
      REAL(KIND=4), PARAMETER :: CGNO=1.66336E+06
!
      NREP = NN/NRS
!
...
!
   ...
   10 FORMAT(//6X,'ISOTOPE MASS =',I4,'. L = ',I2//                     &       
     &  10X,'TOTAL NUMBER OF RESONANCES IS ',16X,I4/                    &       
     &  10X,'NUMBER OF RESONANCES IN THE ENERGY REGION ARE ',I4/)
      DO N=1,NRS
         INOW = NREP*(N-1) + 1
         IF(A(INOW).GE.EL.AND.A(INOW).LE.EU)  THEN
...
!
   ...
   20          FORMAT(10X,'AT RESONANCE ENERGY ',1PE12.5,               &       
   ...
   30          FORMAT(10X,'AT RESONANCE ENERGY ',1PE12.5,               &       
...
!
      IF(LRF.LT.3) THEN
         WRITE(NOUT,40) GTBAR
   40    FORMAT(10X,'AVERAGE TOTAL WIDTH IS ',1PE12.5)
      END IF
      WRITE(NOUT,45) GNOBAR
   45 FORMAT(10X,'AVERAGE REDUCED NEUTRON WIDTH IS ',1PE12.5)
      WRITE(NOUT,50)  ENUN,DELNUN
   50 FORMAT(15X,'THE NUMBER OF DEGREES OF FREEDOM IS',F7.3,' +OR-',    &       
     &    F6.3)
      WRITE(NOUT,55) GGBAR
   55 FORMAT(10X,'AVERAGE GAMMA WIDTH IS ',1PE12.5)
      IF(ENUG.LE.10.)   THEN
         WRITE(NOUT,50) ENUG,DELNUG
      ELSE
         WRITE(NOUT,60)
   60    FORMAT(15X,'NU IS GREATER THAN 10.')
      END IF
      IF(GFBAR.NE.0.0.AND.LRF.NE.3)  THEN
         WRITE(NOUT,65) GFBAR
   65    FORMAT(10X,'AVERAGE FISSION WIDTH IS ',1PE12.5)
         IF(ENUF.LE.10.)    THEN
      ...
      END IF
      WRITE(NOUT,70)  D,SF
   70 FORMAT(10X,'AVERAGE LEVEL SPACING IS ',1PE12.5/                   &       
     &       10X,'STRENGTH FUNCTION IS ',1PE12.5)
!
...
!
      USE PSYCHE_DEF, ONLY : NOUT
      USE PSYCHE_DEF, ONLY : AWR
      USE PSYCHE_DEF, ONLY : ANEUTR
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) L
      REAL(KIND=4) SF,GGBAR
!
      INTEGER(KIND=4) MS,LP1,ITST
      INTEGER(KIND=4) I,K
      REAL(KIND=4) AW
!
      REAL(KIND=4), DIMENSION(4) :: AWT
      DATA AWT/80.,140.,210.,260./
      REAL(KIND=4), DIMENSION(2,2) ::  GT
      DATA GT/.04,.01,9.0,.05/
      REAL(KIND=4), DIMENSION(4,2,2) :: SFL
...
!
   ...
   10    FORMAT(15X,'AVERAGE GAMMA WIDTH',1PE12.5/                      &       
     &    20X,' LIES OUTSIDE LIMITS',1PE12.5,' TO',1PE12.5,' EV.')
      END IF
!
...
!
   ...
   20             FORMAT(15X,'STRENGTH FUNCTION',1PE12.5/               &       
     &                20X,' LIES OUTSIDE LIMITS',1PE12.5,' TO',1PE12.5)
               END IF
               GO TO 100
            END IF
         END DO
...
!
      IMPLICIT NONE
!
      REAL(KIND=4) YIE,ENU,DELNU
!
      INTEGER(KIND=4) N
      REAL(KIND=4) AIPT,YIEL,YIEU,ENUM
!
      INTEGER(KIND=4), PARAMETER :: IYIEMAX=40
      REAL(KIND=4), DIMENSION(IYIEMAX) :: YIEFAC
      DATA YIEFAC/                                                      &       
...
!
      USE PSYCHE_DEF, ONLY : NOUT
      USE PSYCHE_DEF, ONLY : CONSA,SFA
      USE PSYCHE_DEF, ONLY : EPI2
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IPATH,IA,NE,NN,L,NRS
      REAL(KIND=4) AJ
      REAL(KIND=4), DIMENSION(NN) :: A
!
      INTEGER(KIND=4) IOF,INOW,LENN,NERRS,ITF1,ITF2
      INTEGER(KIND=4) N,K
      REAL(KIND=4) DENSD,AJP1,CONSD,SFC
      REAL(KIND=4) DENS,AJPONE,CONS,DENOM,SF,DIF
!
!
!     WRITE OUT HEADING
!
      WRITE(NOUT,5)  IA,L
    5 FORMAT(//6X,'ISOTOPE MASS = ',I4,'. L = ',I2,                     &       
...
!***********************************************************************
!
      REAL(KIND=4) FUNCTION AWRN(IZ)
!
!     EXTRACTS ISOTOPIC PARAMETERS OF AN ELEMENT FROM MASTER TABLE
!
      USE PSYCHE_DEF,ONLY : NISN,ABNIS,IAIS,SPINIS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IZ
!
      INTEGER(KIND=4) NTHEL,IZN,IZN1
!
      INTEGER(KIND=4), PARAMETER :: KISO=287
      INTEGER(KIND=4) I
      INTEGER(KIND=4), DIMENSION(KISO) :: IZASP
      DATA (IZASP(I),I=1,79)/                                           &       
     ...
     & 9223400,-9223535, 9223800/
      REAL(KIND=4), DIMENSION(KISO) :: ABN
      DATA (ABN(I),I=1,79)/                                             &       
...
!
      USE PSYCHE_DEF, ONLY : NOUT
      USE PSYCHE_DEF, ONLY : ZA
      USE PSYCHE_DEF, ONLY : LRP
      USE PSYCHE_DEF, ONLY : NIS,ZAIS,ABNS,SPINS
      USE PSYCHE_DEF, ONLY : NISN,NISMAT,IAIS,ZAIMAT,ABNIS,SPINIS
      USE PSYCHE_DEF, ONLY : EPI1
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      ...
      REAL(KIND=4) ANUM,DIFF
!
!     WRITE OUT HEADER
!
      WRITE(NOUT,5)
    5 FORMAT(/6X,'CHECK ON ISOTOPE PROPERTIES'/)
      ANUM = AMOD(ZA,1000.)
!
...
!     CHECK FILE 3 DATA
!
      USE PSYCHE_DEF, ONLY : JSCR
      USE PSYCHE_DEF, ONLY : NSUB,NFOR
      USE PSYCHE_DEF, ONLY : ELIS
      USE PSYCHE_DEF, ONLY : MT
      USE PSYCHE_DEF, ONLY : NISMAT
      USE PSYCHE_DEF, ONLY : SZMT3,NMT3,MT3,QIVAL,QVAL
      USE PSYCHE_DEF, ONLY : NBS,NBSN,MTS
      USE PSYCHE_DEF, ONLY : C1,C2
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IPART,JPART
      INTEGER(KIND=4) MTT,MTL,NLMOD,NBEG
      INTEGER(KIND=4) N
      INTEGER(KIND=4), PARAMETER :: NPARTS=6
      INTEGER(KIND=4), DIMENSION(NPARTS) :: ISAME
      DATA ISAME/0,1001,1002,1003,2003,2004/
      REAL(KIND=4) Q,QI,QMSAV
!
!     READ THE SECTION
!
      CALL RDTAB1
...
!
      USE PSYCHE_DEF, ONLY : NSUB
      USE PSYCHE_DEF, ONLY : MT
      USE PSYCHE_DEF, ONLY : NISMAT,ZAIMAT
      USE PSYCHE_DEF, ONLY : QUNK
      USE PSYCHE_DEF, ONLY : IZAB
      USE PSYCHE_DEF, ONLY : BIGNO
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      REAL(KIND=4) Q
!
      INTEGER(KIND=4) IPART,IZAIN,IZA,IZARES
      INTEGER(KIND=4) MTLIB,MTALL,MTPROD
      INTEGER(KIND=4) I,N,INIS
      INTEGER(KIND=4) IDIV
      INTEGER(KIND=4), PARAMETER :: IDIV0=100000
      REAL(KIND=4) QNEW,QLIB
      REAL(KIND=4) DIFF
      REAL(KIND=4), PARAMETER :: EPID=.05,EPIQ=2.5E+5
      REAL(KIND=8) AMASS,AMASO,AMASN,AMAST,AMASI,AMASR
!
      INTEGER(KIND=4), PARAMETER :: NPARTS=7
      INTEGER(KIND=4), DIMENSION(NPARTS) :: JPART
      DATA JPART/0,1,1001,1002,1003,2003,2004/
      INTEGER(KIND=4), DIMENSION(NPARTS) :: MTELAS
      DATA MTELAS/102,4,103,104,105,106,107/
      INTEGER(KIND=4), PARAMETER :: NMTS=38
      INTEGER(KIND=4), DIMENSION(NMTS) :: LIBRY
      DATA LIBRY/     4100000,  16200000,  17300000,                    &       
...
!
      USE PSYCHE_DEF, ONLY : NOUT
      USE PSYCHE_DEF, ONLY : JSCR
      USE PSYCHE_DEF, ONLY : NSUB,LRP
      USE PSYCHE_DEF, ONLY : AWR
      USE PSYCHE_DEF, ONLY : MT
      USE PSYCHE_DEF, ONLY : E1,E2
      USE PSYCHE_DEF, ONLY : NCOEF,ENERGY,F
      USE PSYCHE_DEF, ONLY : C2,NP,NP2
      USE PSYCHE_DEF, ONLY : L1H,L2H,C2L,L1L,NPL
      USE PSYCHE_DEF, ONLY : Y
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LVT,LTT,LI
      INTEGER(KIND=4) NE,NOFF
      INTEGER(KIND=4) INT,IP,MTT
      INTEGER(KIND=4) L,N,NN
      REAL(KIND=4), PARAMETER :: WLCONS=3.056E-08
      REAL(KIND=4) ENN,PO,CON,CONS,YT,PERERR
      REAL(KIND=4), DIMENSION(4) :: SIGP
      REAL(KIND=4), DIMENSION(1500) :: EN,WLT,STOT,SEL
!
!     SAVE FLAGS
!
      LVT = L1H
...
!
   90 WRITE(NOUT,95)
   95 FORMAT(6X,'CANNOT PERFORM WICK LIMIT TEST BECAUSE TOTAL ',        &       
...
!
      USE PSYCHE_DEF, ONLY : NOUT
      USE PSYCHE_DEF, ONLY : ISCR,KSCR
      USE PSYCHE_DEF, ONLY : NSUB
      USE PSYCHE_DEF, ONLY : AWR
      USE PSYCHE_DEF, ONLY : MF,MT
      USE PSYCHE_DEF, ONLY : NTERP,INTERP
      USE PSYCHE_DEF, ONLY : MT3,NMT3,QIVAL
      USE PSYCHE_DEF, ONLY : MTTOT,MTDCT
      USE PSYCHE_DEF, ONLY : MF5,MT5
      USE PSYCHE_DEF, ONLY : EGRID,AEBAR,EINT,PKINT
      USE PSYCHE_DEF, ONLY : C1,C2,L2,NR,NP,NR2,NP2
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : NBT2,JNT2
      USE PSYCHE_DEF, ONLY : N1H
      USE PSYCHE_DEF, ONLY : X,Y
!
      IMPLICIT NONE
!
      ...
      INTEGER(KIND=4) LF
      INTEGER(KIND=4) NE,NA
      INTEGER(KIND=4) NK,NREGS,NPTS
      INTEGER(KIND=4) IFP
      INTEGER(KIND=4) INC,I,II,L,NN,NUL
      REAL(KIND=4) E,U,THETA
      REAL(KIND=4) GEBAR,EBAR,EBAR0,FNORM,PK,PERNK
      REAL(KIND=4) EFL,EFH
      REAL(KIND=4) EMULT,ECMPQ
      REAL(KIND=4) AWRX
      REAL(KIND=4) QI
      REAL(KIND=4), EXTERNAL :: ANU
!
...
!
         DO I=2,NK
            CALL RDWRSC(2,KSCR)
            NE = 15000
            CALL MERGE(X(2501),NA,X,NP,Y(2501),NE)
            IF(I.NE.NK)   THEN
               NA = NE
               DO II=1,NA
                  X(II+2500) = Y(II+2500)
...
!
   30 IF(NSUB.EQ.4)  GO TO 100
      WRITE(NOUT,35)
   35 FORMAT(/5X,'ENERGY GRID     TOTAL SECONDARY      ',               &       
     &  'AVERAGE SECONDARY        ENERGY'/                              &       
     &  26X,'ENERGY            NEUTRON ENERGY',                         &       
     &  8X,'AVAILABLE'/                                                 &       
     &  5X,11('-'),6X,14('-'),6X,17('-'),6X,10('-'))
      AWRX = AWR/(AWR+1.)
      DO I=1,NE
         E = EGRID(I)
         X(I) = E
...
!********FLAG ALL TOTAL SECONDARIES G.T. ECM+QI
         ...
         ELSE
            IFLAG = ' '
         END IF
         WRITE(NOUT,'(5X,1PE11.4,6X,1PE11.4,A1,10X,1PE11.4,9X,1PE11.4)')&       
...
!
      ...
      REAL(KIND=4) EE
!
      INTEGER(KIND=4),PARAMETER :: NMUMTS=8
      INTEGER(KIND=4), DIMENSION(NMUMTS) ::  MUMTS
      DATA MUMTS/16,17,24,25,30,37,41,42/
      REAL(KIND=4), DIMENSION(NMUMTS) ::  AMULT
      DATA AMULT/2.,3.,2.,3.,2.,4.,2.,3./
!
...
!
      ...
      USE PSYCHE_DEF, ONLY : MT
      USE PSYCHE_DEF, ONLY : NTERP,INTERP
      USE PSYCHE_DEF, ONLY : NMT3,MT3,QVAL,QIVAL
      USE PSYCHE_DEF, ONLY : EGRID,AEBAR,EINT,PKINT
      USE PSYCHE_DEF, ONLY : NCOEF,ENERGY,F
      USE PSYCHE_DEF, ONLY : EPI3
      USE PSYCHE_DEF, ONLY : C1,C2,L2,NR,NP,C22,L12,L22,NR2,NP2
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : NBT2,JNT2
      USE PSYCHE_DEF, ONLY : L2H,N1H,C2L,L1L,L2L,NPL,N2L
      USE PSYCHE_DEF, ONLY : X,Y
      USE PSYCHE_DEF, ONLY : NSEQP1
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LCT,LANG,LAW,LTP
      INTEGER(KIND=4) NK,NREGS,NPTS
      INTEGER(KIND=4) IZAP
      INTEGER(KIND=4) NW,ND
      INTEGER(KIND=4) LL,NOG,NOC,N1,N2,MM,NU
      INTEGER(KIND=4) INT1,INT2,NE,IFP,NCYC,L,IST,IDISCR,IBZ,NMU
      INTEGER(KIND=4) NSEQPN
      INTEGER(KIND=4) I,II,IK,IE,IU,N,NM
      REAL(KIND=4) E,EMIN,EMAX,ENOW,ENEXT,EU
      REAL(KIND=4) Q,QI,QK
      REAL(KIND=4) PK,XY,YN,X3,Y3,X4,Y4,EBAR,GEBAR,FNORM
      REAL(KIND=4) TEST,YIBZ,SUM,ANS,EAVAIL,PERR
!
      INTEGER(KIND=4), PARAMETER :: NPOUTMAX=100
      CHARACTER(LEN=10), DIMENSION(NPOUTMAX) :: HL
      INTEGER(KIND=4), DIMENSION(NPOUTMAX) :: LIK,JIK,NIK
      REAL(KIND=4),DIMENSION(NPOUTMAX) :: VAL
      REAL(KIND=4), DIMENSION(201) :: XX,YY,ZZ
!
         ...
         ELSE IF(LAW.EQ.7) THEN
            ...
            DO N=1,NE
               ...
               DO NM=1,NMU
                  CALL RDTAB1
                  XX(NM) = C2
                  CALL EAVE(X,Y,N2,NBT,JNT,N1,0.0,0.0,0.0,1,GEBAR,      &       
     &                 FNORM,EBAR)
                  YY(NM) = FNORM
                  ZZ(NM) = GEBAR
               END DO
...
!
      ...
      END DO
      WRITE(NOUT,40) Q
   40 FORMAT(/10X,'ENERGY BALANCE SUMMARY: Q = ',1PE13.5//              &       
     &     23X,'TOTAL SECONDARY ENERGY BY EMITTED PARTICLE')
      IF (NK.GT.1) THEN
         WRITE(NOUT,'(1X,2A10,A7,(10A10))')HE,HA,HP,HS,(HL(I),I=1,NK)
         IF(NK.GT.12) WRITE(NOUT,'(A)') ' '
      ELSE IF (NK.EQ.1) THEN
         WRITE(NOUT,'(1X,2A10,A7,(10A10))')HE,HA,HP,HL(1)
      END IF
      DO IU=1,NU
         E = X(IU)
         SUM = 0.
   ...
   50          CALL TERP1(EGRID(L-1),AEBAR(L-1),EGRID(L),AEBAR(L),      &       
     &              E,ANS,INT1)
         ...
         IF(EAVAIL.LE.0.) THEN
            ...
            END IF
         ELSE
            PERR = 100.*ABS(SUM-EAVAIL)/EAVAIL
         END IF
         IF(MT.EQ.5)   THEN
            IF(NK.EQ.1) THEN
               WRITE(NOUT,'(1X,1PE10.2,17X,(10(1PE10.2)))')  E,VAL(1)
            ELSE
               WRITE(NOUT,'(1X,1PE10.2,17X,(10(1PE10.2)))')             &       
     &                E,SUM,(VAL(I),I=1,NK)
               IF(NK.GT.12) WRITE(NOUT,'(A)') ' '
            END IF
         ELSE
            IF(PERR.LT.999.99) THEN
               IF(NK.EQ.1) THEN
                  WRITE(NOUT,'(1X,2(1PE10.2),0PF7.2,(10(1PE10.2)))')    &       
     &                     E,EAVAIL,PERR,VAL(1)
               ELSE
                  WRITE(NOUT,'(1X,2(1PE10.2),0PF7.2,(10(1PE10.2)))')    &       
     &                     E,EAVAIL,PERR,SUM,(VAL(I),I=1,NK)
                  IF(NK.GT.12) WRITE(NOUT,'(A)') ' '
               END IF
            ELSE
               IF(NK.EQ.1) THEN
                  WRITE(NOUT,'(1X,2(1PE10.2),A,1PE10.2)')               &       
     &                   E,EAVAIL,' ******',VAL(1)
               ELSE
                  WRITE(NOUT,'(1X,2(1PE10.2),A,(10(1PE10.2)))')         &       
     &                   E,EAVAIL,' ******',SUM,(VAL(I),I=1,NK)
                  IF(NK.GT.12) WRITE(NOUT,'(A)') ' '
               END IF
            END IF
         END IF
...
!
      USE PSYCHE_DEF, ONLY : KSCR,LSCR
      USE PSYCHE_DEF, ONLY : AWR
      USE PSYCHE_DEF, ONLY : MF,MT
      USE PSYCHE_DEF, ONLY : MT1213,MTSCR
      USE PSYCHE_DEF, ONLY : NMTBAR,MTBAR
      USE PSYCHE_DEF, ONLY : EGRID
      USE PSYCHE_DEF, ONLY : EBAV
      USE PSYCHE_DEF, ONLY : C1,L1,L2,NR,NP
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : L1H,N1H
      USE PSYCHE_DEF, ONLY : X,Y
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LO,LF,LP
      INTEGER(KIND=4) NE
      INTEGER(KIND=4) NK
      INTEGER(KIND=4) IFP
      INTEGER(KIND=4) I,N,NTH
      REAL(KIND=4) EGK,AWRX,E,ECM,EBAR,YIELD
!
      LO = L1H
!
...
!
      USE PSYCHE_DEF, ONLY : JSCR,KSCR,LSCR
      USE PSYCHE_DEF, ONLY : AWR
      USE PSYCHE_DEF, ONLY : MF,MT
      USE PSYCHE_DEF, ONLY : MT1213,MTSCR
      USE PSYCHE_DEF, ONLY : NMTBAR,MTBAR
      USE PSYCHE_DEF, ONLY : NBSN
      USE PSYCHE_DEF, ONLY : EGRID
      USE PSYCHE_DEF, ONLY : EBAV,YLD
      USE PSYCHE_DEF, ONLY : EPI4
      USE PSYCHE_DEF, ONLY : C1,L1,L2,NR,NP
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : N1H
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MTNOW,NK,LP,LF,NE
      INTEGER(KIND=4) NX,IFP,NSPT
      INTEGER(KIND=4) I,N,NTH
      REAL(KIND=4) EGK,AWRX,E,ECM,EBAR,YIELD,TERP,ESIG,SIG3N
      REAL(KIND=4), DIMENSION(1500) :: SIG3
!
...
!
      USE PSYCHE_DEF, ONLY : NCOEF,ENERGY,F
      USE PSYCHE_DEF, ONLY : NR,NR2,NP2
      USE PSYCHE_DEF, ONLY : L1H,L2H,N1H,N2H,C2L,NPL
      USE PSYCHE_DEF, ONLY : Y
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NK,LI,NI,LTT,NE
...
!
      USE PSYCHE_DEF, ONLY : ISCR,KSCR,LSCR
      USE PSYCHE_DEF, ONLY : MF,MT
      USE PSYCHE_DEF, ONLY : NTERP,INTERP
      USE PSYCHE_DEF, ONLY : MT1213,MTSCR
      USE PSYCHE_DEF, ONLY : MF5,MT5
      USE PSYCHE_DEF, ONLY : NMTBAR,MTBAR
      USE PSYCHE_DEF, ONLY : EGRID,AEBAR,EINT,PKINT
      USE PSYCHE_DEF, ONLY : C2,NR,NP,NR2,NP2
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : NBT2,JNT2
      USE PSYCHE_DEF, ONLY : X,Y
      USE PSYCHE_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MTNOW,MTT
      INTEGER(KIND=4) ICONTG,IFP
      INTEGER(KIND=4) NE,NTG,NPTS,NREGS
      INTEGER(KIND=4) I,J,NGAM,NGAM1
      REAL(KIND=4) U,THETA
      REAL(KIND=4) E,GEBAR,FNORM,TERP,ANUE
      REAL(KIND=4), DIMENSION(3000) ::  XT,YT
!
      IERX = 0
!
!     SAVE CURRENT MT
...
!
      ...
      USE PSYCHE_DEF, ONLY : AWR
      USE PSYCHE_DEF, ONLY : MT
      USE PSYCHE_DEF, ONLY : NMT3,MT3,QVAL
      USE PSYCHE_DEF, ONLY : MT1213
      USE PSYCHE_DEF, ONLY : NMTBAR,MTBAR
      USE PSYCHE_DEF, ONLY : MTTOT,MTDCT
      USE PSYCHE_DEF, ONLY : MF5,MT5
      USE PSYCHE_DEF, ONLY : EPI4
      USE PSYCHE_DEF, ONLY : NP
      USE PSYCHE_DEF, ONLY : X,Y
!
      IMPLICIT NONE
!
      ...
      CHARACTER(LEN=2), DIMENSION(2) :: FLAG
      DATA FLAG/'* ','**'/
      CHARACTER(LEN=11), DIMENSION(7) :: ECHAR
      CHARACTER(LEN=11), PARAMETER :: XXX='     X     '
      INTEGER(KIND=4) MTT,MTNOW,MFT,MFN,MTN,INP,ICMPQ
      INTEGER(KIND=4) NE,NENU,NEGC,NGT
      INTEGER(KIND=4) I,J,K,N,N1213,ITOTAL,NQ,NMPTS,NZERO
      INTEGER(KIND=4), DIMENSION(1) ::  NPLIN,ALLLIN
      REAL(KIND=4) Q,ECMPQ,AWRX
      REAL(KIND=4), DIMENSION(7) :: EFINAL
      REAL(KIND=4), DIMENSION(2000) :: XGT,YGT,XGC,YGC,YGD
      REAL(KIND=4), DIMENSION(1500) :: XN,YN,YNU
!
      ALLLIN(1) = 1
!
...
!
      WRITE(NOUT,10)
   10 FORMAT('1',39X,'COMPOSITE TEST OF FILES 5 12 13 15')
!
...
!
         ...
         IF(NENU.NE.0)   THEN
            ...
            ELSE
               NE = 4000
               CALL MERGE(XN,NENU,XGT,NGT,X,NE)
            END IF
         ELSE
            NE = NGT
            DO I=1,NE
...
!
         ...
         AWRX = AWR/(AWR+1.0)
         WRITE(NOUT,75)    MTNOW
   75    FORMAT(//4X,'SECTION ',I3//                                    &       
     &      14X,'E(IN)           TOTAL          AVERAGE          TOTAL',&       
     &      10X,'AVERAGE          ENERGY          TOTAL'/               &       
     &      21X,4(7X,'SECONDARY'),'       AVAILABLE        NEUTRON'/    &       
     &      20X,2(9X,'NEUTRON'),'          GAMMA           GAMMA',      &       
     &      25X,'AND GAMMA'/                                            &       
     &      19X,4(10X,'ENERGY'),26X,'ENERGY'/                           &       
     &      14X,7('(EV)',12X)/11X,7(10('-'),6X))
         DO NMPTS=1,NE
            FLAG1 = BLANK
            FLAG2 = BLANK
            EFINAL(1) = X(NMPTS)
...
!           OUTPUT TEST RESULTS
!
            WRITE(NOUT,'(5X,4(5X,A11),1X,A2,2X,A11,2(5X,A11),2X,A2)')   &       
...
!
         WRITE(NOUT,80)
   80    FORMAT(/5X,'X  INDICATES VALUE CANNOT BE CALCULATED'/          &       
     &           5X,'*  INDICATES ETOTAL GREATER THAN ECM'/             &       
     &           5X,'** INDICATES ETOTAL GREATER THAN ECM+Q')
   90 ENDDO
!
...
!
      ...
      USE PSYCHE_DEF, ONLY : MT
      USE PSYCHE_DEF, ONLY : ENMIN,ENMAX
      USE PSYCHE_DEF, ONLY : E1,IUNR
      USE PSYCHE_DEF, ONLY : NBOUND,RBOUND
      USE PSYCHE_DEF, ONLY : THERMA
      USE PSYCHE_DEF, ONLY : EPI6
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IPATH
!
      INTEGER(KIND=4) MBOUND,NCURR
      INTEGER(KIND=4) IPATH1,IP,INT,IND
      INTEGER(KIND=4) I,II,J,N
      INTEGER(KIND=4), PARAMETER :: NSAMPL=1000
      INTEGER(KIND=4), PARAMETER :: NCARN=4
      INTEGER(KIND=4), DIMENSION(NCARN) :: ICARN
      DATA ICARN/1,2,18,102/
      INTEGER(KIND=4), PARAMETER :: NGRUPS=203
      REAL(KIND=4), DIMENSION(NGRUPS) :: EGR
      INTEGER(KIND=4), PARAMETER :: NREACS= 4
      REAL(KIND=4), DIMENSION(NREACS) :: RESINT,SIGO
      REAL(KIND=4), DIMENSION(NGRUPS,NREACS) :: SIGP
      REAL(KIND=4) EPIM,EPIP,ECURR,XSEC
      REAL(KIND=4) XLP,XHP,ANS
!
...
!
      IF(MBOUND.GT.1)   THEN
         WRITE(NOUT,'(/6X,A)') 'RESONANCE REGION BOUNDARY TESTS'
         DO II=2,MBOUND,2
            IF(II.EQ.2.AND.EGR(II).LT.ENMIN)  THEN
               WRITE(NOUT,'(/15X,A,1PE12.5,30X,A,1PE12.5)')             &       
     &             'E = ',EGR(II)/EPIM,'SIGMA+  = ',SIGP(II+1,IND)
            ELSE
               WRITE(NOUT,'(/A,1PE12.5,30X,A,1PE12.5/61X,A,1PE12.5)')   &       
     &            '               E = ',EGR(II)/EPIM,'SIGMA-  = ',      &       
...
!
      USE PSYCHE_DEF, ONLY : ISCR
      USE PSYCHE_DEF, ONLY : NIS,NERS,ABNS
      USE PSYCHE_DEF, ONLY : E1,E2,IRES,EMIDLE
      USE PSYCHE_DEF, ONLY : IPT,RESPAR
      USE PSYCHE_DEF, ONLY : CROC
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) LRU,LRF
      INTEGER(KIND=4) NE,NG,NER,LL,NLS,NRS,NSS,NJS
      INTEGER(KIND=4) NGRE,NFRE,NIRE,NCRE,NRCHAN
      INTEGER(KIND=4) I,K,K1,M1,N1,N,NN,NNN,NNNN
      REAL(KIND=4) EL,EU,EAB
      REAL(KIND=4) EMIN,EMID,EMAX
      REAL(KIND=4) ABN,KFAC,RFAC,ARAT,AWRI,AJ,SPI,AC,AS,GJ,DEN,U,DU
      REAL(KIND=4) E0,ER,ERK,G,GN,GG,GF,GX,GFISS,GNK
      REAL(KIND=4) RHO,SF,SHIFT,PF,RHOG,SHIFTK,PFK
      REAL(KIND=4) GAM,GAM1,GAM2,GAM3,GAM4,GAM5,GAM6
      REAL(KIND=4) GSQR,GGG,GAR
      REAL(KIND=4) SR0,SR,Y1,Y2,SRY1,SRY2
      REAL(KIND=4) GY1,GY2,GY3,GY4
      REAL(KIND=4) YG1,YG2,YG3,YG4
      REAL(KIND=4) ARCTAN,ARGLOG,ARCCTN
      REAL(KIND=4) GU,CEE,PENDEP,GAMRCY,CEESQR,CEE1
!
      INTEGER(KIND=4) NREACS
      REAL(KIND=4), DIMENSION(NREACS) :: RESINT,SIG
      INTEGER(KIND=4), PARAMETER :: NEMAX=151
      REAL(KIND=4), DIMENSION(NEMAX) :: EGR
!
!     PROCESS ALL ISOTOPES
!
      RESINT(3) = 0.
...
!       SINGLE ISOTOPE
!
      USE PSYCHE_DEF, ONLY : NERS
      USE PSYCHE_DEF, ONLY : IRES,EMIDLE
      USE PSYCHE_DEF, ONLY : NPED,NRED,NBTED,JNTED,EP,APED
      USE PSYCHE_DEF, ONLY : IPT,RESPAR
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NNIS,IPATH
      REAL(KIND=4) EC
      REAL(KIND=4), DIMENSION(4) :: SIG
!
      INTEGER(KIND=4) LRU,LRF,LSSF,LRUP,NER
      INTEGER(KIND=4) INOWP,NLOOP,IBEG
      INTEGER(KIND=4) I,KK
      REAL(KIND=4) E,E0,DENOM,DE,ANS
      REAL(KIND=4), DIMENSION(4) :: SIGM,SIGO
!
      SIG = 0.
!
...
! **********************************************************************
!
      USE PSYCHE_DEF, ONLY : RESPAR
      USE PSYCHE_DEF, ONLY : PI,CROC
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) INOW
...
!
      INTEGER(KIND=4) NLS
      INTEGER(KIND=4) LL,LRX,NRS
      INTEGER(KIND=4) I,L
      REAL(KIND=4) AWRI,AP,A,SPI,QX
      REAL(KIND=4) K,KFAC,ARAT,PIFAC,RHO,RHOP,PHI
      REAL(KIND=4) ER,EJ,AJ,GJ,GN,GG,GF,GC,GNE,GX,GTT
      REAL(KIND=4) SE,PE,SER,PER,SEC,PEC,RHOC,COS2P,SINSQ,SPOT
      REAL(KIND=4) ERP,EDELT,COMFAC,DEN
!
...
!
      INTEGER(KIND=4) NLS
      INTEGER(KIND=4) LL,LRX,NRS
      INTEGER(KIND=4) J,NUMJ
      INTEGER(KIND=4) I,L
      REAL(KIND=4) AWRI,AP,A,SPI,QX
      REAL(KIND=4) K,KFAC,ARAT,PIFAC,RHO,RHOP,PHI
      REAL(KIND=4) ER,AJ,GN,GG,GF,GC,GNE,GX,GTT
      REAL(KIND=4) SE,PE,SER,PER,SEC,PEC,RHOC,COS2P,SIN2P
      REAL(KIND=4) SUM,FL,AJMIN,AJMAX
      REAL(KIND=4) ERP,EDELT,COMFAC,DEN,DIFF,XF
      REAL(KIND=4), DIMENSION(20) :: GJ
      REAL(KIND=4), DIMENSION(20,2) :: SIGJ
!
!     ZERO OUT CROSS SECTION CONTRIBUTION FROM THIS SECTION (SIGP)
!
      SIGP = 0.0
...
!
      ...
      REAL(KIND=4), DIMENSION(4) :: SIGP
!
      INTEGER(KIND=4) NLS,NRS,LL
      INTEGER(KIND=4) INOWB,JJL,NUMJ,IFIS
      INTEGER(KIND=4) KPSTV,KNGTV,ISKIP,KKKKKK
      INTEGER(KIND=4) I,J,JJ,L,KCHANL
      REAL(KIND=4) SPI,GJD
      REAL(KIND=4) A,AP,RHO,RHOP,SE,PE,PHI
      REAL(KIND=4) ER,GN,GG,GF,GC,PER,A1,A2,A3
      REAL(KIND=4) FL,AJMIN,AJMAX,AJC,AJ,AJPM
      REAL(KIND=8) DEN,DIFF,DE2,P1,P2,GJ,GG4,T1,T2,T3,T4
      REAL(KIND=8) TERMT,TERMN,TERMF,TERMG,SIGNNI,SIGNGI,SIGNFI,SIGNTI
...
!
      ...
      REAL(KIND=4), DIMENSION(4) :: SIGP
!
      INTEGER(KIND=4) NLS,NJS,NLJ,LI,IST
      INTEGER(KIND=4) I,J,L,N
      REAL(KIND=4) AWRI,AP,ARAT,EROOT,ESQ,ECUB
      REAL(KIND=4) K,C,OMG,SNF,CSF
      REAL(KIND=4) BAKT,BAKF,BAKC
      REAL(KIND=4) DE,DW,GR,GI,T1,T2
!
!     ZERO OUT CROSS SECTION CONTRIBUTION FROM THIS SECTION (SIGP)
!
      SIGP = 0.0
...
!
      ...
      REAL(KIND=4), DIMENSION(4) :: SIGP
!
      INTEGER(KIND=4) NLS,NJS,NSS,NRS,LL
      INTEGER(KIND=4) LBK,LPS,ITPT
      INTEGER(KIND=4) J,I1,I2,INOWR,INOWN
      INTEGER(KIND=4) I,II,III,L,N,NS,NJ
      INTEGER(KIND=4) NGRE,NFRE,NIRE,NCRE,NRCHAN
      INTEGER(KIND=4), DIMENSION(4) :: MTRE,LC
      INTEGER(KIND=4), DIMENSION(4,4) :: ICRPT
      REAL(KIND=4) K,KFAC,AWRI,ARAT,PIFAC,RHRES,SABS
      REAL(KIND=4) SE,PE,SD,PD,SN,PN,SER,PER
      REAL(KIND=4) RHO,PHRO,PHRI,RR0,RI0,PHIR,PHRES,PHEN
      REAL(KIND=4) EEFF,EREFF
      REAL(KIND=4) ER,GN,GG,GF,GELIM
      REAL(KIND=4) AC,AS,AJ,GJ,SPI
      REAL(KIND=4) GNE,GNR,GTT,EDELT,DEN,COMFAC
      REAL(KIND=4), DIMENSION(4) :: QRE,GC,SIGC
      COMPLEX(KIND=4) ONE,UNIT,RFCNN,SLSJNN,PHI0,PHI,R0,EXPHI
!
      UNIT = CMPLX(0.0,1.0)
      ONE = CMPLX(1.0,0.0)
!
...
!
      ...
      REAL(KIND=4), DIMENSION(4) :: SIGP
!
      INTEGER(KIND=4) LFW,LSSF,MUF
      INTEGER(KIND=4) NE,NLS,NJS,LL
      INTEGER(KIND=4) NUME,INEXT,NFS,IND,IDG
      INTEGER(KIND=4) MU,NU,LAMDA
      INTEGER(KIND=4) J,L,N,IN
      REAL(KIND=4) SPI,A
      REAL(KIND=4) AWRI,AA,RAT,CONST
      REAL(KIND=4) ENU,ER2,K,RHO,RHOC,PS,VL
      REAL(KIND=4) GFX,GXX,DX,AJ,GJ,AMUN,GNOX,GGX,GNX
      REAL(KIND=4) GC,GS,GFF
      REAL(KIND=4) DIFF,DEN,TEMP,TERG,TERS,TERF,SPOT
      REAL(KIND=4), DIMENSION(2) :: EG
      REAL(KIND=4), DIMENSION(2,3) :: SIGINT
!
!     RETRIEVE NUCLIDE INFORMATION
!
      LFW = RESPAR(INOW+2)
...
!
      ...
      REAL(KIND=4), DIMENSION(4) :: SIGP
!
      INTEGER(KIND=4) NLS,NJS,LSSF,NE,LL
      INTEGER(KIND=4) INT,NUME,IDG,IND,INEXT
      INTEGER(KIND=4) MU,NU,LAMDA
      INTEGER(KIND=4) J,L,N,IN
      REAL(KIND=4) SPI,A
      REAL(KIND=4) ENU,ER2,K,RHO,RHOC,PS,VL
      REAL(KIND=4) AWRI,AA,RAT,CONST
      REAL(KIND=4) AJ,AMUX,AMUN,AMUF
      REAL(KIND=4) GJ,GT,DX,GXX,GNOX,GNX,GGX,GFX,GC,GS,GFF,SPOT
      REAL(KIND=4) GAMMA,GALPHA,GBETA
      REAL(KIND=4) DIFF,DEN,TEMP,TERG,TERS,TERF
      REAL(KIND=4), DIMENSION(2) :: EG
      REAL(KIND=4), DIMENSION(2,3) :: SIGINT
!
!     RETRIEVE NUCLIDE INFORMATION
...
!
      INTEGER(KIND=4) L
      REAL(KIND=4) RHO,SE,PE
!
      REAL(KIND=4) R2,R4,R6,R8,DEN
!
      IF(L.EQ.0) THEN
!        S-WAVE
         SE = 0.0
...
!
      INTEGER(KIND=4) L
      REAL(KIND=4) RHO,RHOC,AMUN,VL,PS
!
      REAL(KIND=4) R2,R4
!
      IF(L.EQ.0) THEN
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) INOW,NE,ISKIP,NUME,IDG
      REAL(KIND=4) E
      REAL(KIND=4), DIMENSION(2) :: EG
!
      INTEGER(KIND=4) IND,NEXT
      INTEGER(KIND=4) N
!
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) MU,NU,LAMBDA,ID
      REAL(KIND=4) GALPHA,GBETA,GAMMA,S,DF
!
      INTEGER(KIND=4) J,K,L
      REAL(KIND=4) XJ,WJ,XK,WK,XL,WL
!
      REAL(KIND=4), DIMENSION(4,10) :: XX
...
!
      USE PSYCHE_DEF, ONLY :NCOEF,NMOM
!
      IMPLICIT NONE
!
      NMOM = NCOEF
...
!
      USE PSYCHE_DEF, ONLY : NCOEFMAX,F
      USE PSYCHE_DEF, ONLY : NMOM,MOMENT
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NFM1
      INTEGER(KIND=4) L,N
      REAL(KIND=4) P10,P20,FL,FNF
      REAL(KIND=4), DIMENSION(NCOEFMAX) :: P1,P2
!
      MOMENT(1) = F(1)
...
!
      USE PSYCHE_DEF, ONLY : MEAN,VAR
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) N,ITEST
      INTEGER(KIND=4) L
      INTEGER(KIND=4) LL
      REAL(KIND=4) QCK
      REAL(KIND=4), EXTERNAL :: QUE
!
      CALL GETMUS(N,ITEST)
      QCK = -1.
      IF(N.LE.0)   THEN
...
!
      INTEGER(KIND=4) N,ITEST
!
      INTEGER(KIND=4) IM1,IP1,NP1,IHF4
      INTEGER(KIND=4) I,K
      REAL(KIND=4) RMO1,RMO2,RMO3
      REAL(KIND=4), DIMENSION(32) :: MU,NORM,L,Q,SIG
      REAL(KIND=4), DIMENSION(32,33) :: A
!
      RMO1 = MOMENT(1)
      RMO2 = MOMENT(2)
      RMO3 = MOMENT(3)
...
!***********************************************************************
!
      REAL(KIND=4)FUNCTION QUE(NDEGRE,XXX)
!
...
!
      USE PSYCHE_DEF, ONLY : NCOEF
      USE PSYCHE_DEF, ONLY : W
      USE PSYCHE_DEF, ONLY : NSEQP1
      USE PSYCHE_DEF, ONLY : EMESS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NPOW,NP1,NM1,NINT,I,NRT,N1,N2,N3,NC
      REAL(KIND=4) PNEG,PROB,F1,F2,F3,FM1,R1,R2,X1,X2,X3,SGN
      REAL(KIND=4), EXTERNAL :: LEGVAL,FINDRT,PRBINT
!
      W = 0.
      PNEG = 0.
      CALL GENSTR(NPOW)
...
!
      USE PSYCHE_DEF, ONLY : NCOEF,F
      USE PSYCHE_DEF, ONLY : NPOWS,W
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) NPOW
...
!
      INTEGER(KIND=4) NP1
      REAL(KIND=4), DIMENSION(NP1) :: F,FP
!
      INTEGER(KIND=4) N,IP1
      INTEGER(KIND=4) I,J
      REAL(KIND=4) FI
!
      N = NP1 - 1
...
!
      INTEGER(KIND=4) NCS
      REAL(KIND=4) XXX,FUNC
!
      INTEGER(KIND=4) I,J
      REAL(KIND=4) FF,FI,SIGN
      REAL(KIND=4), DIMENSION(65) :: P
!
      P(1) = 1.
      P(2) = XXX
      DO I=3,NPOWS
...
!***********************************************************************
!
      REAL(KIND=4) FUNCTION LEGVAL(NP1,XXX)
!
...
!***********************************************************************
!
      REAL(KIND=4) FUNCTION FINDRT(X1,X2,F1)
!
!     FINDS ROOT GIVEN BOUNDS ON ROOT.
!
      USE PSYCHE_DEF, ONLY : NPOWS
!
      IMPLICIT NONE
!
      REAL(KIND=4) X1,X2,F1
!
      INTEGER(KIND=4) I
      REAL(KIND=4) XA,XB,XC,FA,FC
      REAL(KIND=4), EXTERNAL :: LEGVAL
!
      XA = X1
      XB = X2
      FA = F1
...
!***********************************************************************
!
      REAL(KIND=4) FUNCTION PRBINT(R1,R2)
!
...
!
      USE PSYCHE_DEF, ONLY : JIN
      USE PSYCHE_DEF, ONLY : MATP,MFP,MTP
      USE PSYCHE_DEF, ONLY : TEXT
      USE PSYCHE_DEF, ONLY : NSEQP
      USE PSYCHE_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      ...
      USE PSYCHE_DEF, ONLY : MATP,MFP,MTP
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE PSYCHE_DEF, ONLY : NSEQP,NSEQP1
      USE PSYCHE_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      ...
      USE PSYCHE_DEF, ONLY : MAT,MF,MT
      USE PSYCHE_DEF, ONLY : C1H,C2H,L1H,L2H,N1H,N2H
      USE PSYCHE_DEF, ONLY : NSEQ,NSEQP1
      USE PSYCHE_DEF, ONLY : IERX
!
      IMPLICIT NONE
!
...
!
      USE PSYCHE_DEF, ONLY : JIN
      USE PSYCHE_DEF, ONLY : MATP,MFP,MTP
      USE PSYCHE_DEF, ONLY : C12,C22,L12,L22,NR2,NP2
      USE PSYCHE_DEF, ONLY : NBT2,JNT2
      USE PSYCHE_DEF, ONLY : NSEQ,NSEQP,NSEQP1
      USE PSYCHE_DEF, ONLY : IERX
!
...
!
      USE PSYCHE_DEF, ONLY : JIN
      USE PSYCHE_DEF, ONLY : MATP,MFP,MTP
      USE PSYCHE_DEF, ONLY : C1,C2,L1,L2,NR,NP
      USE PSYCHE_DEF, ONLY : NBT,JNT
      USE PSYCHE_DEF, ONLY : X,Y
      USE PSYCHE_DEF, ONLY : NSEQ,NSEQP,NSEQP1
      USE PSYCHE_DEF, ONLY : IERX
...
!
      USE PSYCHE_DEF, ONLY : JIN
      USE PSYCHE_DEF, ONLY : MATP,MFP,MTP
      USE PSYCHE_DEF, ONLY : C1L,C2L,L1L,L2L,NPL,N2L
      USE PSYCHE_DEF, ONLY : Y
      USE PSYCHE_DEF, ONLY : NSEQP,NSEQP1
      USE PSYCHE_DEF, ONLY : IERX
!
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IRW,IUNIT
!
      INTEGER(KIND=4) NI,N2
      INTEGER(KIND=4) I,N
      INTEGER(KIND=4), PARAMETER :: ISIZE=250
!
      IF(IRW.EQ.1)   THEN
!
!     WRITE TO SCRATCH
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=1) IPATH
!
      INTEGER(KIND=4) IMAX,N2
      INTEGER(KIND=4) I,LLL,LI,ILI,N
!
      IF(IPATH.EQ.1)  THEN
...
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IPT0,NC
      INTEGER(KIND=4) I,N,NN
      INTEGER(KIND=4), DIMENSION(6) :: IRESPR
!
!     READ CONT-LIKE RECORD
!
...
!***********************************************************************
!
      SUBROUTINE MERGE(X1,N1,X2,N2,XF,NF)
!
!     ROUTINE TO MERGE TWO ENERGY GRIDS
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) N1,N2,NF
      INTEGER(KIND=4) N
      REAL(KIND=4),DIMENSION(N1) :: X1
      REAL(KIND=4),DIMENSION(N2) :: X2
      REAL(KIND=4),DIMENSION(*) :: XF
!
      INTEGER(KIND=4) N1C,N2C
!
!     INITIALIZE
!
      NF = 0
...
!     INT IS THE STARTING POINT IN THE INTERPOLATION
...
!
   ...
   30 IF(X(IP).NE.X(IP+1))  THEN
         INT = I
         IF(INT.GT.NR)   INT = NR
         CALL TERP1(X(IP),Y(IP),X(IP+1),Y(IP+1),XI,YI,JNT(INT))
      ELSE
         YI = (Y(IP)+Y(IP+1))/2.
      END IF
!
...
!
      INTEGER(KIND=4) INP
      INTEGER(KIND=4) NP,NR
      INTEGER(KIND=4), DIMENSION(NR) :: NTERP,INTERP
      REAL(KIND=4) E,PK
      REAL(KIND=4), DIMENSION(NP) :: EINT,PKINT
!
      INTEGER(KIND=4) J
      INTEGER(KIND=4) I
      REAL(KIND=4) DIFF
...
!
      ...
      REAL(KIND=4) E,ANS
!
      INTEGER(KIND=4) LR,LP,ITRP
      INTEGER(KIND=4) IIMIN,IIMAX
      INTEGER(KIND=4) J,INP
      INTEGER(KIND=4) I
      REAL(KIND=4) DIFF
      REAL(KIND=4) EOLD
      DATA EOLD/0.0/
...
!
      REAL(KIND=4) XLP,XHP,ANS
!
      INTEGER(KIND=4) I,NL,NH,M
      INTEGER(KIND=4) N
      REAL(KIND=4) XL,XH,AN
!
...
!
      INTEGER(KIND=4) J,K
      REAL(KIND=4) A,B,X12,AX12,BX,BX1,Z,ZZ,FJ,FK
      REAL(KIND=4) FACK,ARK,ANSP,CON,EXPA
!
      ANS = 0.0
...
!
      INTEGER(KIND=4) LF
      REAL(KIND=4) E,U,THETA,XY,YN,EBAR
      INTEGER(KIND=4) NP,NR
      INTEGER(KIND=4), DIMENSION(NR) :: NBT,JNT
      REAL(KIND=4), DIMENSION(NP) :: X,Y
!
      INTEGER(KIND=4) NTERP,INT,IFIN
      INTEGER(KIND=4) N
      REAL(KIND=4) X3,X4,Y3,Y4,X2,ANS
      REAL(KIND=4) EX,XMAX,SXMAX
      REAL(KIND=4) C1,C2,C3,C4,C5,C6,C7,C8,C9,C6P,CERF,CON
      REAL(KIND=4), PARAMETER :: PI2=0.8863369
      REAL(KIND=4), EXTERNAL :: ERF
!
      EBAR = 0.0
!
      ...
      ELSE IF(LF.EQ.5) THEN
         ...
         DO N=2,NP
            ...
            ELSE
               X2 = X4
            END IF
            CALL XECSI(X3,Y3,X4,Y4,X3,X2,INT,ANS)
            XY = XY + ANS
            CALL ECSI(X3,Y3,X4,Y4,X3,X2,INT,ANS)
            YN = YN + ANS
...
!
      REAL(KIND=4) T
      REAL(KIND=4), PARAMETER :: P=0.3275911
      REAL(KIND=4), PARAMETER :: A1=0.254829592
      REAL(KIND=4), PARAMETER :: A2=-0.284496736
      REAL(KIND=4), PARAMETER :: A3=1.421413741
      REAL(KIND=4), PARAMETER :: A4=-1.453152027
      REAL(KIND=4), PARAMETER :: A5=1.06140542
!
      IF(X.GT.5.)   THEN
         ERF = 1.
      ELSE
...
!
      USE PSYCHE_DEF, ONLY : NSMASS,IZAB,AMASSB
!
      IMPLICIT NONE
!
      INTEGER(KIND=4) IZA
      INTEGER(KIND=4), PARAMETER :: NMAX=156,ZMAX=100,AMAX=256
!
      INTEGER(KIND=4) IZ,IA,IN
      INTEGER(KIND=4) I
      REAL(KIND=4) ALPHA,BZSQ,ETANZ
      REAL(KIND=4), PARAMETER :: ETA=195.856
