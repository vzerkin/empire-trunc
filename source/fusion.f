...
C COMMON variables
C
      REAL*8 :: ABScs, ELAcs, S1, SINl, SINlcc, SINlcont, TOTcs
...
C Local variables
C
      REAL*8 :: ak2, chsp, cnj, coef, csmax, csvalue, e1tmp, ecms, 
     &          einlab, el, ener, p1, parcnj, qdtmp, r2, rp, s0, s1a, 
     &          smax, smin, sumx, wparg, xmas_npro, xmas_ntrg
      CHARACTER(3) :: ctldir
      CHARACTER(132) :: ctmp
      CHARACTER(23) :: ctmp23
      REAL*8 :: DMAX1
      LOGICAL :: dodwba, fexist, ldbwacalc, lodd, ltlj, relcal
      REAL*8 :: E1, E2, SIGQD, XM1
      REAL :: FLOAT, SNGL
      INTEGER :: i, ichsp, il, ip, ipa, istat1, istat2, itmp1, itmp2, j, 
     &           k, l, lmax, lmin, maxlw, mul, nang
...
C-----Reduced mass corrected for proper mass values
     ...
     &   - SEJc(Npro).EQ.0.0D0)S1 = 1.0
      ...
      DO i = 1, NDLw
        ...
        sel(i) = 0.D0
      ENDDO
      WRITE(ctmp23,'(i3.3,i3.3,1h_,i3.3,i3.3,1h_,i9.9)')INT(ZEJc(Npro)), 
     &      INT(AEJc(Npro)), INT(Z(Ntrg)), INT(A(Ntrg)), 
     &      INT(EINl*1000000)
...
C           Reading no more than 2*NDLW rows
     ...
     &          ' (all previous instructions concerning fusion ignored)'
          DO i = 1, 2*NDLw
            READ(43,*,END = 10)cnj, parcnj, csvalue
...
C--------END of spin distribution from file SDFILE
     ...
     &                ELV(NLV(Ntrg),Ntrg)
            WRITE(8,*)'WARNING: CN excitation energy below continuum'
            WRITE(8,*)'WARNING: cut-off. zero reaction cross section'
...
C--------and calculate transmission coefficients
        ...
        coef = 1.D0
        IF(INT(AEJc(0)).GT.0)coef = 10.*PI/ak2/(2*XJLv(LEVtarg,Ntrg) + 
     &                              1.0)/(2*SEJc(Npro) + 1.0)
 
        CSFus = 0.D0
        DO il = 1, NDLw
          READ(11,*,END = 15)csvalue
          stl(il) = csvalue
...
C--------calculation of o.m. transmission coefficients for absorption
      ELSEIF(KTRlom(Npro,Ntrg).GT.0)THEN
        ...
        ltlj = .FALSE.
        lodd = .FALSE.
        IF((MOD(NINT(Z(Ntrg)),2).NE.0.OR.MOD(NINT(A(Ntrg)-Z(Ntrg)),2)
     &     .NE.0).AND.MOD(NINT(A(Ntrg)),2).NE.0)lodd = .TRUE.
        dodwba = .FALSE.
        DO l = 1, NDCollev
...
C--------------Inelastic cross section (incident.ics)
            OPEN(45,FILE = 'dwba.ICS',STATUS = 'OLD',ERR = 30)
            OPEN(46,FILE = 'ccm.ICS',STATUS = 'OLD',ERR = 30)
            OPEN(47,FILE = 'INCIDENT.ICS',STATUS = 'UNKNOWN')
            READ(45,'(A80)',END = 30,ERR = 30)rstring
            READ(46,'(A80)',END = 20,ERR = 20)       ! first line is taken from dwba
   20       WRITE(47,'(A80)')rstring
            DO i = 2, ND_nlv
              READ(45,'(A80)',END = 30,ERR = 30)rstring
              READ(46,'(A80)',END = 25,ERR = 25)rstring
   25         WRITE(47,'(A80)')rstring
            ENDDO
   30       CLOSE(45,STATUS = 'DELETE')
            CLOSE(46,STATUS = 'DELETE')
            CLOSE(47)
C--------------Angular distribution (incident.ang)
            OPEN(45,FILE = 'dwba.ANG',STATUS = 'OLD',ERR = 55)
 
            READ(45,'(A80)',ERR = 55,END = 55)rstring
            OPEN(46,FILE = 'ccm.ANG',STATUS = 'OLD',ERR = 55)
            READ(46,'(A80)',ERR = 35,END = 35)        ! first line is taken from dwba
   35       OPEN(47,FILE = 'INCIDENT.ANG',STATUS = 'UNKNOWN')
 
            WRITE(47,'(A80)')rstring
            DO i = 1, ND_nlv
C-----------------checking the correspondence of the excited states
              READ(45,'(i5,6x,i4,i5)',END = 55,ERR = 55)istat1, itmp2, 
     &             nang
 
              READ(46,'(i5,6x,i4)',END = 40,ERR = 40)istat2
C-----------------checking the correspondence of the excited states for even-even targets
     ...
     &              ' WARNING: DWBA and CCM state order do not coincide'
              ENDIF
              BACKSPACE(46)
              READ(46,'(A80)',ERR = 40,END = 40)rstring
              GOTO 45
   40         BACKSPACE(45)
              READ(45,'(A80)',ERR = 55,END = 55)rstring
   45         WRITE(47,'(A80)')rstring
              DO j = 1, itmp2*nang     ! ecis06
                READ(45,'(A80)',ERR = 55,END = 55)rstring
                READ(46,'(A80)',ERR = 50,END = 50)rstring
   50           WRITE(47,'(A80)')rstring
              ENDDO
            ENDDO
...
C--------------Experimental angular distribution (incident.ang)
            IF(ICAlangs.GT.0)THEN        ! To be updated for ecis06
              OPEN(45,FILE = 'dwba.EXP',STATUS = 'OLD',ERR = 75)
              READ(45,'(A80)',END = 75)rstring
              OPEN(46,FILE = 'ccm.EXP',STATUS = 'OLD',ERR = 75)
              READ(46,'(A80)',END = 60)      ! first line is taken from dwba
   60         OPEN(47,FILE = 'INCIDENT.EXP',STATUS = 'UNKNOWN')
              WRITE(47,'(A80)')rstring
              DO i = 1, ICAlangs
                READ(45,'(i5,5x,i5)',END = 75)istat1, nang
                READ(46,'(i5)',END = 65)istat2
C-----------------checking the correspondence of the excited states
   ...
   65           BACKSPACE(45)
                ...
                DO j = 1, nang
                  READ(45,'(A80)',END = 75)rstring
                  READ(46,'(A80)',END = 70)rstring
   70             WRITE(47,'(A80)')rstring
                ENDDO
              ENDDO
            ENDIF
   75       CLOSE(45,STATUS = 'DELETE')
            CLOSE(46,STATUS = 'DELETE')
            CLOSE(47)
            IF(DEFormed)THEN
              CALL ECIS2EMPIRE_TL_TRG(Npro,Ntrg,maxlw,stl,sel,.FALSE.)
...
C--------IWARN=4 - 'Energy requested higher than recommended for this potential'
        IF((IWArn.EQ.1.OR.IWArn.EQ.2).AND.FIRst_ein.AND.IOUt.GE.5)
     &     WRITE(8,*)' WARNING: ', KTRlom(0,0), 
     &               ' OMP not recommended for target Z,A=', Z(Ntrg), 
     &               '-', (Ntrg)
        IF((IWArn.EQ.3.OR.IWArn.EQ.4).AND.IOUt.GE.5)WRITE(8,*)
     &     ' WARNING: ', KTRlom(0,0), ' OMP not recommended for E=', 
     &     EINl
        IWArn = 0
        ...
        DO ip = 1, 2      ! over parity
          DO j = 1, NLW !over compound nucleus spin
            ...
            DO ichsp = 1, mul
              ...
              DO k = lmin, lmax
                sumx = sumx + PAR(ip,LVP(LEVtarg,Ntrg),k - 1)*stl(k)
     &                *DRTl(k)
              ENDDO
            ENDDO
            POP(NEX(1),j,ip,1) = coef*sumx*(FLOAT(2*j + 1) - 2.0*S1)
     &                           *FUSred
            CSFus = CSFus + POP(NEX(1),j,ip,1)
            csmax = DMAX1(POP(NEX(1),j,ip,1),csmax)
          ENDDO
   ...
   80 WRITE(8,*)' '
     ...
     &     ' Inelastic scattering results provided by DWBA calculations'
      ...
      IF(CSFus.GT.0.D0.AND.TOTred.NE.1.D0)THEN
        ...
        WRITE(8,'(1x,A18,F5.2,A49,F5.2)')' FUSRED scaled by ', 
     &        SNGL(FUSred), 
...
C--------Corrected scattering radius
        ...
        WRITE(8,*)
        IF(S0_obs.GT.0.)THEN
          WRITE(8,1010)S0_obs, S0_unc
          WRITE(12,1010)S0_obs, S0_unc
 1010     FORMAT(7x,54('*')/6x,' LOW ENERGY NEUTRON SCATTERING:'/6x,
     &           ' Exp.  Strength functions S0 =',f6.3,' (',f6.4,')')
        ELSE
          WRITE(8,
     &     '(7x,54(1h*)/         7x,''LOW ENERGY NEUTRON SCATTERING:'')'
     &     )
          WRITE(12,
     &     '(7x,54(1h*)/         7x,''LOW ENERGY NEUTRON SCATTERING:'')'
     &     )
        ENDIF
        WRITE(8,1020)s0*1D4, FUSred*stl(1), s1a*1D4, FUSred*stl(2), 
     &               s2a*1D4, FUSred*stl(3), EINl*1.D3, TOTcs*TOTred, 
     &               rp, 1.35*(A(Ntrg)**0.333333333)
        WRITE(12,1020)s0*1D4, FUSred*stl(1), s1a*1D4, FUSred*stl(2), 
     &                s2a*1D4, FUSred*stl(3), EINl*1.D3, TOTcs*TOTred, 
     &                rp, 1.35*(A(Ntrg)**0.333333333)
 1020   FORMAT(6x,' Calc. Strength functions S0 =',f6.3,' T0=',d12.6/6x,
     &         '                          S1 =',f6.3,' T1=',d12.6/6x,
     &         '                          S2 =',f6.3,' T2=',d12.6/6x,
     &         ' Elab = ',F6.1,' keV','        Total XS = ',F9.2,
     &         ' mb'/6x,' Scattering radius =',f7.3,' fm'/6x,
     &         '   1.35 A**(1/3)   =',f7.3,' fm'//7x,54('*'))
        WRITE(8,*)
        WRITE(12,*)
        selast = 0.D0
        DO l = 0, maxlw
        ...
        IF(selast.GT.0.D0)THEN
          ...
          WRITE(8,
     &'(7x,28HSHAPE ELASTIC CROSS SECTION=,F10.3,1x,              6H(ECI
     &S:,F10.3,1H),1x,2hmb)')selast, ELAcs
          WRITE(12,
     &'(7x,28HSHAPE ELASTIC CROSS SECTION=,F10.3,1x,              6H(ECI
     &S:,F10.3,1H),1x,2hmb)')selast, ELAcs
          WRITE(53,
     &'(7x,5HElab=,F7.2,1x,3HkeV,              6x,17HSHAPE ELASTIC XS=,F
     &10.3,1x,2hmb)')EINl*1000, selast
          WRITE(8,1030)
          WRITE(12,1030)
          WRITE(53,1030)
          DO l = 0, maxlw
            IF(stl(l + 1).LT.1.D-15)EXIT
            WRITE(8,1040)l, stl(l + 1), 10.D0*PI/ak2*sel(l + 1)
            WRITE(12,1040)l, stl(l + 1), 10.D0*PI/ak2*sel(l + 1)
            WRITE(53,1040)l, stl(l + 1), 10.D0*PI/ak2*sel(l + 1)
          ENDDO
          WRITE(8,1050)
          WRITE(12,1050)
          WRITE(53,1050)
 ...
 1050     FORMAT(6x,' ****************************************')
     ...
     &           '      Sreact = Sfusion + SUM_over_exc.lev.j {Sinl(j)}'
        ...
        ELSE
          ...
          DO l = 0, maxlw
            IF(stl(l + 1).LT.1.D-15)EXIT
            WRITE(8,1070)l, stl(l + 1)
            WRITE(12,1070)l, stl(l + 1)
            WRITE(53,1070)l, stl(l + 1)
          ENDDO
          WRITE(8,1080)
          WRITE(12,1080)
          WRITE(53,1080)
 ...
 1080     FORMAT(6x,' ************************')
     ...
     &           '      Sreact = Sfusion + SUM_over_exc.lev.j {Sinl(j)}'
      ...
      IF(INT(AEJc(0)).GT.0)coef = 10.D0*PI/ak2/(2*XJLv(LEVtarg,Ntrg)
     &                            + 1.0)/(2*SEJc(Npro) + 1.0)
C-----channel spin min and max
      smin = ABS(SEJc(Npro) - XJLv(LEVtarg,Ntrg))
      smax = SEJc(Npro) + XJLv(LEVtarg,Ntrg)
...
C        DO j = 1, NDLW !over compound nucleus spin
          ...
          DO ichsp = 1, mul
            ...
            lmax = MIN0(maxlw,lmax)
            DO k = lmin, lmax
              sumx = sumx + PAR(ip,LVP(LEVtarg,Ntrg),k-1)*stl(k)*DRTl(k)
            ENDDO
          ENDDO
          POP(NEX(1),j,ip,1) = coef*sumx*(FLOAT(2*j+1) - 2.0*S1)*FUSred
          CSFus = CSFus + POP(NEX(1),j,ip,1)
          csmax = DMAX1(POP(NEX(1),j,ip,1),csmax)
        ENDDO
      ENDDO
...
C    &         '             to discrete un-coupled collective levels'
     ...
     &    '      CSFus(SUM_Tl)    CSFus+SINl+CC+SINlcont    ABScs(OMP) '
        WRITE(8,'(4x,3(4x,D12.6,4x))')CSFus, CSFus + SINl + SINlcc + 
     &                                SINlcont, ABScs
        WRITE(8,*)
      ENDIF
...
C Dummy arguments
C
      REAL*8 :: Ap, At, Bfus, Crl, Csfus, E1, Ein, Zp, Zt
C
C Local variables
C
      REAL*8 :: arg, d, dfu, e2, ee2, f, ht, le1, le2, m0, m1, m2, p, t, 
     &          vc, vmax, vmm, vn, x, y
      INTEGER :: INT
      INTEGER :: j, jl
      REAL :: SNGL
...
Ccc
     ...
     &' Fusion cross section calculated using Bass model as a reference'
      ...
      ELSE
        ...
        WRITE(8,
     &'(1X,  '' WARNING: Incident energy below the Bass fusion barrier''
     &)')
        WRITE(8,
     &'(1X,  '' WARNING: For the Bass model, fusion cross section = 0'')
     &')
        WRITE(8,*)
        Csfus = 0.D0
      ENDIF
...
C Dummy arguments
C
      REAL*8 :: Ap, At, Crl, D, Ein
      REAL*8 :: XFUS
C
C Local variables
C
      REAL*8 :: al, args, sumx, tl
      REAL :: FLOAT
      INTEGER :: i, icrl
      INTEGER :: INT
C
...
C COMMON variables
C
      REAL*8 :: BAVe, E, EROt, SIG
      COMMON /EXTRAP/ BAVe, EROt, E, SIG
C
C Dummy arguments
C
      REAL*8 :: A, Ap, At, Bas, Ecm, Expush, Sigi, Trunc
      INTEGER :: Ndlw, Nlw
      REAL*8, DIMENSION(Ndlw) :: Stl
C
C Local variables
C
      REAL*8 :: amu, dintf, prob, r0, rf, xlow, xmax
      REAL*8, EXTERNAL :: F, G
      INTEGER :: j
C
C*** End of declarations rewritten by SPAG
...
C Dummy arguments
C
      INTEGER :: Icalangs, Iopsys, Iret, Length
      CHARACTER(*) :: Outname
C
C Local variables
C
