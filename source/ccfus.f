...
Ccc   * $Id: ccfus.f,v 1.1.1.1 2001-07-09 17:33:39 mike Exp $
C
...
C
     ...
     &                 factor, facw, facwd, fkap, fl, fla(NDCC, 2), 
     &                 flam1, flamem(NDCC), flo, fpi, gl, h2m, homega, 
     &                 p, pa(NDCC, 2), ra, rb, rbar, rcal, rcald, rr, 
     &                 rred, s0, s1, s2, sig0, sigl0(NDLW), sq, su0, 
     &                 su1, su2, sum, ur, vb, vbl, vbw, vbwl
      REAL FLOAT
...
C
     ...
     &      *(1. - 2.*Z(0)/A(0)))*rred + DV - 20.
      ...
      WRITE(6, 99001)DV, vb, rbar, homega
99001 FORMAT(/, '  Parameters for DV=', F6.2, //, '      VB=', F6.1, 
     &       '       RB=', F6.2, '       H-OMEGA=', F5.2, /)
      ...
      IF(NSCc.NE.0)THEN
         WRITE(6, 99002)
99002    FORMAT(/, 2X, 35('*'), /, 
     &          '  *    BETA    * LAMDA *   Q(MEV)   *', /, 2X, 35('*'))
         DO n = 1, NSCc
            WRITE(6, 99003)BETcc(n), FLAm(n), QCC(n)
99003       FORMAT('  *  ', F6.2, '    *  ', F3.0, '  *  ', F6.2, 
     &             '    *')
            IF(BETcc(n).EQ.0.0D0)THEN
               WRITE(6, *)' WARNING !!!'
     ...
     &          + 1.)/rcal**2)/fpi
            ...
            dfla(n, 2) = -dfla(n, 1)
            dfla2(n, 1) = ( - 4.*(fkap*f)**2/sq) + 8.*(fkap*f*f)
     &                    **2/sq**3
            dfla2(n, 2) = -dfla2(n, 1)
...
99004    FORMAT(2X, 35('*'), /)
      ...
      IF(NACc.NE.0)THEN
         WRITE(6, 99005)
99005    FORMAT(2X, 27('*'), /, '  *   F(MEV)   *   Q(MEV)   *', /, 2X, 
     &          27('*'))
         DO n = ns1, nmax
            fkap = 0.71
C           READ (5,*) FCD(N),QCC(N)
            WRITE(6, 99006)FCD(n), QCC(n)
99006       FORMAT('  *  ', F6.2, '    *  ', F6.2, '    *')
...
99008 FORMAT(/, 'Fusion cross section without channel coupling', 
     &       1PE10.3E2, ' mb', /)
      s0 = 0.
      s1 = 0.
      s2 = 0.
