Ccc   * $Author: Capote $
Ccc   * $Date: 2005-05-20 10:19:33 $
Ccc   * $Id: HF-comp.f,v 1.61 2005-05-20 10:19:33 Capote Exp $
C
...
C-----DE spectra (DDX are not done for fission although they could be)
      IF (Nejc.EQ. - 1) THEN
         ...
         DO ie = 1, NDECSE
            DO iejc = 0, NDEJC
               IF (POPcse(Iec,iejc,ie,Nnuc).NE.0) CSEfis(ie,iejc)
     &             = CSEfis(ie,iejc) + POPcse(Iec,iejc,ie,Nnuc)*xnor
            ENDDO
         ENDDO
         RETURN   !if fission
      ENDIF
