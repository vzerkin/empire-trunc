...
C
      ...
      REAL*8, DIMENSION(Lmx) :: Qq
      REAL*8, DIMENSION(Mxr) :: Rwo
      REAL*8, DIMENSION(Np) :: Xp, Yp
C
C Local variables
C
      REAL*8 :: d1, rer, sy, yci
      REAL*8 :: DMAX1
...
C
      INTEGER :: Ier, N1, Np
      REAL*8, DIMENSION(N1,N1 + 2) :: Aa
      REAL*8, DIMENSION(N1) :: Qq
      REAL*8, DIMENSION(Np) :: Xp, Yp
C
C Local variables
C
...
C
      ...
      REAL*8 :: Uu
      REAL*8 :: POLLG1
      REAL*8, DIMENSION(Nl + 1) :: Ql
C
C Local variables
C
      INTEGER :: l, n1
      REAL*8, DIMENSION(20) :: pl
      REAL*8 :: ss
C
C*** End of declarations rewritten by SPAG
C
...
C
      REAL*8 :: Determ
      INTEGER :: M, N
      REAL*8, DIMENSION(NDMscs,NDMscs) :: As
      REAL*8, DIMENSION(NDMscs) :: Bs
C
C Local variables
C
      REAL*8, DIMENSION(NDMscs,NDMscs) :: a
      REAL*8 :: amax, sog, swap, t
      REAL*8, DIMENSION(NDMscs) :: b, pivot
      INTEGER :: i, icolum, irow, j, jcolum, jrow, k, l, l1, nnn
      INTEGER, DIMENSION(NDMscs,2) :: indeks
      INTEGER, DIMENSION(NDMscs) :: ipivot
C
C*** End of declarations rewritten by SPAG
C
C
...
C
      INTEGER, PARAMETER :: ISD = 0, NG = 4*NDMscs - 3*ISD
C
C Dummy arguments
C
      REAL*8 :: Determ
      INTEGER :: M, N
      REAL*8, DIMENSION(NG,NG) :: As
      REAL*8, DIMENSION(NG) :: Bs
C
C Local variables
C
      REAL*8, DIMENSION(NG,NG) :: a
      REAL*8 :: amax, sog, swap, t
      REAL*8, DIMENSION(NG) :: b, pivot
      INTEGER :: i, icolum, irow, j, jcolum, jrow, k, l, l1, nnn
      INTEGER, DIMENSION(NG,2) :: indeks
      INTEGER, DIMENSION(NG) :: ipivot
C
C*** End of declarations rewritten by SPAG
C
C
...
C Local variables
C
      CHARACTER(2), DIMENSION(0:NDZmax) :: mat
C
C*** End of declarations rewritten by SPAG
C
C
...
C
      ...
      WRITE(8,*)' ERROR: INSUFFICIENT MEMORY ALLOCATION TO ACOMODATE'
      WRITE(8,*)' ERROR: ALL REQUESTED NUCLEI;  EXECUTION STOPPED'
      WRITE(8,*)
...
C Dummy arguments
C
      REAL*8 :: Emax, Emin, Si, So, Xi, Xo
      INTEGER :: L, M, N
      REAL*8, DIMENSION(N,L) :: Yi
      REAL*8, DIMENSION(M,L) :: Yo
C
C Local variables
C
      REAL :: FLOAT
...
C--------start intrpolation
        DO io1 = imin, imax
          ...
          ELSEIF(ii1.EQ.N)THEN
            xint = Yi(ii1,ii2) + (yiue - Yi(ii1,ii2))*(xis - FLOAT(ii1))
          ELSE
...
C Local variables
C
      REAL*8 :: dy, e, em, en, facx, yif, yitot, yotot
      INTEGER :: INT
      INTEGER :: it, ll, mm, nn, nx
C
C*** End of declarations rewritten by SPAG
C
C
...
C Dummy arguments
C
      REAL*8 :: Exmax, Exmin, Ezmax, Ezmin, Sxi, Sxo, Szi, Szo, Xi, Xo, 
     &          Zi, Zo
      INTEGER :: Nxi, Nxo, Nzi, Nzo
      REAL*8, DIMENSION(Nxi,Nzi) :: Yi
      REAL*8, DIMENSION(Nxo,Nzo) :: Yo
C
C Local variables
C
      REAL*8 :: f1, f2, f3, f4, summino, t, u, xint, xis, zis
      REAL*8, DIMENSION(0:Nxi + 1,0:Nzi + 1) :: fyi
      INTEGER :: INT
...
C-----Check ranges and steps
      ...
      IF(Exmax - (Xo + (Nxo-1)*Sxo).GT.0.0001)THEN
        ...
        WRITE(8,*)'BINTERMAT: is above the maximum:   ', Xo + (Nxo - 1)
     &            *Sxo
        WRITE(8,*)'BINTERMAT: Execution terminated'
        STOP
      ENDIF
     ...
     &            Xi + (Nxi - 1)*Sxi
      ...
      IF(Ezmax - (Zo + (Nzo-1)*Szo).GT.0.0001)THEN
        ...
        WRITE(8,*)'BINTERMAT: is above the maximum:    ', Zo + (Nzo - 1)
     &            *Szi
        WRITE(8,*)'BINTERMAT: Execution terminated'
        STOP
      ENDIF
