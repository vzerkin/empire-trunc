...
C
      INTEGER, PARAMETER :: NDExd = NDEx + 11
C
C COMMON variables
C
      REAL*8 :: ADEgas3, AKDegas, BEProjdegas, CJGsdegas, EEXcdegas0, 
     &          ESTepdegas, GAQdegas, SGRdegas
      REAL*8, DIMENSION(10,2) :: BDEgas
      REAL*8, DIMENSION(10,10,9) :: BRArdegas
      REAL*8, DIMENSION(10) :: DDDegas, GGDegas
      REAL*8, DIMENSION(10,10) :: ENDidegas, SPIdidegas
      INTEGER :: IACdegas, IATdegas, IEDegas, INDegas, IZCdegas, 
     &           IZDegas, KEYdegas0
      REAL*8 :: JGSdegas
      INTEGER, DIMENSION(10) :: NBRadegas, NUDidegas
      INTEGER, DIMENSION(10,10) :: NEXddegas
      INTEGER, DIMENSION(9) :: NOLedegas
      CHARACTER(79) :: TITdegas
      REAL*8, DIMENSION(9) :: XCDegas
...
C
      ...
      REAL*8 :: excnq, popdiscang, popdiscrete, poptotal, renorm, 
     &          renpop, sumtmp, tmp, totemis
      REAL :: FLOAT
!!      REAL :: gdivp
      INTEGER :: i, idnacont, idnadisc, ie, ie0, ie1, ii, il, ipop, 
     &           ispec, j, jparity, jspin, lmax, na, ndisc, nejc, 
     &           nejcdegas, nnur, nnurdegas, nudim
      INTEGER :: INT
      REAL*8, DIMENSION(3,NDExd,25) :: populdegas
      REAL*8, DIMENSION(3,NDExd) :: specdegas
      REAL*8, DIMENSION(0:2) :: spectot
C
...
C--------Number of bins in discrete region
        ndisc = ELV(NLV(nnur),nnur)/DE + 0.5
        ndisc = MAX(0,ndisc - 1)
C--------Set IDNA-matrix index
        idnacont = 2*nejc
        idnadisc = 2*nejc - 1
        IF(nejc.EQ.0)THEN
          idnacont = 5
        ...
        spectot(nejc) = 0.0
C
C--------Transfer to contiuum
C
        IF(IDNa(idnacont,4).EQ.1)THEN
          ie1 = ndisc + 1
          DO ie0 = ie1, ie1 + NEX(nnur) - 1   ! Excitation energy bin
            sumtmp = 0
            ipop = ie0 - ie1 + 1
            ispec = NEX(nnur) - ipop + 1
            DO jspin = 1, lmax     ! Spin (parities populated evenly)
              tmp = populdegas(nnurdegas,ie0,jspin)*0.5*renorm/DE
              POP(ipop,jspin,1,nnur) = POP(ipop,jspin,1,nnur) + tmp
              POP(ipop,jspin,2,nnur) = POP(ipop,jspin,2,nnur) + tmp
              sumtmp = sumtmp + 2.0*tmp
            ENDDO
            IF(nejc.EQ.1)THEN      !neutron spectra are shifted by 1
              CSE(ispec + 1,nejc,1) = CSE(ispec + 1,nejc,1) + sumtmp
              AUSpec(ispec + 1,nejc) = AUSpec(ispec + 1,nejc) + sumtmp
            ELSE
              CSE(ispec,nejc,1) = CSE(ispec,nejc,1) + sumtmp
...
C-----------Continuum exclusive spectra *** done ***
        ENDIF
C
C--------Transfer to discrete levels
C
        IF(IDNa(idnadisc,4).EQ.1)THEN
C-----------Population evenly distributed over levels
          popdiscrete = 0.0
          DO ie0 = 1, ndisc
            DO jspin = 1, lmax
              popdiscrete = popdiscrete + 
     &                      populdegas(nnurdegas,ie0,jspin)
            ENDDO
          ENDDO
          popdiscrete = popdiscrete*renorm*ESTepdegas/NLV(nnur)
          popdiscang = popdiscrete/4.0/PI
          excnq = EX(NEX(1),1) - Q(nejc,1)
          DO il = 1, NLV(nnur)
            POPlv(il,nnur) = POPlv(il,nnur) + popdiscrete
C--------------store population of discrete levels for recoils' calculations
            REClev(il,nejc) = REClev(il,nejc) + popdiscrete
            ie = (excnq - ELV(il,nnur))/DE + 1.0
            CSE(ie,nejc,1) = CSE(ie,nejc,1) + popdiscrete/DE
            spectot(nejc) = spectot(nejc) + popdiscrete/DE
...
C--------------Add isotropic DEGAS contribution to direct ang. distributions
            IF(nejc.NE.0)THEN
              ...
              DO na = 1, NDAng
                CSAlev(na,il,nejc) = CSAlev(na,il,nejc) + popdiscang
              ENDDO
            ENDIF
          ENDDO
        ENDIF
        spectot(nejc) = spectot(nejc)*DE
        CSEmis(nejc,1) = CSEmis(nejc,1) + spectot(nejc)
      ENDDO   !over ejectiles
C-----total DEGAS emission accepted in calculations (note matrix IDNa)
      totemis = spectot(0) + spectot(1) + spectot(2)
...
C COMMON variables
C
      REAL*8 :: A3, AK, BEProj, CJGs, EEXc0, ESTep, GAQ, JGS
      REAL*8, DIMENSION(10,2) :: B
      REAL*8, DIMENSION(10,10,9) :: BRAr
      REAL*8, DIMENSION(10) :: DD, GG
      REAL*8, DIMENSION(10,10) :: ENDi, SPIdi
      INTEGER :: IAC, IAT, IE, IN, IZ, IZC, KEY0
      INTEGER, DIMENSION(10) :: NBRa, NUDi
      INTEGER, DIMENSION(10,10) :: NEXd
      INTEGER, DIMENSION(9) :: NOLe
      CHARACTER(79) :: TIT
      REAL*8, DIMENSION(9) :: XC
...
C     IN+IZ = 0 ==> STOP OF THE PROGRAM
      ...
      READ(ir,1030)EEXc0, ESTep, AK, JGS, CJGs, BEProj, GAQ
 1030 FORMAT(8F8.2)
...
C-----(LIMIT: .LE. NUDI)
      DO j = 1, 10
        ...
        IF(nudim.NE.0)THEN
          DO i = 1, nudim
            READ(ir,1040)ENDi(j,i), SPIdi(j,i), NEXd(j,i)
 1040       FORMAT(2F6.2,i6)
...
C
            IF(i.LE.NBRa(j))READ(ir,1050)(NOLe(ii),XC(ii),ii = 1,9)
 1050       FORMAT(9(i2,f6.3))
C              IF (I.LE.NBRA)
...
C-----
      ...
      WRITE(iw,1010)IN, IZ, KEY0
 1010 FORMAT(4I5,f10.3)
      WRITE(iw,1020)EEXc0, ESTep, AK, JGS, CJGs, BEProj, GAQ
 1020 FORMAT(8F8.2)
      WRITE(iw,1060)B      ! binding energies
      WRITE(iw,1060)GG     ! single-particle level density g
      WRITE(iw,1060)DD     ! pairing energies
      WRITE(iw,1070)NUDi
      ...
      DO j = 1, 10
        ...
        IF(nudim.NE.0)THEN
          DO i = 1, nudim   ! i=1 means g.s. level, etc
            WRITE(iw,1030)ENDi(j,i), SPIdi(j,i), NEXd(j,i)
 1030       FORMAT(2F6.2,i6)
            IF(i.LE.NBRa(j))THEN
              DO IE = 1, 9     ! ie=1 means deay goes to g.s., etc
                bbb = BRAr(j,i,IE)
                IF(IE.GT.0.AND.bbb.GT.0.)WRITE(iw,1040)IE, bbb
 1040           FORMAT(9(i2,f6.3))
              ENDDO
            ENDIF
          ENDDO
          WRITE(iw,1050)
...
C COMMON variables
C
      REAL*8 :: ADEgas3, AF, AKDegas, BEProjdegas, C2, CJGs0, CJGsdegas, 
     &          EEXc0degas, EF, ESTep, ESTepdegas, EXCef, G, GAQ, 
     &          GAQdegas, JGS, JGSdegas, SGRdegas, SIGma, SPP, T1111, 
     &          XX2, XX22
      REAL*8, DIMENSION(10,2) :: BDEgas
      REAL*8, DIMENSION(10,10,9) :: BRArdegas
      REAL*8, DIMENSION(10) :: DDDegas, GGDegas
      REAL*8, DIMENSION(10,10) :: ENDi, ENDidegas, SPIdi, SPIdidegas
      REAL*8, DIMENSION(140) :: FAL
      REAL*8, DIMENSION(40) :: FFPair, FFQ
      INTEGER :: I0, IAC, IACdegas, IAO, IAT, IATdegas, IE00, IEDegas, 
     &           II0, IIA, IIP, IJ, IL0, INDegas, ITJgs, ITSpp, IZ, IZC, 
     &           IZCdegas, IZDegas, IZO, KEY, KEYdegas0, N, NBR, NEStep, 
     &           NUDim
      INTEGER, DIMENSION(10) :: NBRadegas, NUDidegas
      INTEGER, DIMENSION(10,10) :: NEXddegas
      INTEGER, DIMENSION(9) :: NOLedegas
      REAL*8, DIMENSION(25,NDExd) :: OM
      REAL*8, DIMENSION(10,10,10) :: OMD
      REAL*8, DIMENSION(50,90) :: RFAc
      REAL*8, DIMENSION(NDExd) :: SG
      REAL*8, DIMENSION(25,NDExd,25) :: TC
      CHARACTER(79) :: TITdegas
      REAL*8, DIMENSION(25) :: TL
      REAL*8, DIMENSION(2,NDExd,25) :: TLP
      REAL*8, DIMENSION(9) :: XCDegas
      REAL*8, DIMENSION(3,25) :: XP
      REAL*8, DIMENSION(25,3,25) :: XX0
      REAL*8, DIMENSION(25,5,25) :: XX02
...
C Dummy arguments
C
      REAL*8, DIMENSION(3,NDExd,25) :: Popul
      REAL*8, DIMENSION(3,NDExd) :: Se
C
C Local variables
C
      REAL*8 :: a0, a1, a2, a3, aj, ak, akfc, ared, aver, b0, b0q, 
     &          beproj, bm, bmq, br, c0, cj, cjgs, cjgs1, cm, cp, e, 
     &          eeo, eer, eexc, eexc0, eg1, egm, emtg, emtp, endim, 
     &          endir, engy, eom, estep2, feed, flip, fsci, ga1, ge, 
     &          geg, gf, hlp3, jc, jc0, pi, pro, q, rexc, rl, ro1, ro2, 
     &          s, sg1, sgi, sgiq, sgir, sgr, sigf, sigfc, sigmacntot, 
     &          su, sumx, sumpro, t00, t0001, t11, t111, xdea
      REAL*8, DIMENSION(25,25) :: alms, alps, emg, emp, tau, tf0, xde
      REAL*8, DIMENSION(25) :: alp, f, pin0, sci, sigmacn, t, tf
      REAL*8, DIMENSION(10,2) :: b
      REAL*8, DIMENSION(10,10,9) :: brar
      REAL*8 :: CFA, CHAGUG, FPAIR, FQ, RFA, RO, X0, X02, X2, X22, XDAMP
      REAL*8 :: DABS, DLOG, DMOD
      REAL*8, DIMENSION(10) :: dd, feebr2, feebra, feepar, gg
      REAL*8, DIMENSION(2) :: epmxef
      REAL :: FLOAT
      REAL*8, DIMENSION(2,90) :: gae
      INTEGER :: i, iatx, ie, ie0, ie1, ieo, ieoblo, ieom, ier, igm, ih, 
     &           ii1, iid, iiir, iipnuc, iis, ijc, ik, il, ilis, ilma, 
     &           iloblo, in, indexe, inx, ip, ip1, ir, is, isu, iw, izx, 
     &           j, jav, k, key0, kna, l, m, m00, nbr1, ndi, nestdi, 
     &           neste1, nestem, nesteu, nn, nne, nne1, nudi1, nudim1
      INTEGER :: INT, MAX0, MIN0
      INTEGER, DIMENSION(10) :: nbra, nudi
      INTEGER, DIMENSION(10,10) :: nexd
      REAL*8, DIMENSION(10,10,10) :: omd1, omd2
      REAL*8, DIMENSION(2,25,NDExd) :: omr
      REAL*8, DIMENSION(2,25,10) :: r
      REAL*8, DIMENSION(3,NDExd) :: st
      REAL*8, DIMENSION(9,25,NDExd,25) :: tm
      REAL*8, DIMENSION(9) :: xc
C
C*** End of declarations rewritten by SPAG
C
...
Coblo    stop                ! Degas stop replaced by return
 1010   FORMAT(/4X,'DEGAS - Discrete Pre-Equilibrium-Equilibrium Gamma',
...
C        WRITE(IW,5)  tit
     ...
     &               /(j + m00)
          r(2,j,1) = (FLOAT(IZO) + (j - 1)*izx/FLOAT(iatx))/(j + m00)
        ENDDO
...
C        REACTION C.S. AND FEEDING OF COMPOUND SPIN CALCULATION
        ...
        IF(key0.GE.3.AND.key0.NE.5)READ(ir,1020)TL
 1020   FORMAT(10F6.2)
...
C        AT INCIDENT ENERGY FOR L=0, 1, ..., 14
   ...
   20     l = INT(rl) + 1
          IF(l.LE.24)THEN
            sumx = sumx + TL(l)
            rl = rl + 1.
            IF(rl.LE.(jc + s))GOTO 20
          ENDIF
          s = s + 1.
          IF(s.LE.(JGS + SPP))GOTO 15
          sumx = sumx*sigfc
          IF(sumx.GT.emtp)THEN
            ilma = ijc
            emtp = sumx
          ENDIF
          sgr = sgr + sumx
...
Coblo    IF (SGR.LE.0.d0) STOP       ! Degas stop replaced by return
        ...
        WRITE(iw,1030)IAT, eexc0, ESTep, sgr, ak, eg1, ga1, sg1, gg, b, 
     &                nudi, nbra
 1030   FORMAT(/' A = ',I3,'    Eexc = ',F5.2,
     &         ' MeV      energy step = ',F4.2,
     &         ' MeV'/' Reaction c.s. = ',F6.1,' mb      K'' = ',F5.0,
     &         ' MeV**3'/' GDR param:    E = ',F4.1,' MeV     width = ',
     &         F4.2,' MeV     peak c.s. = ',F5.0,
          ...
          DO i = 1, 25
            DO il = 1, 25
              DO ie = 1, NDExd
                ...
                sumx = sumx + TC(i,ie,il)
              ENDDO
            ENDDO
          ENDDO
          IF(sumx.LE.1.E-9)GOTO 70
        ENDIF
C
          ...
          DO i = 1, 25
            ...
            IF(ip.GT.0)THEN
              ...
              DO j = 1, IIA
                IJ = II0 + j
                ik = IJ - IIA
                r(1,i,IJ) = MAX((ip1*r(1,i,ik) - 1.D0)/flip,0.D0)
              ENDDO
              IJ = IJ + 1
            ...
            DO i = 1, 25
              ...
              IF(ip.GT.0)THEN
                ...
                DO j = 1, 2
                  ...
                  IF(nudim1.NE.0)rexc = rexc + ENDi(iiir,nudim1)
                  IF(iiir.LE.10.AND.ie.GT.nudi(iiir))omr(j,i,ie)
                ...
                IF(ndi.EQ.( - 1))THEN
                  ...
                  IF(j.LE.10)THEN
                    DO i = 1, 10
                      ih = i - 1
                      ip = i - IIA + m00
                      OMD(i,j,ie) = RO(ENDi(IIP,i),G,ip,ih)
                      sumx = sumx + OMD(i,j,ie)
                    ENDDO
                    IF(sumx.NE.0.)THEN
                      DO i = 1, 10
                        OMD(i,j,ie) = OMD(i,j,ie)/(sumx*ESTep)
                      ENDDO
                    ENDIF
                  ELSE
                    WRITE(iw,1040)II0, IIP, ie, CJGs0, SPIdi(IIP,ie), j
 1040               FORMAT('spiny   ii0,iip',2I3,'  ie',i3,
     ...
     &                 OMD(i,j,ie) = (2.*SPIdi(IIP,ie) + 1)/ESTep
                  ENDDO
                ENDIF
              ENDIF
            ENDDO
            ...
            DO ie = 1, nudi1
              ...
              IF(ndi.EQ.( - 1))THEN
                ...
                DO i = 1, 10
                  ...
                  sumx = sumx + omd1(i,j,ie)
                ENDDO
                IF(sumx.NE.0.)THEN
                  DO i = 1, 10
                    omd1(i,j,ie) = omd1(i,j,ie)/(sumx*ESTep)
                  ENDDO
                ENDIF
              ELSE
                ...
                DO i = 1, 10
                  N = 2*i - IIA + m00 - 2
                  IF(.NOT.(ndi.GE.0.AND.(N.LT.ndi.OR.N.GT.(ndi+1))))
     &               omd1(i,j,ie) = (2.*SPIdi(iiir,ie) + 1)/ESTep
                ENDDO
              ENDIF
            ENDDO
          ENDIF
            ...
            DO ie = 1, nudi1
              ...
              IF(ndi.EQ.( - 1))THEN
                ...
                DO i = 1, 10
                  ...
                  sumx = sumx + omd2(i,j,ie)
                ENDDO
                IF(sumx.NE.0.)THEN
                  DO i = 1, 10
                    omd2(i,j,ie) = omd2(i,j,ie)/(sumx*ESTep)
                  ENDDO
                ENDIF
              ELSE
                j = SPIdi(iiir,ie) - cjgs1 + 1.
                DO i = 1, 10
                  N = 2*i - IIA + m00 - 2
                  IF(.NOT.(ndi.GE.0.AND.(N.LT.ndi.OR.N.GT.(ndi+1))))
     &               omd2(i,j,ie) = (2.*SPIdi(iiir,ie) + 1)/ESTep
                ENDDO
              ENDIF
            ENDDO
          ENDIF
...
C
            DO i = 1, 25
              ...
              IF(ie.GT.nudi(IIP).OR.nexd(IIP,ie).LE.0)THEN
                ...
                a1 = 0.25*((ip - 1)**2 + (ih - 1)**2 + ip - 3*ih + 2)
                a2 = 0.25*((ip + 1)**2 + (ih + 1)**2 + ip - 3*ih - 2)
...
C                    ge = G*(e-dd(iip))
     ...
     &               *DLOG(ge - a0))
                  IF(cp.NE.0.)alp(i) = 0.5*(EXP((N+1)*DLOG(ge-a2) - (N-1
     &                                 )*DLOG(ge-a0)) - hlp3)
     &                                 /(cp*(N + 1))
                  IF(i.EQ.25.AND.OM(i + 1,ie).EQ.0.0D+0)alp(i) = 0.
                ENDIF
              ENDIF
              DO IJ = 1, 25
                cj = CJGs0 + IJ - 1
            ...
            DO IJ = 1, 25
              ...
              DO i = 2, 25
                N = 2*i - IIA + m00 - 1
                alms(i,IJ) = 0.
                IF(.NOT.(ie.LE.NUDim.AND.(il.GT.10.OR.i.GT.10)))THEN
                  IF(ie.GT.NUDim.OR.il.GT.10.OR.i.GT.10.OR.OMD(i,IJ,ie)
     &               .NE.0.)THEN
                    IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10.AND.N.GT.2)
     &                 alms(i,IJ) = alps(i - 1,IJ)*OMD(i - 1,IJ,ie)
     &                              /OMD(i,IJ,ie)
                    IF(.NOT.(ie.GT.NUDim.AND.(N.LE.2.OR.OM(i,ie).EQ.0.D0
     &                 )))alms(i,IJ) = alps(i - 1,IJ)*OM(i - 1,ie)
     &                                 *RFAc(N - 2,ijc)
     &                                 /(OM(i,ie)*RFAc(N,ijc))
                  ENDIF
                ENDIF
...
C--------------EMISSION RATES CALCULATIONS FOR GIVEN NUCLEI
     ...
     &               ro1 = OMD(i,il,ie)
                  IF(ie.LE.NUDim.AND.(il.GT.10.OR.i.GT.10))ro1 = 0.
                        ...
                        IF(ieom.GT.0)THEN
                          DO ieo = 1, ieom
                            ...
                            IF(eer.GE.( - estep2))THEN
                              endir = 0.
                              IF(nudim1.NE.0)endir = ENDi(iiir,nudim1)
                              IF(eer.GE.(endir + estep2))THEN
                                ier = INT((eer - endir)/ESTep + 0.5)
     &                                + nudim1
                                ro2 = 0.
                                IF(ier.GT.nudim1)ro2 = omr(j,i,ier)
...
C                                                  (EOM-EEO, SU)
   ...
   30                               l = INT(rl) + 1
                                    IF(l.LE.24)THEN
                                      sumx = sumx + TLP(j,ieo,l)
                                      rl = rl + 1.
                                      IF(rl.LE.(j + s))GOTO 30
                                    ENDIF
                                    s = s + 1.
                                    IF(s.LE.(su + 0.5))GOTO 25
                                    t11 = t11 + sumx*ro2*RFAc(N - 1,isu)
                                    IF(N.EQ.1)WRITE(iw,1050)j, iis, ieo, 
     &                                 eeo, t11, sumx, ro2
 1050                               FORMAT(' part-c  j,iis',i2,i3,
     &                                '  ieo,eeo',i2,f4.1,'  t11',e10.3,
     &                                '  sum',e10.3,'  ro2',e10.3)
                                  ENDDO
                                  CYCLE
                                ENDIF
                              ENDIF
                              IF(i.LE.10.AND.nudim1.NE.0)THEN
                                DO iid = 1, nudim1
...
C                                                     Emission (E, CJ) --->
C                                                     (EOM-EEO, SU)
                                      IF(CJGs0.EQ.0.D0)su = 0.5 + iis - 
     &                                  1
                                      IF(CJGs0.EQ.0.5D0)su = iis - 1
                                      isu = 2*su + 1
                                      IF(SPIdi(iiir,iid).NE.su)CYCLE
   ...
   35                                 l = INT(rl) + 1
     ...
     &                                    sumx*omd2(i,iis,iid)
                        ...
                        ENDIF
                      ENDIF
                      tau(i,il) = tau(i,il) + akfc*r(j,i,IIP)*t11
                    ENDDO
                  ENDIF
                  t11 = 0.
                  IF(ie.EQ.NEStep)THEN
                                             !Gamma cascade not allowed
                    IF(ie.GT.NBR)THEN
...
C                                   IER = IE-IGM
                          eer = e - egm
                          IF(eer.GE.( - estep2).AND.N.GT.0)THEN
                            t0001 = 0.
                            sgi = egm*egm*SG(igm)
     ...
     &                         sgi = sgi*sgi/(sgi + sgiq)
                            ...
                            DO is = 1, 3
                              ...
                              IF(su.GE.0.D0)THEN
                                ...
                                bm = 0.
                                b0 = 0.
                                IF(N.GE.3.AND.i.NE.1.AND.is.LE.3.AND.
     &                             ((N-2)*XX0(i-1,is,il) + geg*XP(is,il)
     &                             ).NE.0.)bm = geg*XP(is,il)
     &                             /((N - 2)*XX0(i - 1,is,il)
     &                             + geg*XP(is,il))
                                IF(is.LE.3.AND.
     &                             (N*XX0(i,is,il) + geg*XP(is,il))
     &                             .NE.0.)b0 = N*XX0(i,is,il)
     &                             /(N*XX0(i,is,il) + geg*XP(is,il))
 
                                bmq = 0.
                                IF(N.GE.1.AND.i.NE.1)
     &                             bmq = geg*XX22/((N - 2)
     &                             *XX02(i - 1,is,il) + geg*XX22)
                                b0q = N*XX02(i,is,il)
     &                                /(N*XX02(i,is,il) + geg*XX22)
                                endir = 0.
                                IF(NUDim.NE.0)endir = ENDi(IIP,NUDim)
                                IF(eer.LT.(endir + estep2))THEN
                                  ...
                                  IF(il.LE.10.AND.ilis.LE.10.AND.
     &                               ilis.GT.0.AND.i.LE.10.AND.
     &                               NUDim.NE.0)THEN
                                    DO iid = 1, NUDim
                                      IF(ABS(ENDi(IIP,iid) - eer)
     &                                  .LE.estep2.AND.SPIdi(IIP,iid)
     &                                  .EQ.su)THEN
     ...
     &                                    *OMD(i - 1,ilis,iid)
                                  ...
                                  ENDIF
                                ELSE
                                  iiir = INT((eer - endir)/ESTep + 0.5)
     &                              + NUDim
                                  IF(iiir.LT.1)CYCLE
                                  T1111 = b0*sgi*OM(i,iiir)*RFAc(N,isu)
     &                              + b0q*sgiq*OM(i,iiir)*RFAc(N,isu)
                                  IF(i.NE.1.AND.N.GT.2)T1111 = T1111 + 
     &                               sgi*OM(i - 1,iiir)*RFAc(N - 2,isu)
     &                               *bm + sgiq*OM(i - 1,iiir)
     &                               *RFAc(N - 2,isu)*bmq
                                ENDIF
                                t11 = t11 + T1111
                  ...
                  emg(i,il) = 0.
                  IF(ro1.NE.0.)THEN
                    IF(ie.GT.NUDim.AND.N.GT.0)emp(i,il) = tau(i,il)
     &                 /(ro1*RFAc(N,ijc))
                    IF(ie.GT.NUDim.AND.N.GT.0)emg(i,il)
     &                 = t11*C2/(ro1*RFAc(N,ijc))
                    IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10)emp(i,il)
     &                 = tau(i,il)/ro1
                    IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10)emg(i,il)
     &                 = t11*C2/ro1
                  ENDIF
                  t11 = emp(i,il) + emg(i,il) + alps(i,il) + alms(i,il)
                  IF(t11.NE.0.D0)tau(i,il) = 1./t11
                  IJ = i - 1
                  IF(il.EQ.ilma.AND.ie0.EQ.1.AND.II0.EQ.0)WRITE(iw,1060)
     &               ie, il, IJ, alps(i,il), alms(i,il), emp(i,il), 
     &               emg(i,il)
 1060             FORMAT(1X,3I4,2X,3E12.4,3X,E12.4)
...
C
            IF(ie0.EQ.1)WRITE(iw,1070)II0
 1070       FORMAT(/' Solutions for nucleus of key ',i2//3x,'E ',4x,'J',
     &             4x,'particle',4x,'gamma',4x,'population',
                ...
                IF(ie.GT.NUDim.OR.il.LE.10)THEN
                  ...
                  DO i = 1, nn
                    ...
                    IF(i.LE.10.AND.il.LE.10)THEN
                      IF(ie.LE.NUDim.AND.OMD(i,il,ie).NE.0.)nne = i
                    ENDIF
                  ENDDO
                  IF(nne.NE.0)THEN
                    nne1 = nne - 1
                    ...
                    DO i = 1, nn
                      tf(i) = 0.
                      IF(ie.GT.NUDim.AND.ie.NE.NEStep)tf(i) = tf0(i,il)
                      IF(ie.GT.NUDim.AND.II0.NE.0)tf(i) = tf(i)
     &                   + tm(II0,i,ie,il)
C                             IF (e.ge.14.1  .and.II0.NE.0)     TF(I) =
C                             TF(I) + TM(II0,I,IE,IL)
                      IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10.AND.
     &                   ie.NE.NEStep)tf(i) = tf0(i,il)
                      IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10.AND.
     &                   II0.NE.0)tf(i) = tf(i) + tm(II0,i,ie,il)
                      IF(ie.EQ.NEStep.AND.II0.EQ.0.AND.i.EQ.1)tf(1)
     &                   = tf(1) + pin0(il)
                      feed = feed + tf(i)
                    ENDDO
                    IF(ie.GT.NUDim.OR.nexd(IIP,ie).LT.0)THEN
                      IF(feed.LE.1.E-9)CYCLE
...
C--------------------------------THIS IF CAN BE DELETED
                        DO j = 1, nne1
                          i = nne - j
                          f(i) = alps(i,il)*tau(i,il)*alms(i + 1,il)
     &                           *tau(i + 1,il)
                          IF(f(i).EQ.1.)GOTO 40
...
C-----------------------------------BY CH.+G. ALGORITHM
                        ...
                        DO j = 1, nne1
                          ...
                          IF(ABS(fsci - 1.).LT.1.E-9)THEN
                            IF(DABS(alps(i,il)*tau(i,il) - 1.).LT.1.E-9)
     &                         THEN
                              sci(i) = 1.
                              EXIT
                            ENDIF
                        ...
                        ENDDO
                        IF(ie.NE.5.OR.il.NE.1)THEN
                        ENDIF
                      ENDIF
                      DO i = 1, nne
                        br = 0.
                        sumpro = 0.
                        IF(i.NE.1)br = br + alps(i - 1,il)*t(i - 1)
                        IF(nne1.NE.0)THEN
                          DO k = i + 1, nne
                            pro = 1.
                            DO m = i + 1, k
                              pro = pro*alms(m,il)*tau(m,il)*sci(m)
                            ENDDO
                    ...
                    ELSEIF(il.LE.10)THEN
                      DO i = 1, 10
                        IF(emg(i,il).NE.0.)TC(i,ie,il) = tf(i)/emg(i,il)
                      ENDDO
                    ENDIF
...
C--------------------------ONE EXC. ENERGY)
   40               emtg = 0.
                    ...
                    ENDDO
                    IF(ie.LE.NUDim)THEN
                      IF(nexd(IIP,ie).LE.0.AND.emtg.NE.feed.AND.
     &                   emtg.NE.0.)THEN
                        DO i = 1, 10
                          TC(i,ie,il) = TC(i,ie,il)*feed/emtg
                        ENDDO
                        GOTO 40
                      ENDIF
                    ENDIF
                    IF(feed.GE.1.E-3.AND.ie.GE.(NEStep - 1))THEN
...
C                             write(iw,51) e,cj,sigmacn(il),sigmacntot
     ...
     &                 WRITE(iw,1080)e, cj, emtp, emtg, feed, 
     &                               (TC(i,ie,il),i = 1,3)
 1080               FORMAT(1X,f5.2,1x,f4.1,1x,2G11.4,2x,g11.4,2x,3E9.2)
                  ENDIF
                ENDIF
...
Coblo                      do 4401 i=1,4    ! Excitons limited to i=1,4
                    ...
                    N = 2*i - IIA - 1 + m00
                    T1111 = 0.
                    IF(.NOT.((ie.GT.NUDim).AND.(N.LE.0.OR.OM(i,ie).EQ.0.
     &                 )))THEN
                      IF(.NOT.((ie.LE.NUDim).AND.(i.GT.10.OR.il.GT.10)))
     &                   THEN
                    ...
                    ENDIF
                    tf0(i,il) = T1111
                    Popul(1,ie,il) = Popul(1,ie,il) + tf0(i,il)
                  ENDDO
                ENDDO
                IF(ie.NE.1)GOTO 45
              ENDIF
...
CMH         DO iloblo = 1, 25
CMH            Popul(1, NEStep, iloblo) = sigmacn(iloblo)
CMH         ENDDO
          IF(NBR.LE.0)THEN
          ENDIF
C
...
C              de-excitation of discrete levels according to branchings
     ...
     &                 + tm(II0,i,ie,il)
            ...
            ENDDO
            WRITE(iw,1090)
 1090       FORMAT(//' Decay according to the branching ratios'//
     &          ' Level spin  populated (particle +cont.gam.+disc.gam.)'
     &          )
            DO ie0 = 1, NBR
              ie = nbr1 - ie0
              tf(ie) = feebra(ie) + feebr2(ie) + feepar(ie)
              WRITE(iw,1100)ENDi(IIP,ie), SPIdi(IIP,ie), tf(ie), 
     &                      feepar(ie), feebra(ie), feebr2(ie)
 1100         FORMAT(f6.3,f5.1,f10.3,'  (',e9.3,'+',e9.3,'+',e9.3,')')
              ie1 = ie - 1
              IF(ie1.NE.0)THEN
                DO i = 1, ie1
                  IF(brar(IIP,ie,i).NE.0)THEN
                    ...
                    WRITE(iw,1110)gae(1,i), gae(2,i)
 1110               FORMAT(23x,'Egam =',f6.3,' MeV,   c.s. =',g10.3,
     &                     ' mb')
                  ENDIF
                ENDDO
              ENDIF
...
C
                  DO j = 1, 3
                    ...
                    IF(ie.GT.NUDim)eom = EXCef + (ie - NUDim)*ESTep
...
C                                      ! Gamma cascade not allowed
     ...
     &                         sgi = sgi*sgi/(sgi + sgiq)
                            ...
                            DO is = 1, 3
                              ...
                              IF(su.GE.0.D0)THEN
                                ...
                                bm = 0.
                                b0 = 0.
                                IF(N.GT.2.AND.i.NE.1.AND.is.LE.3.AND.
     &                             ((N-2)*XX0(i-1,is,il) + geg*XP(is,il)
     &                             ).NE.0.)bm = geg*XP(is,il)
     &                             /((N - 2)*XX0(i - 1,is,il)
     &                             + geg*XP(is,il))
     ...
     &                                /(N*XX02(i,is,il) + geg*XX22)
                                ...
                                IF(eer.LT.(endir + estep2))THEN
                                  ...
                                  IF(is.EQ.4)ilis = il - 2
                                  IF(is.EQ.5)ilis = il + 2
                                  IF(il.LE.10.AND.NUDim.NE.0.AND.
     &                               i.LE.10.AND.ilis.LE.10.AND.
     &                               ilis.GT.0)THEN
                                    DO iid = 1, NUDim
                                      IF(ABS(ENDi(IIP,iid) - eer)
     &                                  .LE.estep2.AND.SPIdi(IIP,iid)
...
C----------------------------------------Emission(E, CJ)--->(EOM-EEO, SU)
   ...
   55                               l = INT(rl) + 1
                                    IF(l.LE.24)THEN
                                      sumx = sumx + TLP(j,ieo,l)
                                      rl = rl + 1.
                                      IF(rl.LE.(j + s))GOTO 55
                                    ENDIF
                                    s = s + 1.
                                    IF(s.LE.(su + 0.5))GOTO 50
                                    t111 = t00*sumx*ro2*RFAc(N - 1,isu)
     &                                *r(j,i,IIP)*akfc/ro1
                                    t11 = t11 + t111
                                    ii1 = II0 + IIA + j
                                    IF(II0.LE.5.AND.j.LE.2.AND.ii1.LE.9)
     &                                 tm(ii1,i,ier,iis)
...
C----------------------------------------Emission(E, CJ)--->(EOM-EEO, SU)
   ...
   60                                 l = INT(rl) + 1
                                      ...
                                      ELSE
                                        IF(j.EQ.1)
     &                                   t111 = t00*sumx*omd1(i,iis,iid)
     &                                    *r(j,i,IIP)*akfc/ro1
                                        IF(j.EQ.2)
     &                                   t111 = t00*sumx*omd2(i,iis,iid)
     &                                    *r(j,i,IIP)*akfc/ro1
                                        ii1 = II0 + IIA + j
     ...
     &                                    = tm(ii1,i,iid,iis) + t111
                            ...
                            ENDIF
                          ENDIF
   65                     Se(j,ieo) = Se(j,ieo) + t11/ESTep
                        ENDIF
                      ENDDO
                    ENDIF
                  ENDDO
...
C
          WRITE(iw,1120)II0
 1120     FORMAT(/' SPECTRA (in mb/MeV) from nucleus of key ',
     &           i2/'  Energy',7X,'neutrons',8X,'protons',9X,'gammas')
          DO ieo = 1, nesteu
            eeo = ESTep*ieo
            DO j = 1, 3
            ...
            ENDDO
            sumx = Se(1,ieo) + Se(2,ieo) + Se(3,ieo)
            IF(sumx.NE.0.)WRITE(iw,1130)eeo, (Se(j,ieo),j = 1,3)
 1130       FORMAT(1X,F8.3,3X,3G15.5)
          ENDDO
          WRITE(iw,1140)(Se(j,nestem),j = 1,3)
 1140     FORMAT(' Integrals  ',3G15.5,' mb'//)
        ENDIF
      ELSE
        ESTep = eexc/NDExd
        GOTO 10
...
C COMMON variables
C
      REAL*8 :: AF, C2, CJGs0, ESTep, EXCef, G, GAQ, SIGma, T1111, XX2, 
     &          XX22
      REAL*8, DIMENSION(10,10) :: ENDi, SPIdi
      INTEGER :: I0, IAT, IE0, II0, IIA, IIP, IJ, IL0, IZ, N, NBR, 
     &           NEStep, NUDim
      REAL*8, DIMENSION(25,NDExd) :: OM
      REAL*8, DIMENSION(10,10,10) :: OMD
      REAL*8, DIMENSION(50,90) :: RFAc
      REAL*8, DIMENSION(NDExd) :: SG
      REAL*8, DIMENSION(25,NDExd,25) :: T0
      REAL*8, DIMENSION(3,25) :: XP
      REAL*8, DIMENSION(25,3,25) :: XX0
      REAL*8, DIMENSION(25,5,25) :: XX02
      COMMON /BUF   / IIA, IIP, II0
...
Coblo DO 20 IEG=IE1,NESTEP
      DO ieg = NEStep, NEStep          ! Gamma cascade not allowed
        ...
        IF(igm.NE.0)THEN
          ...
          DO is = 1, 3
            ...
            IF(is.EQ.4)ilp = il - 2
            IF(is.EQ.5)ilp = il + 2
            IF(s.GE.0..AND.s.LT.24..AND.ilp.LE.25.AND.ilp.GT.0)THEN
              isu = 2*s + 1
              IF(.NOT.(ieg.LE.NUDim.AND.(ilp.GT.10.OR.i.GT.10)))THEN
                sgi = 0.
                sgiq = 0.
                IF(XX0(i,is,ilp).NE.0..OR.XP(is,ilp).NE.0.)THEN
     ...
     &               /(N*XX02(i,is,ilp) + geg*XX22)
                  ...
                  sgiq = sgir
                  IF(N.GT.0)THEN
                    IF(ieg.GT.NUDim.AND.(OM(i,ieg)*RFAc(N,ijc).NE.0.)
     &                 .AND.is.LE.3)T1111 = T1111 + T0(i,ieg,ilp)
     &                 *sgi*N*XX0(i,is,ilp)/(OM(i,ieg)*RFAc(N,isu))
                    IF(ieg.GT.NUDim.AND.(OM(i,ieg)*RFAc(N,ijc).NE.0.))
     &                 T1111 = T1111 + T0(i,ieg,ilp)
     &                         *sgiq*N*XX02(i,is,ilp)
     &                         /(OM(i,ieg)*RFAc(N,isu))
                    IF(ieg.GT.NUDim.AND.i.EQ.25)CYCLE
                  ENDIF
                  IF(ieg.GT.NUDim.AND.OM(i + 1,ieg).NE.0.D0.AND.
     &               is.LE.3.AND.(i + 1).LE.25)T1111 = T1111 + 
     &               T0(i + 1,ieg,ilp)*geg*sgi*XP(is,ilp)
     &               /(OM(i + 1,ieg)*RFAc(N + 2,isu))
 
                  IF(ieg.LE.NUDim.AND.OMD(i,ilp,ieg).NE.0..AND.is.LE.3)
     &               T1111 = T1111 + T0(i,ieg,ilp)*sgi*N*XX0(i,is,ilp)
     &                       /OMD(i,ilp,ieg)
                  IF(ieg.GT.NUDim.AND.OM(i + 1,ieg).NE.0.D0.AND.(i + 1)
     &               .LE.25)T1111 = T1111 + T0(i + 1,ieg,ilp)
     &                              *geg*sgiq*XX22/(OM(i + 1,ieg)
     &                              *RFAc(N + 2,isu))
                  IF(ieg.LE.NUDim.AND.OMD(i,ilp,ieg).NE.0.)
     &               T1111 = T1111 + T0(i,ieg,ilp)*sgiq*N*XX02(i,is,ilp)
     &               /OMD(i,ilp,ieg)
                  IF(i.NE.10)THEN
                    IF(ieg.LE.NUDim.AND.OMD(i + 1,ilp,ieg).NE.0.D0.AND.
     &                 is.LE.3.AND.(i + 1).LE.25)T1111 = T1111 + 
     &                 T0(i + 1,ieg,ilp)*geg*sgi*XP(is,ilp)
     &                 /OMD(i + 1,ilp,ieg)
                    IF(ieg.LE.NUDim.AND.OMD(i + 1,ilp,ieg).NE.0.D0)
     ...
     &                         *geg*sgiq*XX22/OMD(i + 1,ilp,ieg)
        ...
        ENDIF
      ENDDO
      IF(ie.GT.NUDim.AND.N.GT.0.AND.OM(i,ie).EQ.0.)T1111 = 0.
      IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10.AND.OMD(i,il,ie).EQ.0.)
     &   T1111 = 0.
      IF(ie.GT.NUDim.AND.N.GT.0.AND.OM(i,ie).NE.0.)
     &   T1111 = T1111*OM(i,ie)*C2*RFAc(N,ijc)
      IF(ie.LE.NUDim.AND.il.LE.10.AND.i.LE.10.AND.OMD(i,il,ie).NE.0.)
     &   T1111 = T1111*OMD(i,il,ie)*C2
      END SUBROUTINE GAMMD
...
C Local variables
C
      REAL*8 :: a, a23, a3, ala, am, amu, anu, d, del, e0, e1, e2, ec, 
     &          ec2, egr, em, gam, p, q, r, sgm, sig, xi, z
      REAL :: FLOAT
C
C*** End of declarations rewritten by SPAG
C
...
C-----------NEUTRONS
        ...
        ENDIF
    5   sig = p*(En - xi)**2 + ala*em + amu + anu*(2. - En/xi)/xi
        IF(En.LT.e0)sig = 0.
      ENDIF
      CHAGUG = sig
      END FUNCTION CHAGUG
...
!---------------------------------------------------------------------------
C
      FUNCTION CLEBSH(A,A1,B,B1,C,C1)
C
C*** Start of declarations rewritten by SPAG
C
C COMMON variables
...
C
      REAL*8 :: DABS
      INTEGER :: i, iii, j, j1, j10, j2, j3, j4, j5, j6, j7, j8, j9, 
     &           kh1, kh2, kh3, kl1, kl2, kmax, kmin
      INTEGER :: INT, MAX0, MIN0
      REAL*8 :: ph, qq, su, sumx, x
C
C*** End of declarations rewritten by SPAG
C
                      ...
                      IF(x.LE.0.001D0)THEN
                        ...
                        IF(x.LE.0.001D0)THEN
                          IF((A1 + B1).EQ.C1)THEN
                            ...
                            j10 = INT(B + B1) + 3
                            qq = SQRT((2*C + 1.)
     &                           *EXP(F(j1) + F(j2) + F(j3) + F(j4)
     &                           + F(j5) - F(j6) - F(j7) - F(j8) - F(j9)
     &                           - F(j10)))
                            sumx = 0.
                            DO iii = kmin, kmax
                              i = iii - 1
                              ...
                              j5 = i + 3
                              j6 = i - kl2 + 3
                              su = ph*EXP(F(j1) + F(j2) - F(j3) - F(j4)
     &                             - F(j5) - F(j6))
                              sumx = sumx + su
                              IF(DABS(sumx/su).LT.1.D-8)GOTO 10
                            ENDDO
                            CLEBSH = qq*sumx
                            RETURN
                          ENDIF
...
C
      ...
      if(TRI(A,B,C)   .eq. 0.D0) return
      if(TRI(C,A1,B1) .eq. 0.D0) return
      if(TRI(A1,B,C1) .eq. 0.D0) return
      if(TRI(A,C1,B1) .eq. 0.D0) return
     ...
     &   - F(j6) - F(j7) - F(j8))
         sumx = sumx + su
         IF(DABS(sumx/su).LT.1.D-8) return
      ENDDO

      qq = DEL(A,B,C)*DEL(A,B1,C1)*DEL(A1,B,C1)*DEL(A1,B1,C)
      SIXJ = sumx*qq
...
!---------------------------------------------------------------------------
      ...
      PURE REAL*8 FUNCTION TRI(A,B,C)
...
C
C
      FUNCTION THREEJ(A,A1,B,B1,C,C1)
C
C*** Start of declarations rewritten by SPAG
C
C Dummy arguments
...
C COMMON variables
C
      REAL*8 :: AF, C2, CJGs0, ESTep, EXCef, G, GAQ, JGS, SIGma, SPP, 
     &          T1111
      REAL*8, DIMENSION(10,10) :: ENDi, SPIdi
      INTEGER :: I0, IAO, IAT, IE00, IJ, IL0, ITJgs, ITSpp, IZ, IZO, 
     &           KEY, NBR, NEStep, NN, NUDim
      REAL*8, DIMENSION(25,NDExd) :: OM
      REAL*8, DIMENSION(10,10,10) :: OMD
      REAL*8, DIMENSION(50,90) :: RFAc
      REAL*8, DIMENSION(NDExd) :: SG
...
C Local variables
C
      REAL*8 :: a, b, c, d, j1, j2, j3, sumx, sumc
...
C
   ...
   25   ij3 = 2*j3 + 1
C
        IF(ij3.LE.90)d = RFAc(NN - 1,ij3)*(SIXJ(1.D0,j2,j1,j3,J,S)**2)
     &                   + d
        j3 = j3 + 1.
        IF(j3.LE.(j2 + S))GOTO 25
      ENDIF
      c = c*d
      sumc = sumc + c
      j1 = j1 + 1.
      IF(j1.LE.(j2 + 1))GOTO 20
      b = b*sumc
      sumx = sumx + b
      j2 = j2 + 1.
      IF(j2.LE.24.5)GOTO 10
      X0 = sumx*a
99999 END FUNCTION X0
...
C COMMON variables
C
      REAL*8 :: AF, C2, CJGs0, ESTep, EXCef, G, GAQ, SIGma, T1111
      REAL*8, DIMENSION(10,10) :: ENDi, SPIdi
...
C Local variables
C
      REAL*8 :: a, b, c, j1, j2, l, sumx, sumc
      INTEGER :: ij1, ij2
      REAL*8 :: THREEJ, TRI
C
C*** End of declarations rewritten by SPAG
...
C-----Eq. (17) of the Report
   ...
   20 ij2 = 2*j2 + 1
      ...
      sumc = sumc + c
      j2 = j2 + 1.
      IF(j2.LE.(j1 + l))GOTO 20
      b = b*sumc
      sumx = sumx + b
      j1 = j1 + 1.
      IF(j1.LE.24.5)GOTO 10
      X2 = a*sumx*(2*J + 1.)/(2*S + 1.)
99999 END FUNCTION X2
...
C Local variables
C
      REAL*8 :: a, j4, q, q1, r, r1, rn, sumx, sumb
...
C           J integer  =>  j4=1/2,  Q= J-1/2, J+1/2
          ...
          r1 = RFAc(1,ijj)
          sumx = FFQ(iq1)*r1/(r + r1)
          IF(q.GE.0.)sumx = sumx + FFQ(iq)*r/(r + r1)
        ELSE
C           J half-integer  =>  j4=0,  Q=J
   ...
   20 ijq = 2*q + 1
      ...
      q = q + 1.
      iq = INT(q + 0.5)
      IF(q.LE.(j4 + J).AND.q.LE.20.)GOTO 20
      sumx = sumx + sumb*a
      j4 = j4 + 1.
      IF(j4.LE.24.5)GOTO 10
      XDAMP = sumx
99999 END FUNCTION XDAMP
...
C
   ...
   10 ij3 = INT(j3 + 1.)
      ...
      j5 = ABS(j3 - Q)
      sumb = 0.
   20 b = THREEJ(j5,.5D0,Q, - .5D0,j3,.0D0)**2
      ij5 = 2*j5 + 1
      sumb = sumb + ij5*RFAc(1,ij5)*b
      j5 = j5 + 1.
      IF(j5.LE.(j3 + Q).AND.j5.LE.24.5)GOTO 20
      sumx = sumx + sumb*a
      j3 = j3 + 1.
      ij3 = INT(j3 + 1.)
      IF(j3.LE.24.5)GOTO 10
      FQ = sumx
      END FUNCTION FQ
...
C-----Eq. (12) of the Report
   ...
   10 ij1 = 2*j1 + 1
      ...
      j2 = ABS(j1 - J3)
      sumb = 0.0
   20 b = THREEJ(j1,.5D0,j2, - .5D0,J3,.0D0)**2
      ij2 = 2*j2 + 1
      sumb = sumb + ij2*RFAc(1,ij2)*b
      j2 = j2 + 1.
      IF(j2.LE.(j1 + J3).AND.j2.LE.24.5)GOTO 20
      sumx = sumx + sumb*a
      j1 = j1 + 1.
      IF(j1.LE.24.5)GOTO 10
      FPAIR = sumx
...
C Local variables
C
      REAL*8 :: a, al, al1, an, an2, at, at3, cl, d, dp, elab, fac, r, 
     &          r0, rat, se, v0, vc, vn, zt
      REAL*8 :: DF, F
      REAL :: FLOAT
      INTEGER :: i
C
...
C-----Analog of Eq. (16) of the Report for E2
   ...
   20 ij1 = 2*j1 + 1
      ...
      ELSE
        j3 = ABS(j2 - S)
   25   ij3 = 2*j3 + 1
        IF(ij3.LE.90)d = RFAc(NN - 1,ij3)*(SIXJ(2.D0,j2,j1,j3,J,S)**2)
     &                   + d
        j3 = j3 + 1.
        IF(j3.LE.(j2 + S))GOTO 25
      ENDIF
...
C-----Analog of  Eq. (17) of the Report for E2
   ...
   20 ij2 = 2*j2 + 1
      ...
      j1 = j1 + 1.
      IF(j1.LE.24.5)GOTO 10
      X22 = a*sumx*(2*J + 1.)/(2*S + 1.)
99999 END FUNCTION X22
