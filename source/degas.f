         ...
         IF (IDNa(idnacont,4).EQ.1) THEN
            ...
            DO ie0 = ie1, ie1 + NEX(nnur) - 1 ! Excitation energy bin
               ...
               IF (nejc.EQ.1) THEN !neutron spectra are shifted by 1
                  ...
                  AUSpec(ispec + 1,nejc) = AUSpec(ispec + 1,nejc)
     &               + sumtmp
               ELSE
...
C
     ...
     &       EEXc0, ENDi(10,10), ESTep, GAQ, GG(10), JGS, SPIdi(10,10), 
     &       XC(9)
...
C     (FMT 1X,79H)   TITLE CARD OF THE REACTION
      READ (ir,99010) IN, IZ, KEY0
99010 FORMAT (4I5,f10.3)
...
C     IN+IZ = 0 ==> STOP OF THE PROGRAM
      ...
      READ (ir,99015) EEXc0, ESTep, AK, JGS, CJGs, BEProj, GAQ
99015 FORMAT (8F8.2)
...
C-----(LIMIT: .LE. NUDI)
      DO j = 1, 10
         ...
         IF (nudim.NE.0) THEN
            DO i = 1, nudim
               READ (ir,99020) ENDi(j,i), SPIdi(j,i), NEXd(j,i)
99020          FORMAT (2F6.2,i6)
...
C
               IF (i.LE.NBRa(j)) READ (ir,99025)
     &                                 (NOLe(ii),XC(ii),ii = 1,9)
99025          FORMAT (9(i2,f6.3))
C              IF (I.LE.NBRA)
...
C
      ...
      WRITE (iw,99010) IN, IZ, KEY0
99010 FORMAT (4I5,f10.3)
      WRITE (iw,99015) EEXc0, ESTep, AK, JGS, CJGs, BEProj, GAQ
99015 FORMAT (8F8.2)
      WRITE (iw,99035) B   ! binding energies
      WRITE (iw,99035) GG  ! single-particle level density g
      WRITE (iw,99035) DD  ! pairing energies
      WRITE (iw,99040) NUDi
      ...
      DO j = 1, 10
         ...
         IF (nudim.NE.0) THEN
            DO i = 1, nudim ! i=1 means g.s. level, etc
               WRITE (iw,99020) ENDi(j,i), SPIdi(j,i), NEXd(j,i)
99020          FORMAT (2F6.2,i6)
               IF (i.LE.NBRa(j)) THEN
                  DO IE = 1, 9 ! ie=1 means deay goes to g.s., etc
                     bbb = BRAr(j,i,IE)
                     IF (IE.GT.0 .AND. bbb.GT.0.) WRITE (iw,99025) IE, 
     &                   bbb
99025                FORMAT (9(i2,f6.3))
                  ENDDO
               ENDIF
...
C        REACTION C.S. AND FEEDING OF COMPOUND SPIN CALCULATION
         ...
         IF (key0.GE.3 .AND. key0.NE.5) READ (ir,99010) TL
99010    FORMAT (10F6.2)
...
99015    FORMAT (/' A = ',I3,'    Eexc = ',F5.2,
     ...
     &           ' MeV**3'/' GDR param:    E = ',F4.1,
     &           ' MeV     width = ',F4.2,' MeV     peak c.s. = ',F5.0,
     &           ' mb'/
     &           ' Single-particle level dens. (1/MeV) and binding',
                  ...
                  IF (j.LE.10) THEN
                     IF (ndi.EQ.( - 1)) THEN
                        ...
                        IF (j.LE.10) THEN
                           ...
                           IF (sum.NE.0.) THEN
                              DO i = 1, 10
                                 OMD(i,j,ie) = OMD(i,j,ie)/(sum*ESTep)
                              ENDDO
                           ENDIF
                        ELSE
                           WRITE (iw,99020) II0, IIP, ie, CJGs0, 
     &                            SPIdi(IIP,ie), j
99020                      FORMAT ('spiny   ii0,iip',2I3,'  ie',i3,
            ...
            IF (nudi1.NE.0) THEN
               DO ie = 1, nudi1
                  ...
                  ELSE
                     ...
                     DO i = 1, 10
                        N = 2*i - IIA + m00 - 2
                        IF (.NOT.(ndi.GE.0 .AND. (N.LT.ndi.OR.N.GT.(ndi+
     &                      1)))) omd1(i,j,ie) = (2.*SPIdi(iiir,ie) + 1)
     &                      /ESTep
                     ENDDO
                  ENDIF
               ENDDO
               ...
               DO ie = 1, nudi1
                  ...
                  ELSE
                     ...
                     DO i = 1, 10
                        N = 2*i - IIA + m00 - 2
                        IF (.NOT.(ndi.GE.0 .AND. (N.LT.ndi.OR.N.GT.(ndi+
     &                      1)))) omd2(i,j,ie) = (2.*SPIdi(iiir,ie) + 1)
     &                      /ESTep
                     ENDDO
                  ENDIF
               ENDDO
...
C                    ge = G*(e-dd(iip))
     ...
     &                      *DLOG(ge-a0)) - hlp3)/(cp*(N + 1))
                        IF (i.EQ.25 .AND. OM(i + 1,ie).EQ.0.0D+0) alp(i)
     &                      = 0.
                     ENDIF
                  ENDIF
                  DO IJ = 1, 25
  ...
  160          ENDDO
     ...
     &                         *RFAc(N - 2,ijc)/(OM(i,ie)*RFAc(N,ijc))
                        ENDIF
                     ENDIF
                  ENDDO
               ENDDO
...
C                                                     (EOM-EEO, SU)
  ...
  170                               ENDDO
                                 ENDIF
                              ENDIF
                              tau(i,il) = tau(i,il) + akfc*r(j,i,IIP)
     &                           *t11
                           ENDDO
                        ENDIF
                        t11 = 0.
...
C                                   IER = IE-IGM
  ...
  172                                  ENDDO
     ...
     &                      WRITE (iw,99030) ie, il, IJ, alps(i,il), 
     &                             alms(i,il), emp(i,il), emg(i,il)
99030                   FORMAT (1X,3I4,2X,3E12.4,3X,E12.4)
...
C
               IF (ie0.EQ.1) WRITE (iw,99035) II0
99035          FORMAT (/' Solutions for nucleus of key ',i2//3x,'E ',4x,
     &                 'J',4x,'particle',4x,'gamma',4x,'population',
...
C              de-excitation of discrete levels according to branchings
     ...
     &                         + tm(II0,i,ie,il)
               ...
               ENDDO
               WRITE (iw,99045)
99045          FORMAT (//' Decay according to the branching ratios'//
     &          ' Level spin  populated (particle +cont.gam.+disc.gam.)'
     &          )
               DO ie0 = 1, NBR
                  ie = nbr1 - ie0
                  tf(ie) = feebra(ie) + feebr2(ie) + feepar(ie)
                  WRITE (iw,99050) ENDi(IIP,ie), SPIdi(IIP,ie), tf(ie), 
     &                             feepar(ie), feebra(ie), feebr2(ie)
99050             FORMAT (f6.3,f5.1,f10.3,'  (',e9.3,'+',e9.3,'+',e9.3,
     &                    ')')
                  ie1 = ie - 1
                  IF (ie1.NE.0) THEN
                     DO i = 1, ie1
                        IF (brar(IIP,ie,i).NE.0) THEN
                           ...
                           WRITE (iw,99055) gae(1,i), gae(2,i)
99055                      FORMAT (23x,'Egam =',f6.3,' MeV,   c.s. =',
     &                             g10.3,' mb')
                        ENDIF
                     ENDDO
                  ENDIF
                           ...
                           IF (eom.GE.estep2 .AND. ieom.GT.0) THEN
                              DO ieo = 1, ieom
                                 ...
                                 IF (eer.GE.( - estep2)) THEN
                                    IF (j.EQ.3) THEN
                                       geg = G*eeo
                                       IF (ie.NE.NEStep) GOTO 198
C                                      ! Gamma cascade not allowed
                                       IF (ie.LE.NBR) GOTO 198
...
C----------------------------------------Emission(E, CJ)--->(EOM-EEO, SU)
  ...
  192                                    ENDDO
                                       ...
                                       ENDIF
                                    ENDIF
  194                               Se(j,ieo) = Se(j,ieo) + t11/ESTep
                                 ENDIF
  196                         ENDDO
                           ENDIF
  198                   ENDDO
...
C
            WRITE (iw,99060) II0
99060       FORMAT (/' SPECTRA (in mb/MeV) from nucleus of key ',
     ...
     &              'gammas')
            DO ieo = 1, nesteu
               ...
               IF (sum.NE.0.) WRITE (iw,99065) eeo, (Se(j,ieo),j = 1,3)
99065          FORMAT (1X,F8.3,3X,3G15.5)
            ENDDO
            WRITE (iw,99070) (Se(j,nestem),j = 1,3)
99070       FORMAT (' Integrals  ',3G15.5,' mb'//)
         ENDIF
      ELSE
         ESTep = eexc/NDEXD
         GOTO 100
...
Coblo DO 20 IEG=IE1,NESTEP
      DO ieg = NEStep, NEStep          ! Gamma cascade not allowed
         ...
         IF (igm.NE.0) THEN
            ...
            DO is = 1, 3
               ...
               IF (is.EQ.4) ilp = il - 2
               IF (is.EQ.5) ilp = il + 2
               IF (s.GE.0. .AND. s.LT.24. .AND. ilp.LE.25 .AND. 
     &             ilp.GT.0) THEN
                  isu = 2*s + 1
     ...
     &                      sgi = sgi*sgi/(sgi + sgiq)
                        ...
                        IF (N.GT.0) THEN
                           IF (ieg.GT.NUDim .AND. 
     &                         (OM(i,ieg)*RFAc(N,ijc).NE.0.) .AND. 
     &                         is.LE.3) T1111 = T1111 + T0(i,ieg,ilp)
     &                         *sgi*N*XX0(i,is,ilp)
     &                         /(OM(i,ieg)*RFAc(N,isu))
                           IF (ieg.GT.NUDim .AND. 
     &                         (OM(i,ieg)*RFAc(N,ijc).NE.0.))
     &                         T1111 = T1111 + T0(i,ieg,ilp)
     &                         *sgiq*N*XX02(i,is,ilp)
     &                         /(OM(i,ieg)*RFAc(N,isu))
                           IF (ieg.GT.NUDim .AND. i.EQ.25) GOTO 20
                        ENDIF
                        IF (ieg.GT.NUDim .AND. OM(i + 1,ieg)
     &                      .NE.0.D0 .AND. is.LE.3 .AND. (i + 1).LE.25)
     &                      T1111 = T1111 + T0(i + 1,ieg,ilp)
     &                              *geg*sgi*XP(is,ilp)
     &                              /(OM(i + 1,ieg)*RFAc(N + 2,isu))
 
                        IF (ieg.LE.NUDim .AND. OMD(i,ilp,ieg)
     &                      .NE.0. .AND. is.LE.3) T1111 = T1111 + 
     &                      T0(i,ieg,ilp)*sgi*N*XX0(i,is,ilp)
     &                      /OMD(i,ilp,ieg)
                        IF (ieg.GT.NUDim .AND. OM(i + 1,ieg)
     &                      .NE.0.D0 .AND. (i + 1).LE.25)
     &                      T1111 = T1111 + T0(i + 1,ieg,ilp)
   ...
   20       ENDDO
      ...
      IF (ie.LE.NUDim .AND. il.LE.10 .AND. i.LE.10 .AND. OMD(i,il,ie)
     &    .EQ.0.) T1111 = 0.
      IF (ie.GT.NUDim .AND. N.GT.0 .AND. OM(i,ie).NE.0.)
     &    T1111 = T1111*OM(i,ie)*C2*RFAc(N,ijc)
      IF (ie.LE.NUDim .AND. il.LE.10 .AND. i.LE.10 .AND. OMD(i,il,ie)
     &    .NE.0.) T1111 = T1111*OMD(i,il,ie)*C2
      END
C
...
C Local variables
C
      REAL*8 a, b, c, d, j1, j2, j3, sum, sumc
...
C
     ...
     &       FFQ(40), G, GAQ, JGS, OM(25,NDEXD), OMD(10,10,10), 
     &       RFAc(50,90), SG(NDEXD), SIGma, SPIdi(10,10), SPP, T1111, 
     &       TC(25,NDEXD,25)
      INTEGER I0, IAO, IAT, IE00, IJ, IL0, ITJgs, ITSpp, IZ, IZO, KEY, 
     &        N, NBR, NEStep, NUDim
      COMMON /DAM   / FFQ, FFPair
...
C COMMON variables
C
      REAL*8 EF, FAL(140), JGS, SPP, TL(25)
      INTEGER IAC, IAO, ITJgs, ITSpp, IZC, IZO, KEY
      COMMON  FAL, EF, IAC, IZC
      COMMON /MC    / SPP, JGS, IAO, IZO, KEY, ITSpp, ITJgs
      COMMON /TRA   / TL
...
C     if (KEY.EQ.1)  VC= 0.
     ...
     &                    - 37.55/se,0.D0)
      DO i = 1, 25
         ...
         IF (KEY.NE.1 .AND. al1.GT.cl) TL(i) = an*EXP( - (al1 - cl)/dp)
     &       /2.
      ENDDO
      END
C
