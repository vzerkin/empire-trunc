...
Ccc   * $Id: scnd-preeq.f,v 1.1.1.1 2001-07-09 17:33:39 mike Exp $
C
...
C
     ...
     &                 popsub(NDEX, NDLW, 2), ratioro, sum, sumem, welld
      REAL FLOAT, REAL
      INTEGER i, icse, ie, iec, ier, iermax, ietl, iexc, ip, itlc, j, 
     &        jc, nh, np
      INTEGER MAX0
...
C-----calculation of factorial      ***** done *******
         ...
         itlc = iexc - 5
         B = 1000.
C        !ignore binding condition in level densities
         bind = Q(Nejc, Nnuc)
         G = A(Nnuc)/GDIv
         np = 1
C        ! assume 1p-1h configuration in the first residue
         nh = 1
C        ! assume 1p-1h configuration in the first residue
         welld = 35.0
C        ! potential well depth (for p-h state densiies)
...
C-----------------probability *** done ***
                     ...
                     POP(iermax, jc, ip, Nnur)
     &                  = POP(iermax, jc, ip, Nnur) + pop1
...
C-----------------probability *** done ***
                     ...
                     POP(ier, jc, ip, Nnur) = POP(ier, jc, ip, Nnur)
     &                  + pop1
                     CSE(icse, Nejc, Nnuc) = CSE(icse, Nejc, Nnuc)
     &                  + pop1
