...
Ccc   * $Id: MSD-orion2.f,v 1.1.1.1 2001-07-09 17:33:38 mike Exp $
C
...
C
     ...
     &        KEXcom(50), KEXcom1(28), KTLout(50), KTLout1(28), KTRl(30)
     &        , KTRl1(28), l9(9), LBTrf(4), lcaltr, LDWmxr(4), 
...
C
            ...
            RZEcr(n) = rc
C
            IF(EXTcom(5).GT.0.0D0 .AND. n.EQ.2)RZEr(n) = EXTcom(5)
     &         *RZEr(n)
            IF(EXTcom(6).GT.0.0D0 .AND. n.EQ.3)RZEr(n) = EXTcom(6)
     &         *RZEr(n)
            IF(EXTcom(7).GT.0.0D0 .AND. n.EQ.1)VSOr(n) = EXTcom(7)
     &         *VSOr(n)
C
         ENDDO
C
...
99002    FORMAT(24X, 10('*'), 6X, 
     ...
     &          //29X, '(ON PROGRAM ORION2, Munich/Giessen 1994)', 15X, 
     &          '(ELAB=', F6.3, ')')
      ENDIF
C
      IF(Iout.GT.3)THEN
...
99009    FORMAT(' KTRL  ', 28I4)
         WRITE(6, 99010)KEXcom1
99010    FORMAT(' KEXCOM', 28I4)
         WRITE(6, 99011)KTLout1
...
c Dummy arguments
c
      DOUBLE PRECISION Ad , Ai , As , Atar , Av , Aw , E , Rc , Rd , 
...
C
     ...
     &                 wr, z1, z2, z3, zl
      ...
      INTEGER i, inc, inci, irol, j, j2, l, l1, l2, lf, ll, lmx, n, n1, 
     &        ninc, nstep
      INTEGER MIN0
C
C
...
C                       THAT: 18 EXSG(L1)=EXSGO=EXSGO*CMPLX(ZL/Z3,ETA/Z3)
C
                     ENDIF
...
C
     ...
     &                 xbarsi, xbarsp, xbarw, xbfac, xmem(MAXX)
      INTEGER k, n, nx
C
C
C
...
C
C        do nn=1,NXMAX
C        write(30,3000)XMEM(nn),(VINTR(nn,i),VINTI(nn,i),i=1,JLSMAX)
C        enddo
...
C
CBF   DIMENSION DRSQD(3),REACXS(3),RCONST(3),ISTP(3)
C
...
C
                  ...
                  ENDDO
               ENDDO
               IF(KTLout(3).NE.0)WRITE(6, 99002)na, nc, 
     &                                 (CMAt1(mc, na), mc = ncmin, nc)
...
C
     ...
     &        llmax, lm10ps, lm20ps, lmm, lmp, lmpos, mm, mm1, n, n1mx, 
     &        n1wx, n2mx, na, nanglx, nb, njl1, njl1mx, nlr, nr, nr1, 
     &        nw1, nw2
      INTEGER MAX0, MIN0
...
C Local variables
C
      DOUBLE PRECISION fms, ph, t1, xconst
      INTEGER IABS, MIN0
      INTEGER ii, iimax, iimin, is3ke3, ji, jl, jla, jlamx, jlb, jlbmi, 
     &        jlbmx, jlmod, jlmttl, jlr, jlrg, jltohf, jx, kaspar, ke3, 
     &        l1p1, l1p1mx, l2p1, l2p1mx, lap1mx, lbp1mx, m, mm, mmmax, 
     &        mrg, ms, n, n1, n1mx, n2, na, namp, nampmi, nampmx, njl1, 
     &        njl1mx, njl2, njl2mx, nlr, nrun, nrun0
...
C
                        nampmx = namp*NANglr
                        IF(nampmx.GT.8300)THEN
                           WRITE(6, *)'In ORION dimension of XAMP must '
     &                                , 'be at least ', nampmx
                           STOP
                        ENDIF
                        DO namp = 1, nampmx
...
C
C
CB                      OUTPUT OF SGMA AND SGMAT IF KTLOUT(6)=1
C
                        IF(KTLout(6).NE.0)THEN
C
                           IF(L1Tr*L2Tr.NE.L1Maxm*L2Maxm)THEN
                              IF(L1Tr.GT.3)GOTO 10
                              IF(L2Tr.GT.3)GOTO 10
                           ENDIF
C
                           WRITE(6, 99002)L1Tr, L2Tr
99002                      FORMAT(' XSEC-811.SGMA.L1,L2=', 2I4)
C
                           DO nlr = 1, 2
                              DO ii = 1, iimax
                                 WRITE(6, 99003)
...
c
     ...
     &        jami8, jamit8, jamitw, jamx, jamx8, jamxp1, jamxt8, 
     &        jamxtw, jatw, jb, jbmi, jbmx, jbtw, jc, jcmi, jcmitw, 
     &        jcmx, jcmxtw, jctw, jj1, jj2, jl, jlmmmm, jltohf, jtl, 
     &        jtwmi, jtwmx, kaspar, ktr, la, latw, lb, lbtw, lc, lctw, 
     &        legbas, ll, ll1, ll2, lmpos, loccbc, mbast, mjlbtw, mjtw, 
     &        mlb, mm, mmmax, ms, msmsp, msp, msprg, msptw, msrg, mstw, 
     &        n1, n2, na, namp, njla, njlb, njlb0, njlc, njlc0, nla, 
     &        nlb, nlc
C
C
