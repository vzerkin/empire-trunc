...
C
      INTEGER, PARAMETER :: NGLxx = 100
C
C COMMON variables
C
      REAL*8, DIMENSION(NGLxx) :: ANGler, FAClm, PLM10m, PLM20m, THEta
      REAL*8, DIMENSION(4) :: ARAtio, CE, CFUnir, CHArgr, DFNf, DFNr, 
     &                        DFNsf, DFNsir, DFNspf, DFNspr, DFNsr, 
     &                        DFNwf, DFNwr, DVXf, DVXr, DZEro, ECM, EGS, 
     &                        PMAsr, QVAlue, RMAsr, RZEcf, RZEcr, RZEf, 
     &                        RZEr, RZEsf, RZEsir, RZEspf, RZEspr, 
     &                        RZEsr, RZEwf, RZEwr, SGMazz, TMAsr, VSOf, 
     &                        VSOr, VSXf, VSXr, WN, WNIni, WSFf, WSFr, 
     &                        WSOr, WSXf, WSXr, ZPR, ZTR
      REAL*8, DIMENSION(2) :: C1Mem
      REAL*8 :: CONst1, CONst2, ELAb, ETA, ETUnit, RAC, RD, RHOmx, 
     &          SGMaz, SQRt10, U9, WNUnit, XBAr, XMAx, XMEs
      COMPLEX*16, DIMENSION(NGLxx) :: CSUm2
      REAL*8, DIMENSION(50) :: EXTcom, RACie
      REAL*8, DIMENSION(10) :: EXTcom2
      REAL*8, DIMENSION(500) :: FAClog
      INTEGER, DIMENSION(3) :: ISTw, LDWmxr1
      INTEGER :: JJ, JLSmax, LMAx, MXRow, NANglr, NCHanl, NXCple, NXMax
      INTEGER, DIMENSION(4) :: KCFf, LBTrf, LDWmxr, LTRamx, NNDim, NODf
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      INTEGER, DIMENSION(8) :: KEXcom1, KTLout1, KTRl1
      INTEGER, DIMENSION(28) :: KEXcom2, KTLout2, KTRl2
      INTEGER, DIMENSION(30) :: KTRl
      INTEGER, DIMENSION(120) :: LLRow
      REAL*8, DIMENSION(975,NGLxx) :: P
      REAL*8, DIMENSION(25,NGLxx,2) :: SGMa
      REAL*8, DIMENSION(NGLxx,2) :: SGMat
      COMPLEX*16 :: TTI, TTR, ZERo
      REAL*8, DIMENSION(1000,2) :: WR1
      REAL*8, DIMENSION(5000,2) :: WR2
      COMPLEX*16, DIMENSION(9000,4) :: XAMp
...
C Dummy arguments
C
      REAL*8 :: Apro, Atar, Extcom1, Q2, Q3, Zpro, Ztar
      INTEGER :: Icompff, Iout, Ist, Ktrl3, Ldw, Ltrmax, Ndang
      REAL*8, DIMENSION(Ndang) :: Angle
C
C Local variables
C
      REAL*8 :: ad, ai, amupmu, as, av, aw, dvs, e, ec, fn, mi, mt, rc, 
     &          ri, rs, rv, rw, v, vi, vs, w, wd
      CHARACTER(3) :: ampmwr, holamu, holpmu
      INTEGER :: i, ind, j, kase, kder, maxi, mini, n, n1mx, n1wx, n2mx, 
     &           na, nc, nejc, nlr, no, nw1, nw2, nw3, nz
      REAL*8, DIMENSION(4) :: wsof
      REAL*8, DIMENSION(1000,2) :: xwr1
      REAL*8, DIMENSION(5000,2) :: xwr2
C
...
C
          ...
          CALL OPMPARN(Atar,Ztar,nejc,e,v,dvs,w,wd,vs,vi,av,aw,ad,as,ai,
     &                 rv,rw,RD,rs,ri,rc,mi,mt)
...
C
 ...
 1010   FORMAT('1'//)
        WRITE(8,1020)ELAb
 1020   FORMAT(24X,10('*'),6X,'MULTI-STEP DIRECT-REACTION CALCULATION',
     &         6X,10('*')/39X,'(ON PROGRAM ORION3, Munich/Giessen 1997)'
     &         /47X,'(ELAB=',F8.3,' [MeV])')
      ENDIF
      IF(Iout.GT.3)THEN
        DO n = 1, NCHanl
          WRITE(8,1030)n, TMAsr(n), PMAsr(n), ZTR(n), ZPR(n), LDWmxr(n), 
     &                 ISTw(n)
 1030     FORMAT(/21X,'CHANNEL NO.',I1,6X,'TMAS=',F5.1,2X,'PMAS=',F5.1,
     &           2X,'ZT=',F5.1,'  ZP=',F5.1,3X,'LDWMX=',I2,3X,'ISTW=',
     &           I1/)
          WRITE(8,1140)VSXr(n), DVXr(n), WSXr(n), WSFr(n), VSOr(n), 
     &                 WSOr(n), DFNr(n), DFNwr(n), DFNsr(n), DFNspr(n), 
     &                 RZEr(n), RZEwr(n), RZEsr(n), RZEspr(n), RZEcr(n)
        ENDDO
      ENDIF
      IF(Iout.GT.3)THEN
        WRITE(8,1040)(LTRamx(j),QVAlue(j + 1),j = 1,JLSmax)
 1040   FORMAT(/51X,'TRANSITIONS ARE'/42X,'LTRAMX=',I2,5X,
     &         'QVALUE.(-1)=',F7.3)
        WRITE(8,1050)
 1050   FORMAT(/,48X,'FORM FACTOR PARAMETERS')
      ENDIF
      DO n = 1, JLSmax
        IF(LBTrf(n).GE.0)THEN
          IF(Iout.GT.3)WRITE(8,1060)n, LBTrf(n), NODf(n), EGS(n), 
     &                              DZEro(n)
 1060     FORMAT(/21X,'JLS=',I1/21X,'LBTR=',I2,2X,'NOD=',I2,2X,'B.E.=',
     &           F7.4,3X,'DZERO=',F7.1)
        ELSE
          nc = NODf(n)
          kder = -LBTrf(n)
          IF(Iout.GT.3)WRITE(8,1070)n, nc, kder, KCFf(n), DZEro(n)
 1070     FORMAT(/21X,'JLS=',I1/21X,'NC=',I1,2X,'KDER=',I1,2X,'KCFF=',
     &           I1,5X,'BETA=',F7.4)
        ENDIF
        IF(Iout.GT.3)WRITE(8,1140)VSXf(n), DVXf(n), WSXf(n), WSFf(n), 
     &                            VSOf(n), wsof(n), DFNf(n), DFNwf(n), 
     &                            DFNsf(n), DFNspf(n), RZEf(n), RZEwf(n)
     &                            , RZEsf(n), RZEspf(n), RZEcf(n)
      ENDDO
      IF(Iout.GT.3)WRITE(8,1080)(i,i = 1,28)
 1080 FORMAT(////7X,28I4)
      IF(Iout.GT.3)THEN
        WRITE(8,1090)KTRl2
 1090   FORMAT(' KTRL  ',28I4)
        WRITE(8,1100)KEXcom2
 1100   FORMAT(' KEXCOM',28I4)
        WRITE(8,1110)KTLout2
 1110   FORMAT(' KTLOUT',28I4)
        WRITE(8,1120)EXTcom2
 1120   FORMAT(' EXTCOM',10F10.5)
      ENDIF
      ampmwr = holamu
      IF(amupmu.NE.0.D0)ampmwr = holpmu
      IF(Iout.GT.3)WRITE(8,1130)XMEs, ampmwr
 1130 FORMAT(' XMES =',F7.5,3X,' UNIT =',A3)
...
C-----write results to TAPE15
      ...
      ENDDO
      CLOSE(49)
 1140 FORMAT(21X,'VSX,DVX,WSX,WSF,VSO,WSO',18X,'=',6F8.3/21X,
     &       'DFN,DFNW,DFNS,DFNSP',14X,'=',4F8.3/21X,
     &       'RZERO,RZEROW,RZEROS,RZROSP,RZEROC=',5F8.3)
      END SUBROUTINE ORION
...
C Dummy arguments
C
      REAL*8 :: Ad, Ai, As, Atar, Av, Aw, Dvs, E, Mi, Mt, Rc, Rd, Ri, 
     &          Rs, Rv, Rw, V, Vi, Vs, W, Wd, Ztar
      INTEGER :: Nejc
C
C Local variables
C
      REAL*8 :: ak2, eicms
      INTEGER :: iloc, itmp1, izaf, komp, nnuc
      INTEGER :: INT
C
C*** End of declarations rewritten by SPAG
C
...
C-----calculate o.m. parameters for ejectile NEJC on target NNUC at energy ENER
      ...
      IF(iloc.EQ.1)THEN
        WRITE(8,*)' ORION has been called for the nucleus Z=', INT(Ztar)
     &            , ' A=', INT(Atar)
...
C COMMON variables
C
      REAL*8, DIMENSION(NGLxx) :: ANGler, THEta
      REAL*8, DIMENSION(4) :: ARAtio, CE, CFUnir, CHArgr, DFNr, DFNspr, 
     &                        DFNsr, DFNwr, ECM, PMAsr, QVAlue, RMAsr, 
     &                        RZEcr, RZEr, RZEspr, RZEsr, RZEwr, SGMazz, 
     &                        TMAsr, VSOr, VSXr, WN, WNIni, WSFr, WSXr
      REAL*8 :: ELAb, ETA, ETUnit, RD, RHOmx, SGMaz, WNUnit, XBAr, XMAx, 
     &          XMEs
      COMPLEX*16, DIMENSION(50,3) :: EXSgri
      REAL*8, DIMENSION(50) :: EXTcom
      REAL*8, DIMENSION(70) :: F, FD, G, GD
      REAL*8, DIMENSION(50,3) :: FC, FDC, GC, GDC
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      INTEGER, DIMENSION(30) :: KTRl
      INTEGER, DIMENSION(4) :: LDWmxr, LTRamx, NNDim
      INTEGER, DIMENSION(120) :: LLRow
      INTEGER :: LMAx, MXRow, NANglr, NCHanl, NXCple, NXMax
      COMPLEX*16 :: TTI, TTR, ZERo
      COMMON  F, FD, G, GD
      COMMON /CHANEL/ TMAsr, PMAsr, RMAsr, CHArgr, ARAtio, CFUnir, VSXr, 
     &                WSXr, WSFr, VSOr, DFNr, DFNwr, DFNsr, DFNspr, 
...
C Local variables
C
      REAL*8 :: a1, aovb, charge, denom, dx, e1, einv1, einv2, einv3, 
     &          einv5, einv7, einv9, eta2, eta6, etasq, fl, fmodtp, 
     &          fnxmax, pmas, rmas, sg, sigma0, tmas, vsx, x
...
Cmh         eta2a=2.0*eta
          eta6 = eta2 + 16.0
          sigma0 = ( - (ETA/(12.*eta6))
     &             *(1. + (eta2-48.)/(30.*eta6**2) + ((eta2-160.)
     &             *eta2+1280.)/(105.*eta6**4))) - ETA + (ETA/2.)
     &             *LOG(eta6) + 3.5*ATAN(0.25*ETA)
     ...
     &             - ETA - (0.08333333333*einv1 + 0.00277777777*einv3 + 
     ...
     &             0.00084175084*einv9)
      ...
      XBAr = RZEr(1)*a1
      IF(Iout.GT.3)THEN
        WRITE(8,1010)(ECM(i1),i1 = 1,NCHanl)
 1010   FORMAT(/' ECM   ',6E15.5)
        WRITE(8,1020)(WN(i1),i1 = 1,NCHanl)
 1020   FORMAT(' WN    ',6E15.5)
        WRITE(8,1030)(WNIni(i1),i1 = 1,NCHanl)
 1030   FORMAT(' WNINI ',6E15.5)
        WRITE(8,1040)(CE(i1),i1 = 1,NCHanl)
 1040   FORMAT(' ETA   ',6E15.5)
        WRITE(8,1050)(SGMazz(i1),i1 = 1,NCHanl)
 1050   FORMAT(' SIGM0 ',6E15.5)
        WRITE(8,1060)XMAx, XBAr, NXCple, NXMax, (LDWmxr(n),n = 1,NCHanl)
 1060   FORMAT(' XMAX,XBAR=',2E13.5/' NXCPLE,NXMAX,LDWMXR(N)=',8I5)
      ENDIF
      DO i1 = 1, NCHanl
        LMAx = LDWmxr(i1) + 1
        lmaxm1 = LMAx - 1
...
CBF      MATCHING POINT IN MSTEP IS AT NXMAX-2
 ...
 1070   FORMAT(/)
        DO n = 1, NCHanl
          WRITE(8,1080)n
 1080     FORMAT(' COULOMB FUNCTIONS FOR NO.',I1,' CHANNEL')
          LMAx = LDWmxr(n) + 1
          WRITE(8,1090)(FC(l,n),l = 1,LMAx,2)
 1090     FORMAT(3X,'F ',10E12.4)
          WRITE(8,1100)(GC(l,n),l = 1,LMAx,2)
 1100     FORMAT(3X,'G ',10E12.4)
          WRITE(8,1110)(FDC(l,n),l = 1,LMAx,2)
 1110     FORMAT(3X,'FD',10E12.4)
          WRITE(8,1120)(GDC(l,n),l = 1,LMAx,2)
 1120     FORMAT(3X,'GD',10E12.4)
          WRITE(8,1130)(EXSgri(l,n),l = 1,LMAx,4)
 1130     FORMAT(3X,'EX ',5('(',E10.3,E12.4,'),'))
        ENDDO
      ENDIF
      CALL OMPOTEN
C     REWIND 7
      END SUBROUTINE CCCTRL
...
C
      ...
      REAL*8 :: ELAb, ETA, ETUnit, RD, RHOmx, SGMaz, WNUnit, XBAr, XMAx, 
     &          XMEs
      COMPLEX*16, DIMENSION(70) :: EXSg
      REAL*8, DIMENSION(70) :: F, FP, G, GP, W
      INTEGER, DIMENSION(4) :: LDWmxr, LTRamx
      INTEGER :: LMAx, MXRow, NANglr, NCHanl, NXCple, NXMax
      COMPLEX*16 :: TTI, TTR, ZERo
...
C Local variables
C
      REAL*8 :: accy, an, bn, denom, eta2, etac, ff, fn, fo, fpn, fpo, 
     &          gn, go, gpn, gpo, gs, gs1, gt, gt1, h, hinc, hsq, ps, 
     &          ps1, pt, pt1, r2, r4, ro, ro2, roincx, s, s12, s3, s4, 
     &          sf, sg, sigmao, sp, spf, spg, sumx, sump, t3, t4, test, 
     &          tetao, tra, trb, wr, z1, z2, z3, zl
      REAL*8, DIMENSION(7) :: dvcf
      COMPLEX*16 :: exsgo
      INTEGER :: i, inc, inci, irol, j, j2, l, l1, l2, lf, ll, n, n1, 
     &           ninc, nstep
      INTEGER :: MIN0
C
C*** End of declarations rewritten by SPAG
...
C<<<<<<<<<<<<<       variant A
C                    IERR = 1
                WRITE(8,1010)inc, ro, wr
 1010           FORMAT('0ASYMPT. EXPANS. DOES NOT CONVERGE',I5,2E12.4/)
              ELSE
                inci = inci + 1
                inc = inc + inci
                ro = ro + inci*hinc
          ...
          IF(LMAx.NE.1.OR.inc.NE.0)THEN
            ...
            ro = RHOmx
            irol = 0
            IF(ETA + SQRT(etac + ll*(ll+1)).GT.ro.OR.inc.GT.0)irol = 1
            sumx = 0.
            sump = 0.
            l = 0
            lf = 0
...
C
   25       IF(lf.LT.ll)THEN
              ...
              IF(l.GT.ll)THEN
                ...
                go = gn
                gpo = gpn
                IF(ABS(s/sumx).GE.accy.OR.ABS(sp/sump).GE.accy)THEN
C<<<<<<<<<<<<<          variant A
C                       IF (L .LT. LMX) GO TO 10
                  IF(l.LT.200)GOTO 25
                  WRITE(8,1020)l, go, gpo, sumx, sump
 1020             FORMAT('0NO CONV IN F-RECURRENCE ',I5,4E15.6)
                ENDIF
                go = fo
                gpo = fpo
                l = ll
...
C PARAMETER definitions
C
      INTEGER, PARAMETER :: MAXx = 4000, NGLxx = 100
C
C COMMON variables
C
      REAL*8, DIMENSION(NGLxx) :: ANGler, THEta
      REAL*8, DIMENSION(4) :: ARAtio, CE, CFUnir, CHArgr, DFNr, DFNsir, 
     &                        DFNspr, DFNsr, DFNwr, DVXf, DVXr, ECM, 
     &                        PMAsr, QVAlue, RMAsr, RZEcr, RZEr, RZEsir, 
     &                        RZEspr, RZEsr, RZEwr, SGMazz, TMAsr, VSOr, 
     &                        VSXr, WN, WNIni, WSFr, WSOr, WSXr
      REAL*8 :: ELAb, ETA, ETUnit, RD, RHOmx, SGMaz, WNUnit, XBAr, XMAx, 
     &          XMEs
      REAL*8, DIMENSION(50) :: EXTcom
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      ...
      INTEGER :: LMAx, MXRow, NANglr, NCHanl, NXCple, NXMax
      COMPLEX*16 :: TTI, TTR, ZERo
      REAL*8, DIMENSION(MAXx,3) :: VCEnti, VCEntr, VCOulm, VSPin, VSPini
      COMMON /CHANEL/ TMAsr, PMAsr, RMAsr, CHArgr, ARAtio, CFUnir, VSXr, 
     &                WSXr, WSFr, VSOr, DFNr, DFNwr, DFNsr, DFNspr, 
     &                RZEr, RZEwr, RZEsr, RZEspr, RZEcr, LLRow, NNDim, 
     &                QVAlue, ECM, CE, WN, WNIni, SGMazz
...
C Local variables
C
      REAL*8 :: charge, dfn, dfns, dfnsp, dfnspi, dfnw, dvx, dx, rzero, 
     &          rzeroc, rzeros, rzerow, rzrosi, rzrosp, tmas, vclfc1, 
     &          vclfc2, vso, vspfc, vsx, wsf, wso, wspfc, wsx, x, xbarc, 
     &          xbars, xbarsi, xbarsp, xbarw, xbfac
      INTEGER :: k, n, nx
      REAL*8, DIMENSION(3,5) :: pform
      REAL*8, DIMENSION(4000) :: xmem
C
C*** End of declarations rewritten by SPAG
C
C
...
C           Geometry parameters of imaginary surface potential (k=3) used
        ...
        IF(KTLout(2).EQ.2)THEN
          WRITE(8,1010)n
 1010     FORMAT(/' OPTICAL POTENTIAL FOR CHANNEL NO. ',I1/6X,'R',8X,
     &           'VCENTR',6X,'VCENTI',6X,'VCOULM',6X,'VSPIN',7X,
     &           'VSPINI')
          DO nx = 10, NXMax, 10
            WRITE(8,1020)xmem(nx), VCEntr(nx,n), VCEnti(nx,n), 
     &                   VCOulm(nx,n), VSPin(nx,n), VSPini(nx,n)
 1020       FORMAT(1X,6E12.4)
          ENDDO
        ENDIF
      ENDDO
      END SUBROUTINE OMPOTEN
...
C
      REAL*8, DIMENSION(NGLxx) :: ANGler, THEta
      REAL*8, DIMENSION(4) :: ARAtio, CE, CFUnir, CHArgr, DFNf, DFNr, 
     &                        DFNsf, DFNspf, DFNspr, DFNsr, DFNwf, 
     &                        DFNwr, DVXf, DVXr, DZEro, ECM, EGS, PMAsr, 
     &                        QVAlue, RMAsr, RZEcf, RZEcr, RZEf, RZEr, 
     &                        RZEsf, RZEspf, RZEspr, RZEsr, RZEwf, 
     &                        RZEwr, SGMazz, TMAsr, VSOf, VSOr, VSXf, 
     &                        VSXr, WN, WNIni, WSFf, WSFr, WSXf, WSXr, 
     &                        ZPR, ZTR
      REAL*8 :: ELAb, ETA, ETUnit, RD, RHOmx, SGMaz, WNUnit, XBAr, XMAx, 
     &          XMEs
      REAL*8, DIMENSION(50) :: EXTcom
      INTEGER :: JLSmax, LMAx, MXRow, NANglr, NCHanl, NXCple, NXMax
      INTEGER, DIMENSION(4) :: KCFf, LBTrf, LDWmxr, LTRamx, NNDim, NODf
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      INTEGER, DIMENSION(30) :: KTRl
      INTEGER, DIMENSION(120) :: LLRow
      COMPLEX*16 :: TTI, TTR, ZERo
      REAL*8, DIMENSION(MAXx,2) :: VINti, VINtr
      COMMON /CHANEL/ TMAsr, PMAsr, RMAsr, CHArgr, ARAtio, CFUnir, VSXr, 
     &                WSXr, WSFr, VSOr, DFNr, DFNwr, DFNsr, DFNspr, 
     &                RZEr, RZEwr, RZEsr, RZEspr, RZEcr, LLRow, NNDim, 
     &                QVAlue, ECM, CE, WN, WNIni, SGMazz
...
C Local variables
C
      REAL*8 :: dfn, dfns, dfnw, dvc1, dvc2, dvx, dx, dzr, exr, exs, 
     &          exw, fehler, t1, t2, t3, t4, tmas, vfac1, vfac2, vsx, 
     &          wdfc1, wfac1, wsf, wsx, x, xbars, xbarw, xbfac, xmest
      INTEGER :: i, iffpr, kder, n, n1, n2, nt, nx
      REAL*8, DIMENSION(MAXx) :: xmem
C
C*** End of declarations rewritten by SPAG
C
C
...
C           transfer)
          IF(kder.EQ.3)VINtr(nx,n1) = vfac2*(exr - 1.0)*exr/(1.0 + exr)
     &                                **3
C
C           Real surface contribution is not considered in this case
C           this formfactor is not used within EMPIRE
C           (because only inelastic scattering is calculated with ORION, no
C           transfer)
          IF(kder.EQ.4)VINtr(nx,n1) = vfac1*exr*exw/((1.0 + exr)**2*(1.
     &                                + exw))
C
C           Real surface contribution is not considered in this case
C           this formfactor is not used within EMPIRE
...
C        CLOSE(30)
        IF(KTRl(6).EQ.0)THEN
          DO i = 16, NXMax, 5
            PRINT 1010, xmem(i), (VINtr(i,n),VINti(i,n),n = 1,JLSmax)
 1010       FORMAT(' ',F6.2,8E13.5)
          ENDDO
        ENDIF
      ENDIF
      END SUBROUTINE FFCAL
...
C
     ...
     &                        RZEcr, RZEr, RZEspr, RZEsr, RZEwr, SGMazz, 
     &                        TMAsr, VSOr, VSXr, WN, WNIni, WSFr, WSXr
      COMPLEX*16, DIMENSION(MAXx,2) :: CFOrm
      COMPLEX*16, DIMENSION(42,61) :: CMAt1
      COMPLEX*16, DIMENSION(42,42,61) :: CMAt2
      REAL*8 :: ELAb, ETA, ETUnit, H, HSQ, HTWelv, RD, RHOmx, SGMaz, 
     &          WNUnit, XBAr, XMAx, XMEs
      REAL*8, DIMENSION(50) :: EXTcom
      INTEGER, DIMENSION(3) :: ISTw
      INTEGER :: JJ, LMAx, MXRow, NANglr, NCHanl, NRX, NXCple, NXMax
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      INTEGER, DIMENSION(30) :: KTRl
      INTEGER, DIMENSION(4) :: LDWmxr, LTRamx, NNDim
      INTEGER, DIMENSION(120) :: LLRow
...
C
      REAL*8 :: bova2, dx, x
      COMPLEX*16 :: cmat0
      REAL*8, DIMENSION(3) :: drsqd
      INTEGER :: ii1, ii2, ii3, jatw, jbtw, jbtwmn, jbtwmx, jctw, 
     &           jctwmn, jctwmx, kheq, kieq, l1trx, l1twx, l2trx, l2twx, 
     &           la, latw, lb, lbmax, lbmin, lbtw, lc, lcmax, lcmin, 
     &           lctw, ldwmxa, ldwmxb, ldwmxc, m1, m2, m3, mb, mc, n, 
     &           n1, n2, n3, na, namx, nb, nbmin, nbmx, nc, ncmin, ncmx, 
     &           nct, nx
      INTEGER, DIMENSION(3) :: istp
C
C*** End of declarations rewritten by SPAG
C
C
...
C
                  IF(NCHanl.NE.2)THEN
                    ...
                    jbtwmx = jctw + l2twx
                    nbmin = l2twx - lctw + lbmin + (jbtwmn + 1)/2 + 1
                    nb = nbmin - 1
C
...
C
                    DO lb = lbmin, lbmax
                      ...
                      DO m3 = -ISTw(3), ISTw(3), istp(3)
                        n3 = n3 + 1
                        jbtw = lbtw + m3
                        IF(jbtw.GE.jbtwmn.AND.jbtw.LE.jbtwmx)THEN
                          nb = nb + 1
                          CALL MSTEP(lbtw,jbtw,cmat0,kheq,ii3)
CBF                                 SMAT(LB,N3,II3)=1.+2.*TTI*CMAT0
                          CALL MSTEP(lbtw,jbtw,CMAt2(nb,nc,na),kieq,ii3)
                        ENDIF
                      ENDDO
                    ENDDO
                    IF(KTLout(3).NE.0)WRITE(8,1010)na, nc, nb, 
     &                 (CMAt2(mb,nc,na),mb = nbmin,nb)
C6020                      FORMAT(' REACXS:   ',E13.5,'  (mb)')
 1010               FORMAT(/'NA=',I3,' NC=',I3,' NB=',I3,
...
C
            ...
            IF(KTLout(3).NE.0)THEN
              WRITE(8,1020)na, nc, (CMAt1(mc,na),mc = ncmin,nc)
 1020         FORMAT(/'NA=',I3,' NC=',I3,
     &               ' YIELD CMAT1(1..NC,NA)'/(4(:'(',E11.4,',',E11.4,
     &               ' )',3X)))
C
C                 END OF ONE-STEP
C
...
C
     ...
     &                        RZEcr, RZEr, RZEspr, RZEsr, RZEwr, SGMazz, 
     &                        TMAsr, VSOr, VSXr, WN, WNIni, WSFr, WSXr
      COMPLEX*16, DIMENSION(0:MAXx,3) :: CFH, CFI
      COMPLEX*16, DIMENSION(MAXx,2) :: CFOrm
      COMPLEX*16, DIMENSION(0:MAXx,2) :: CSRc
      COMPLEX*16, DIMENSION(2,3) :: CST
      REAL*8 :: ELAb, ETA, ETUnit, H, HSQ, HTWelv, RD, RHOmx, SGMaz, 
     &          WNUnit, XBAr, XMAx, XMEs
      COMPLEX*16, DIMENSION(50,3) :: EXSgri
      REAL*8, DIMENSION(50,3) :: FC, FDC, GC, GDC
      INTEGER, DIMENSION(4) :: LDWmxr, LTRamx, NNDim
      INTEGER, DIMENSION(120) :: LLRow
      INTEGER :: LMAx, MXRow, NANglr, NCHanl, NRX, NXCple, NXMax
      COMPLEX*16 :: TTI, TTR, ZERo
      REAL*8, DIMENSION(MAXx,3) :: VCEnti, VCEntr, VCOulm, VSPin, VSPini
      REAL*8, DIMENSION(MAXx) :: XSQiv
      COMMON  CFOrm, XSQiv
...
C Local variables
C
      COMPLEX*16 :: c, c1, c2, cdet, cfd, cff, chd, chp, cnrm
      COMPLEX*16, DIMENSION(0:4000) :: cu
      REAL*8 :: eps, fll, fso, ui, ur
...
CB    MATCHINGPOINT IS AT NRM=NRX-2
      nrm = NRX - 2
      DO n = 1, NRX
        ur = VCEntr(n,Nch) + fso*VSPin(n,Nch) + VCOulm(n,Nch) - ECM(Nch)
...
C
     ...
     &                        TMAsr, VSOr, VSXr, WN, WNIni, WSFr, WSXr
      REAL*8, DIMENSION(2) :: C1Mem
      REAL*8 :: CONst1, CONst2, ELAb, ETA, ETUnit, PLM10, PLM20, RAC, 
     &          RADian, RD, RHOmx, SGMaz, SQRt10, U9, WNUnit, XBAr, 
     &          XMAx, XMEs
      COMPLEX*16, DIMENSION(NGLxx) :: CSUm2
      REAL*8, DIMENSION(50) :: EXTcom, RACie
      REAL*8, DIMENSION(500) :: FAClog
      INTEGER, DIMENSION(3) :: ISTw
      INTEGER :: ISTw1, ISTw2, ISTw3, J12mxt, J1Tw, J2Tw, JAS, JBS, JCS, 
     &           JJ, JLMitw, JLMxtw, JLRang, KASe, L12mxm, L1Maxm, L1Tr, 
     &           L1Tw, L2Maxm, L2Tr, L2Tw, LCAltr, LDWmxa, LDWmxb, 
     &           LDWmxc, LMAx, MMXtr, MXRow, NANglr, NCHanl, NXCple, 
     &           NXMax
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      INTEGER, DIMENSION(30) :: KTRl
      INTEGER, DIMENSION(4) :: LDWmxr, LTRamx, NNDim
      INTEGER, DIMENSION(120) :: LLRow
      INTEGER, DIMENSION(40) :: NBSamp
      REAL*8, DIMENSION(975,NGLxx) :: P
      REAL*8, DIMENSION(30) :: PL
      REAL*8, DIMENSION(25,NGLxx,2) :: SGMa
      REAL*8, DIMENSION(NGLxx,2) :: SGMat
      COMPLEX*16 :: TTI, TTR, ZERo
      REAL*8, DIMENSION(1000,2) :: WR1
...
C
      ...
      INTEGER :: istw23, l0posi, l1p1, l1p1mx, l2p1, l2p1mx, lbcp1x, ll, 
     &           llmax, lm10ps, lm20ps, lmm, lmp, lmpos, mm, mm1, n, 
...
C     IF(NCHANL .EQ. 2) L2P1MX = 1
      IF(KTLout(4).NE.0)WRITE(8,1010)
 1010 FORMAT(/' ENTER XSEC')
C
...
C
      IF(KTLout(4).NE.0)WRITE(8,1020)L1Maxm, L2Maxm
 1020 FORMAT('1',47X,' CROSS SECTIONS FOR L1MAX=',I2,' L2MAX=',I2/)
      nanglx = MIN(NANglr,7)
      l1p1mx = L1Maxm + 1
      l2p1mx = L2Maxm + 1
...
C
        IF(KTLout(4).NE.0)THEN
          WRITE(8,1030)
 1030     FORMAT(//44X,10('*'),' ONE-STEP CROSS SECTIONS ',10('*')/)
          IF(KTRl(8).EQ.1)WRITE(8,1040)
 1040     FORMAT(44X,9('*'),'  J1LOW AND J1UP SEPERATED ',9('*')/)
          WRITE(8,1050)(ANGler(n),n = 1,nanglx)
 1050     FORMAT(7X,'L/R L1  J1 ',5X,40(4X,F5.1,4X))
        ENDIF
        DO l1p1 = 1, l1p1mx
          L1Tr = l1p1 - 1
          L1Tw = L1Tr + L1Tr
          DO njl1 = 1, njl1mx
            ...
            IF(J1Tw.GE.0)THEN
              ...
              DO na = 1, NANglr
                ...
                SGMat(na,2) = 0.5*(WR1(nb,1) - WR1(nb,2))/SGMat(na,1)
              ENDDO
              IF(KTLout(4).NE.0)THEN
                WRITE(8,1060)
 1060           FORMAT(' ')
                nlrw = holl
                WRITE(8,1120)nlrw, L1Tr, J1Tw, 
     &                       (WR1(nr,1),nr = nr1,nr1 + nanglx - 1)
                nlrw = holr
                WRITE(8,1120)nlrw, L1Tr, J1Tw, 
     &                       (WR1(nr,2),nr = nr1,nr1 + nanglx - 1)
                nlrw = holx
                WRITE(8,1120)nlrw, L1Tr, J1Tw, 
     &                       (SGMat(na,1),na = 1,nanglx)
                nlrw = hola
                WRITE(8,1070)nlrw, L1Tr, J1Tw, 
     &                       (SGMat(na,2),na = 1,nanglx)
 1070           FORMAT(8X,A1,2I4,'/2',5X,40F13.5)
              ENDIF
            ENDIF
          ENDDO
        ENDDO
...
C
      IF(KTLout(4).NE.0)THEN
        WRITE(8,1080)
 1080   FORMAT(//44X,10('*'),' TWO-STEP CROSS SECTIONS ',10('*')/)
        WRITE(8,1090)(ANGler(na),na = 1,nanglx)
 1090   FORMAT(6X,'L/R L1   L2 ',40(4X,F5.1,4X))
      ENDIF
      DO l1p1 = 1, l1p1mx
        L1Tr = l1p1 - 1
        DO l2p1 = 1, l2p1mx
 ...
 1100       FORMAT(' ')
            nlrw = holl
            WRITE(8,1130)nlrw, L1Tr, L2Tr, 
     &                   (WR2(nr,1),nr = nr1,nr1 + nanglx - 1)
            nlrw = holr
            WRITE(8,1130)nlrw, L1Tr, L2Tr, 
     &                   (WR2(nr,2),nr = nr1,nr1 + nanglx - 1)
            nlrw = holx
            WRITE(8,1130)nlrw, L1Tr, L2Tr, (SGMat(na,1),na = 1,nanglx)
            nlrw = hola
            WRITE(8,1110)nlrw, L1Tr, L2Tr, (SGMat(na,2),na = 1,nanglx)
 1110       FORMAT(7X,A1,I4,I5,1X,40F13.5)
          ENDIF
        ENDDO
      ENDDO
C     REWIND 7
   10 RETURN
 1120 FORMAT(8X,A1,2I4,'/2',5X,40E13.5)
 1130 FORMAT(7X,A1,I4,I5,1X,40E13.5)
 1140 FORMAT(F9.5,8E12.4)
      END SUBROUTINE XSEC
...
C
      REAL*8, DIMENSION(NGLxx) :: ANGler, FAClm, PLM10m, PLM20m, THEta
      REAL*8, DIMENSION(2) :: C1Mem
      REAL*8, DIMENSION(250) :: CLEbmm
      REAL*8 :: CONst1, CONst2, ELAb, ETA, ETUnit, RAC, RD, RHOmx, 
     &          SGMaz, SQRt10, U9, WNUnit, XBAr, XMAx, XMEs
      COMPLEX*16, DIMENSION(NGLxx) :: CSUm2
      REAL*8, DIMENSION(50) :: EXTcom, RACie
      REAL*8, DIMENSION(500) :: FAClog
      INTEGER :: IA, IB, IC, ID, IE, IG, ISTw1, ISTw2, ISTw3, J12mxt, 
     &           J1Tw, J2Tw, JAS, JBS, JCS, JLMitw, JLMxtw, JLRang, 
     &           KASe, L12mxm, L1Maxm, L1Tr, L1Tw, L2Maxm, L2Tr, L2Tw, 
     &           LDWmxa, LDWmxb, LDWmxc, LMAx, MXRow, NANglr, NCHanl, 
     &           NXCple, NXMax
      INTEGER, DIMENSION(50) :: KEXcom, KTLout
      INTEGER, DIMENSION(30) :: KTRl
      INTEGER, DIMENSION(9) :: L9
      INTEGER, DIMENSION(4) :: LDWmxr, LTRamx
      INTEGER, DIMENSION(40) :: NBSamp
      REAL*8, DIMENSION(975,NGLxx) :: P
      REAL*8, DIMENSION(25,NGLxx,2) :: SGMa
...
C Local variables
C
      REAL*8 :: fms, ph, t1, xconst
      INTEGER :: IABS, MIN0
      INTEGER :: ii, iimax, iimin, is3ke3, ji, jl, jla, jlamx, jlb, 
     &           jlbmi, jlbmx, jlmod, jlmttl, jlr, jlrg, jltohf, jx, 
     &           kaspar, ke3, l1, l1p1, l1p1mx, l2, l2p1, l2p1mx, 
     &           lap1mx, lbp1mx, m, mm, mmmax, mrg, ms, n, n1, n1mx, n2, 
     &           na, namp, nampmi, nampmx, njl1, njl1mx, njl2, njl2mx, 
     &           nlr, nrun, nrun0
      COMPLEX*16 :: phlr, xss1, xss2
      REAL*8, DIMENSION(0:10,100) :: xsig1
      REAL*8, DIMENSION(0:10,0:10,100) :: xsig2
C
C*** End of declarations rewritten by SPAG
C
C
        ...
        DO jlb = jlbmi, jlbmx, JBS
          ...
          DO jla = 1, jlamx, JAS
            ...
            IF(ji.LE.jx)THEN
              ...
              IF(n2.NE.jlmttl)WRITE(8,1010)jla, jlb, n2, jlmttl
 1010         FORMAT(//////'   XSEC-63.  JA,JB,N2,JLMTTL=',4I5)
            ENDIF
          ENDDO
        ENDDO
      ENDDO
              ...
              DO njl2 = 1, njl2mx
                ...
                IF(J2Tw.GE.0)THEN
                  ...
                  IF(nampmx.GT.100*100)THEN
                    WRITE(8,*)' sub XSEC - increase XAMP to:', nampmx
                    STOP
                  ENDIF
                  DO namp = 1, nampmx
                    DO n = 1, 4
...
CB                      II=2..JLRANG+1
                  ...
                  DO ii = iimin, iimax
                    ...
                    DO mm = 1, mmmax
                      fms = xconst
                      IF(mm.EQ.1.AND.jlmod.EQ.1)fms = 0.5*xconst
                      ...
                      DO na = 1, NANglr
                        ...
                        DO nlr = 1, 2
                          ...
                          phlr = ph*(0.0,1.0)
                          IF(ISTw3.EQ.0)THEN
                            xss1 = XAMp(namp,1) + phlr*XAMp(namp,2)
                            t1 = fms*(xss1*DCONJG(xss1))
                          ELSEIF(mm.EQ.1)THEN
                            xss1 = XAMp(namp,1) + phlr*XAMp(namp,2)
                            xss2 = XAMp(namp,3) - phlr*XAMp(namp,4)
                            t1 = fms*(xss1*DCONJG(xss1)
     &                           + xss2*DCONJG(xss2))
                          ELSE
                            xss1 = XAMp(namp,1) - phlr*XAMp(namp,2)
     ...
     &                           + xss2*DCONJG(xss2))
                          ENDIF
                          SGMa(ii,na,nlr) = SGMa(ii,na,nlr) + t1
                        ENDDO
                      ENDDO
                    ENDDO
                  ENDDO
...
C
                  IF(L2Tr.EQ.0)THEN
                    ...
                    DO na = 1, NANglr
                      ...
                      ENDDO
                      xsig1(L1Tr,na) = 0.5*(SGMa(1,na,1) + SGMa(1,na,2))
                    ENDDO
                  ENDIF
C
...
C        OPEN (3,FILE = 'xs.d',STATUS = 'unknown')
        DO l1 = 0, L1Maxm
          WRITE(8,1020)l1, l1, (l2,l2 = 0,MIN(9,L2Maxm))
 1020     FORMAT(/' 1- and 2-step Cross Sections for L1:',I3/'   Theta',
     &           5X,'L1:',I2,9(7X,'L2:',I2))
...
C    &              I3/'#  Theta',8X,'L1:',I2,9(10X,'L2:',I2))
          DO na = 1, NANglr
            WRITE(8,1030)ANGler(na), xsig1(l1,na), 
     &                   (xsig2(l1,l2,na),l2 = 0,MIN(9,L2Maxm))
 1030       FORMAT(F8.3,10E12.4)
...
C Local variables
C
      REAL*8 :: a1, a2, ab, b1, b12sjc, b1b2, b2, c1, c2, c3, c5, h12, 
     &          h1p, h2p, hatac1, hatbc2, hatj1, hatj2, hatja, hatjb, 
     &          hatjc, hatl1, hatl2, hatla, hatlb, hatlc, s1, s1p, s2, 
     &          s2p, s5, sga, sjc
      COMPLEX*16 :: ari, cmri, csum1, eri
      REAL :: FLOAT
      INTEGER :: IABS, MAX0, MIN0
      INTEGER :: ie1, ie1max, ie1min, ii, iimax, iimin, ja, jadpmx, 
     &           jami, jamit8, jamitw, jamx, jamx8, jamxp1, jamxt8, 
     &           jamxtw, jatw, jb, jbmx, jbtw, jc, jcmi, jcmitw, jcmx, 
     &           jcmxtw, jctw, jj1, jj2, jl, jlmmmm, jltohf, jtl, jtwmi, 
     &           jtwmx, kaspar, ktr, la, latw, lb, lbtw, lc, lctw, 
     &           legbas, ll, ll1, ll2, lmpos, loccbc, mbast, mjlbtw, 
     &           mjtw, mlb, mm, mmmax, ms, msmsp, msp, msprg, msptw, 
     &           msrg, mstw, n1, n2, na, namp, njla, njlb, njlb0, njlc, 
     &           njlc0, nla, nlb, nlc
C
C*** End of declarations rewritten by SPAG
C
        ...
        DO nlb = 0, ISTw3
          ...
          IF(lb.LE.LDWmxb)THEN
            ...
            DO ja = jami, jamx, JAS
              ...
              jcmxtw = MIN0(jatw + J1Tw,jbtw + J2Tw,2*LDWmxc + ISTw2)
                  ...
                  IF(la.LE.LDWmxa)THEN
                    ...
                    DO jc = jcmi, jcmx, JCS
                      DO nlc = 0, ISTw2
                        lc = jc - 1 + nlc
                        lctw = 2*lc
                        jctw = lctw*(1 - ISTw2) + (2*jc - 1)*ISTw2
                        IF(KASe.EQ.2.OR.KASe.EQ.3)
     &                     ktr = MIN0(jatw + jctw - L1Tw,
     &                     L1Tw - IABS(jatw - jctw))
                        IF(KASe.EQ.4)
     &                     ktr = MIN0(jatw + lctw - J1Tw,J1Tw - 
     &                     IABS(jatw - lctw))
                        IF(ktr.LT.0)EXIT
                        IF(L1Tr.GE.ABS(la - lc).AND.L1Tr.LE.la + lc)THEN
...
CB
                              ...
                              ELSE
                                ...
                                IG = L1Tw
                                CALL RACHLF
                                s1p = 1 - 2*MOD((jatw + jctw + L1Tw)/2,
     &                                2)
                                h1p = hatja*hatjc/hatl1
                                b1 = a1*RAC*s1p*h1p
                              ENDIF
                              njlc = njlc0 + lc + (jctw + 3)/2
                              IF(KASe.NE.2.AND.L2Tr.EQ.0.AND.lb.EQ.lc)
     &                           THEN
                                csum1 = b1*CMAt1(njlc,njla)
                                IF(KASe.EQ.1)csum1 = csum1/hatl1
                              ENDIF
                              IF(NCHanl.NE.2.AND.lb.LE.LDWmxb)THEN
CB
CB                                           CALCULATION OF SECOND STEP PART
CB
                                jtwmi = MAX0(IABS(jatw - jbtw),JLMitw)
                                jtwmx = MIN0(jatw + jbtw,JLMxtw)
                                IF(jtwmi.LE.jtwmx)THEN
                                  ktr = MIN0(jctw + jbtw - L2Tw,
     &                                  L2Tw - IABS(jctw - jbtw))
                                  IF(kaspar.NE.1.OR.ktr.GE.0)THEN
     ...
     &                                  2*MOD((lc + lb - L2Tr)/2,2)
                                      ...
                                      b2 = a2
                                      IF(KASe.EQ.1.OR.KASe.EQ.4)THEN
     ...
     &                                    2)
                                      ...
                                      ENDIF
                                      b1b2 = b1*b2
                                      IF(KASe.EQ.1.OR.KASe.EQ.4)sjc = 1.
                                      IF(KASe.EQ.2.OR.KASe.EQ.3)
     &                                  sjc = 1 - 
     ...
     &                                  IABS(IC - ID)) + 1
                    ...
                    DO ii = iimin, iimax
                      IF(kaspar.EQ.2)THEN
                        ...
                        IF(ii.GT.1)mmmax = (JLMitw + 1)/2 + (ii - 2)
                        jj1 = IABS(jatw - lbtw)
                        IF(ii.EQ.1)jj2 = J1Tw
                        IF(ii.GT.1)jj2 = IABS(J1Tw - J2Tw) + 2*(ii - 2)
                      ...
                      ELSE
                        ...
                        IF(ii.EQ.1)ll2 = L1Tr
                        IF(ii.GT.1)ll2 = IABS(L1Tr - L2Tr) + (ii - 2)
                        mbast = (ll2*(ll2 + 1) - ll1*(ll1 + 1))/2
                      ENDIF
                      IF(mbast.GE.0)THEN
                          ...
                          DO ms = 1, msrg
                            ...
                            IF(IABS(mjlbtw).LE.IC)THEN
                              ...
                              DO msp = 1, msprg
                                ...
                                DO na = 1, NANglr
                                  namp = namp + 1
                                  XAMp(namp,msmsp) = XAMp(namp,msmsp)
     &                              + ari*P(lmpos,na)
                                ENDDO
                              ENDDO
                            ENDIF
...
C
      ...
      REAL*8, DIMENSION(30) :: PL
      REAL*8 :: PLM10, PLM20, RADian
      COMMON /LEGENC/ LCAltr, MMXtr, RADian, PLM20, PLM10, PL
C
C Local variables
C
      REAL*8 :: co, cosab, ct, fl, flm1, flp1, fm, fmp1, pl0, plp10, si, 
     &          twlm1, twlp1
      INTEGER :: k1, m, mmax
      INTEGER :: MIN0
C
...
C
      REAL :: FLOAT
      INTEGER :: iabc, iabmc, iahf, ibcma, ibhf, icamb, ichf, igma, 
     &           igmb, igmc, igtw, k1, k2, k3
      INTEGER :: IABS, MIN0
      REAL*8 :: r1, s1
C
C*** End of declarations rewritten by SPAG
...
C
      ...
      ELSE
        ...
        s1 = -s1
      ENDIF
   10 RAC = s1*SQRT(fn/fd)
      END SUBROUTINE RACHLF
...
C Local variables
C
      REAL*8, DIMENSION(50) :: dfccor, t1fc, t2fc
      REAL*8 :: f1, f2, f3, f4, f5, f6, f7, f8, flabcd, fsq, sumx, t1log, 
     &          t2log
      INTEGER :: i1, i2, i3, i4, iabcd1, iabe, iabep, iabtw, iacf, 
     &           iacfp, iadftw, ibcftw, ibdf, ibdfp, ibea, icde, icdep, 
     &           icdtw, icfa, idec, idfb, ieab, iecd, ieloc, iemax, 
     &           iemin, ier, ierang, ifac, ifbd, nez, nezitw, nezr, 
     &           nezrag, nezxtw, nz, nzmax, nzmin, nzp1x2
      INTEGER :: IABS, MAX0, MIN0
C
C*** End of declarations rewritten by SPAG
C
...
C
     ...
     &        - FAClog(iacfp + 1) - FAClog(ibdfp + 1)) + flabcd
      ...
      DO ier = 1, ierang
        ...
        DO nz = nzmin, nzmax
          nez = nz + ieloc
          sumx = sumx + t1fc(nez)*t2fc(nz)
        ENDDO
        Racie(ier) = dfccor(ier)*sumx
      ENDDO
      END SUBROUTINE RACSIM
