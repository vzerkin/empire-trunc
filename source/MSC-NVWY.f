...
C
      INTEGER, PARAMETER :: NC2 = NDMscs + 2, IV = 25
C
C COMMON variables
C
      INTEGER, DIMENSION(NC2) :: IE
      REAL*8, DIMENSION(0:2*NDMscs + IV,NDLw) :: OMJd
      REAL*8, DIMENSION(NDMscs,NDMscs) :: PIM
      REAL*8, DIMENSION(NC2,2) :: PREp
      REAL*8, DIMENSION(NDEx,NDLw,2,2,NDMscs) :: SCRm
      REAL*8, DIMENSION(NDEx,NDLw,2) :: WP
      REAL*8, DIMENSION(NDEx,NDMscs,3,2) :: YP
      REAL*8, DIMENSION(NDEx,NC2,3,2) :: ZP
      REAL*8, DIMENSION(NDEx,NC2,2) :: ZSUm
...
C Local variables
C
      REAL*8 :: corr, hisr, ps1, ps2, s, smax, smin, tl1, tl2, tl3, tlj, 
     &          xjc, xjr
      REAL :: FLOAT
      INTEGER :: i, ichsp, ier, iermax, ietl, iexc, ip1, ip2, ipar, 
     &           itlc, j, jr, l, lmax, lmaxf, lmin, mul, n
      INTEGER :: MIN0
C
...
C-----------------------factor of 4 missing (it will be applied when PS is calculated)
     ...
     &                  /(1. + tlj*ZSUm(iermax,n + 2,Nejc))**2
                  ps1 = ps1 + 
     &                  (tl1*YP(iermax,n,1,Nejc)*OMJd(IE(n) - 3,jr)
     &                  + tl2*YP(iermax,n,2,Nejc)*OMJd(IE(n) - 1,jr)
     &                  + tl3*YP(iermax,n,3,Nejc)*OMJd(IE(n) + 1,jr))
     &                  *PREp(n,Nejc)*4.
                ENDDO
                ps2 = 0.0
                DO l = lmin + 1, lmax, 2
...
C--------------------do loop over l   ***done***
                SCRm(iermax,jr,ip1,Nejc,n) = SCRm(iermax,jr,ip1,Nejc,n)
     &            + ps1
                SCRm(iermax,jr,ip2,Nejc,n) = SCRm(iermax,jr,ip2,Nejc,n)
     &            + ps2
...
C--------------------IP1 and IP2 decide which parity each SUMTL  goes to
     ...
     &                  /(1. + tlj*ZSUm(ier,n + 1,Nejc))**2
                  tl3 = tlj*ZP(ier,n,3,Nejc)
     &                  /(1. + tlj*ZSUm(ier,n + 2,Nejc))**2
                  ps2 = ps2 + (tl1*YP(ier,n,1,Nejc)*OMJd(IE(n) - 3,jr)
     &                  + tl2*YP(ier,n,2,Nejc)*OMJd(IE(n) - 1,jr)
     &                  + tl3*YP(ier,n,3,Nejc)*OMJd(IE(n) + 1,jr))
     &                  *PREp(n,Nejc)*4.
                ENDDO
C--------------------do loop over l   ***done***
                SCRm(ier,jr,ip1,Nejc,n) = SCRm(ier,jr,ip1,Nejc,n) + ps1
                SCRm(ier,jr,ip2,Nejc,n) = SCRm(ier,jr,ip2,Nejc,n) + ps2
                PIM(n,n) = PIM(n,n) + (ps1 + ps2)
              ENDDO
...
C PARAMETER definitions
C
      INTEGER, PARAMETER :: NC2 = NDMscs + 2, ISD = 0, NG = 4*NDMscs - 
     &                      3*ISD, IV = 25
C
C COMMON variables
C
      REAL*8 :: BMSc, EXTr, G, SIGnx
      REAL*8, DIMENSION(100) :: FACt
      REAL*8, DIMENSION(1) :: GSP
      INTEGER, DIMENSION(NC2) :: IE
      REAL*8, DIMENSION(0:2*NDMscs + IV,NDLw) :: OMJd
      REAL*8, DIMENSION(NDMscs,NDMscs) :: PIM
      REAL*8, DIMENSION(NDMscs) :: PIM1
      REAL*8, DIMENSION(NC2,2) :: PREp
      REAL*8, DIMENSION(NDEx,NDLw,2,2,NDMscs) :: SCRm
      REAL*8, DIMENSION(NDEx,NDLw,2) :: WP
      REAL*8, DIMENSION(NDEx,NDMscs,3,2) :: YP
...
C Local variables
C
      REAL*8 :: accn, atil, ccre, cgd, coulb, det, ecc, ecor, ecor1, 
     &          ecor2, ecor3, efc, eg, egdr, egdrr, emis, en, entl, eps, 
     &          eptl, ew, gamma, gdrw, ggg, omn, omp, pop1, pop2, 
     &          popars, popt, prop, propt, rbu, rog, rop, sdabs, sp, 
     &          spw, sumtlg, tg, tg1, tg2, tg3, tgc, tl1, tl2, tl3, tlg, 
     &          tls, tx, vl, vmn, vu, xga, xxxx, y, yd, ygdr, yre, ysp
      REAL*8, DIMENSION(NG,NG) :: apig, pig, undeg
      REAL*8, DIMENSION(NDMscs,NDMscs) :: apim, undex
      REAL*8, DIMENSION(NDMscs) :: aw, awm, bbb, pimem, popart, spg
      REAL*8, DIMENSION(NDMscs,NDLw) :: awj
      REAL*8 :: E1, OMJ, ROPHM, VQ, W, WILLI, WOBL, WT
      REAL :: FLOAT
      REAL*8, DIMENSION(NDMscs,NDEx) :: gspmsc
      INTEGER :: i, icse, ie1, iec, iex1, iexc, ikc, iloc, inc, ini, 
     &           ipii, irflag, ispin, ix, ixni, izares, izpcon, j, jr, 
     &           jx, k, k1, k2, k3, kc, kc1, kcn, ke, key_tmp, kg, kgin, 
     &           ki, ks, m, m1, m2, nc1, ncm, ncont, nejc, nnur, 
     &           npigzero, nresn, nresp
      INTEGER, DIMENSION(NC2) :: ih, ip
      INTEGER, DIMENSION(NG) :: mpigx, mpigy
      REAL*8, DIMENSION(NG) :: pigx, pigy, spr
      REAL*8, DIMENSION(NC2,4) :: roph
      REAL*8, DIMENSION(NC2) :: roph3, rophc, ropht
      REAL*8, DIMENSION(NDEx) :: sdsp
      REAL*8, DIMENSION(NC2,2) :: xep
C
C*** End of declarations rewritten by SPAG
C
Ccc
...
C-----s.p.l.d. for MSC is set to A/GDIV (default g=A/13)
      ...
      IF(STMro.EQ.1.0D0)THEN
        WRITE(8,
     &'(     '' Microscopic p-h state densities will be calculated withp
     &arameters'')')
        CALL TRATES
        ini = 1000000
      ...
      ENDIF
      Nvwful = .FALSE.
      IF(NOUt.GT.0)WRITE(8,1010)
 1010 FORMAT(1X,//,1X,'Heidelberg M.S.C. (ausiliary output)',//)
      IF(SQRT(1.5D0*G*EXCn) + 6D0.LT.2.0D0*NDMscs + 1.D0)Nvwful = .TRUE.
      IF(Nvwful)WRITE(8,1020)
 1020 FORMAT(1X,
     &'C.N. decay will be totaly calculated in terms of the Heidelberg M
     &.S.C. theory')
      IF(.NOT.Nvwful)WRITE(8,1030)
 1030 FORMAT(1X,
...
C-----peaks is added to the average; this being absolutely arbitrary)
      IF(GDRpar(6,1).NE.0.0D0)gdrw = (GDRpar(2,1)*GDRpar(3,1) + GDRpar(5
     &                               ,1)*GDRpar(6,1))
     &                               /(GDRpar(3,1) + GDRpar(6,1))
...
C-----
      ...
      IF(EX1.EQ.0.0D0.AND.EX2.EQ.0.0D0)THEN
        ...
        ENDIF
      ENDIF
      WRITE(8,1040)xep(1,1), xep(1,2), IE(1)
 1040 FORMAT(1X,'Initial particle type number of excitons for neutrons='
...
C-----
      DO i = 2, NC2
        ...
        ih(i) = ih(i - 1) + 1.
        ip(i) = ip(i - 1) + 1.
        xep(i,1) = xep(i - 1,1) + PREp(i - 1,1)*vl + PREp(i - 1,2)*vu
        xep(i,2) = xep(i - 1,2) + PREp(i - 1,1)*vu + PREp(i - 1,2)*vl
...
C--------calculation of the class population from the initial channel
        IF(aw(k).GT.0.0D0)THEN
          popart(k) = rbu - 1 + WILLI(IE(k),ecor)/W(ip(k),ih(k),ecor)
...
C----------------------
          ...
          IF(ew - ecor2.GT.0.0D0)THEN
            ...
            IF(yre.NE.0D0)ZP(i,k,2,1) = yre*G*G/W(ip(k) - 1,ih(k),ew - 
     &                                  ecor2)/ip(k)
          ENDIF
...
C----------------------
     ...
     &       /(ip(NDMscs) + 1)
      ...
      DO i = 1, NEX(nresn)
        DO k = 3, NC2
          ZSUm(i,k,1) = ZP(i,k,1,1) + ZP(i,k - 1,2,1) + ZP(i,k - 2,3,1)
        ENDDO
...
C----------------------
     ...
     &                    ew - ecor2)/(ip(NDMscs) + 1)
      ...
      DO i = 1, NEX(nresp)
        DO k = 3, NC2
          ZSUm(i,k,2) = ZP(i,k,1,2) + ZP(i,k - 1,2,2) + ZP(i,k - 2,3,2)
        ENDDO
...
C-----
      IF(NOUt.GT.1)THEN
        DO k = 1, NDMscs
          WRITE(8,1050)k, ip(k), ih(k), IE(k), EXCn, awm(k)
 1050     FORMAT(1X,//,'  STEP K=',I3,' P=',I2,' H=',I2,' N=',I2,
     &           ' EXCN=',F8.3,' AWM=',G11.3,//,4X,
     &'Y(.,K,1)  Y(.,K,2)   Y(.,K,3)  Z(.,K,1)   Z(.,K,2)   Z(.,K,3)   Z
     &SUM(.,K)',//)
          WRITE(8,1060)(i,YP(i,k,1,1),YP(i,k,2,1),YP(i,k,3,1),
     &                 ZP(i,k,1,1),ZP(i,k,2,1),ZP(i,k,3,1),ZSUm(i,k,1),
     &                 i = 1,30)
 1060     FORMAT(I3,7G11.3)
        ENDDO
      ENDIF
      DO i = 1, 2
        ipii = -( - 1.0)**i
...
C-----------------ang. mom. coeff. being neglected as they nearly cancel out
              ecor = ((ip(k1)**2 + ih(k1)**2)/4.0 + (ip(k1) - ih(k1))
     &               /4.0 - ih(k1)/2.0)/G
...
C-----------------ang. mom. coeff. being neglected as they nearly cancel out
     ...
     &               /4.0 + (ip(NDMscs) - ih(NDMscs))/4.0 - ih(NDMscs)
     &               /2.0)/G
...
C--------------printout of the pi matrix
            IF(NOUt.GT.2)THEN
              WRITE(8,1070)j, i
 1070         FORMAT(1X,'PIM MATRIX   J=',I2,' I=',I2)
              DO m2 = 1, NDMscs
                WRITE(8,1210)(PIM(m1,m2),m1 = 1,NDMscs)
              ENDDO
              WRITE(8,1220)
            ENDIF
...
C--------------
            ...
            IF(NOUt.GT.2)THEN
              WRITE(8,1080)j, i
 1080         FORMAT(1X,'Inverse PIM matrix   J=',I2,' I=',I2)
              DO m2 = 1, NDMscs
                WRITE(8,1210)(PIM(m1,m2),m1 = 1,NDMscs)
              ENDDO
              WRITE(8,1220)
...
C--------------
            DO k1 = 1, NDMscs
              DO k2 = 1, NDMscs
                undex(k1,k2) = 0.
                DO k3 = 1, NDMscs
                  undex(k1,k2) = undex(k1,k2) + apim(k3,k1)*PIM(k3,k2)
                ENDDO
              ENDDO
            ENDDO
            IF(NOUt.GT.2)THEN
              WRITE(8,1090)j, i
 1090         FORMAT(1X,'Control matrix   J=',I2,' I=',I2)
              DO m2 = 1, NDMscs
                WRITE(8,1210)(undex(m1,m2),m1 = 1,NDMscs)
              ENDDO
              WRITE(8,1220)
            ENDIF
C--------------
...
C--------------
            DO k1 = 1, NDMscs
              spg(k1) = 0.0
              DO k2 = 1, NDMscs
                PIM(k1,k2) = PIM(k1,k2)*POP(NEX(1),j,i,1)*popart(k2)
              ENDDO
            ENDDO
            IF(NOUt.GT.1)WRITE(8,1100)POP(NEX(1),j,i,1)
 1100       FORMAT(1X,'Population to be distributed=',E12.5,' mb',/)
            IF(NOUt.GT.3)WRITE(8,1110)PIM1
 1110       FORMAT(1X,'PIM TO BE USED (NORMALIZED):',/,1X,(12E11.4),/)
C
                ...
                DO kg = 1, NDMscs
                  DO ki = 1, NDMscs
                    DO jr = 1, NLW
                      ...
                      popt = pop1 + pop2
                      IF(icse.GT.1)THEN
                        CSE(icse,nejc,1) = CSE(icse,nejc,1) + popt
                        AUSpec(icse,nejc) = AUSpec(icse,nejc) + popt
                        POPcse(ke,nejc,icse,INExc(nnur))
     &                    = POPcse(ke,nejc,icse,INExc(nnur)) + popt
 
                      ENDIF
                      POP(ke,jr,1,nnur) = POP(ke,jr,1,nnur) + pop1
                      POP(ke,jr,2,nnur) = POP(ke,jr,2,nnur) + pop2
                      emis = emis + popt*DE
...
C--------------proton emission (if accepted, note IF on IDNA)
     ...
     &                    = POPcse(ke,nejc,icse,INExc(nnur)) + popt
            ...
            IF(NOUt.GT.0)WRITE(8,1120)j, i, spg
 1120       FORMAT(1X,'J=',I2,' I=',I1,
     &             '  emission from subsequent stages:',/,(1X,12E11.4),
     &             //)
            IF(NOUt.GT.0)WRITE(8,1220)
C
C************* Gamma emission  ******************************************
C
...
C--------------------is not consistent with the actual level densities.
                ...
                accn = atil*(1 + SHC(1)*(1 - EXP((-gamma*EX(iec,1))))
     &                 /EX(iec,1))
C
...
C                       DO KG = 1, NC2
     ...
     &                     /4.0 + (ip(kg) - ih(kg))/4.0 - (ih(kg) - 1)
     &                     /2.0)/G
                    rophc(kg) = WT(IE(kg) - 2,ip(kg) - 1,ih(kg) - 1,
     &                          egdrr - ecor)
                  ELSE
     ...
     &               *OMJd(IE(kg) - 2,j - 1)
                  ...
                  roph3(kg) = roph(kg,1) + roph(kg,2) + roph(kg,3)
...
C--------------------particle emission in diagonal elements
                ...
                DO kg = kgin, NDMscs
                  DO kc = 1, 4
                    ...
                    pig(ikc,ikc) = pimem(kg)*roph(kg,kc)/ropht(kg)
                    IF(pig(ikc,ikc).EQ.0.0D0)npigzero = npigzero + 1
                  ENDDO
                ENDDO
...
C--------------------------summing the matrix elements on the right
                    pig(1,1) = pig(1,1) - pig(1,2) - pig(1,3) - pig(1,4)
     &                         - pig(1,5)
...
C--------------------------cancel out
     ...
     &                     /4.0 + (ip(kg) - ih(kg))/4.0 - ih(kg)/2.0)/G
...
C--------------------------omitted in VMN
                    ...
                    ygdr = WOBL(ip(kg),ih(kg) - 1,egdrr - ecor,0)
     &                     /WOBL(ip(kg),ih(kg) - 1,EXCn - ecor,0)
                    ecor = (((ip(kg)-1)**2 + (ih(kg)-1)**2)
     &                     /4.0 + (ip(kg) - ih(kg))/4.0 - (ih(kg) - 1)
     &                     /2.0)/G
                    CALL GDOWN(y,ip(kg) - 1,ih(kg) - 1,egdrr - ecor)
C--------------------------Y should be multiplied by G**3 but omitted since
C--------------------------omitted in VMN
                    IF(kg.GT.1)THEN
                      ysp = y/aw(kg - 1)
...
C--------------------------omitted in VMN
                    ...
                    ygdr = WOBL(ip(NDMscs),ih(NDMscs) - 1,egdrr - ecor,
     &                     0)
     &                     /WOBL(ip(NDMscs),ih(NDMscs) - 1,EXCn - ecor,
     &                     0)
...
C--------------------------omitted in VMN
                    ...
                    cgd = -6.28319*vmn*ysp
                    DO kc1 = 1, 3
                      pig(ix,ix) = pig(ix,ix) + 6.28319*roph(NDMscs,4)
     &                             *ygdr*gdrw*D1Fra
                      pig(ix + kc1 - 4,ix + kc1 - 4)
     &                  = pig(ix + kc1 - 4,ix + kc1 - 4)
...
C                          WRITE(8,'('' 2,.-->2,4, VERT. COL.'',G12.5)')PROPT
                    ...
                    propt = prop*gdrw*D1Fra*G/WT(5,3,2,EXCn - ecor)
...
C-----------------------printout of the PIG matrix
                  IF(NG.LE.12.AND.NOUt.GT.2)THEN
                    WRITE(8,1130)j, i
 1130               FORMAT(1X,'PIG matrix   J=',I2,' I=',I2)
                    DO m2 = 1, NG
                      WRITE(8,1230)(pig(m2,m1),m1 = 1,NG)
                    ENDDO
                    WRITE(8,1220)
                  ENDIF
C-----------------------
...
C                       CALL MATIN1(pig,ggg,NG,1,det)
                  eps = 1.0D-8
                  CALL MTXINV(pig,pigx,pigy,mpigx,mpigy,NG,eps,irflag)
                  IF(irflag.NE.0)THEN
...
C-----------------------printout of the inverted PIG matrix
                  IF(NG.LE.12.AND.NOUt.GT.2)THEN
                    WRITE(8,1140)j, i
 1140               FORMAT(1X,'Inverse PIG matrix   J=',I2,' I=',I2)
                    DO m2 = 1, NG
                      WRITE(8,1230)(pig(m2,m1),m1 = 1,NG)
                    ENDDO
                    WRITE(8,1220)
                  ...
                  IF(NOUt.GT.2)THEN
                    DO k1 = 1, NG
                      DO k2 = 1, NG
                        undeg(k1,k2) = 0.0
                        DO k3 = 1, NG
                          undeg(k1,k2) = undeg(k1,k2) + apig(k3,k1)
     &                      *pig(k3,k2)
                        ENDDO
                      ENDDO
                    ENDDO
                    IF(NG.LE.12)THEN
                      WRITE(8,1150)j, i
 1150                 FORMAT(1X,'Control PIG matrix   J=',I2,' I=',I2)
                      DO m2 = 1, NG
                        WRITE(8,1230)(undeg(m2,m1),m1 = 1,NG)
                      ENDDO
                    ELSE
                      WRITE(8,1160)j, i
 1160                 FORMAT(1X,
     ...
     &                  ,I2,'D I=',I2,/)
                      WRITE(8,1210)(undeg(m1,m1),m1 = 1,NG)
                    ENDIF
                    WRITE(8,1220)
                  ENDIF
C-----------------------
...
C-----------------------
                  DO k1 = 1, NG
                    spr(k1) = 0.0
                    IF(ISD.EQ.1)pig(1,k1) = pig(1,k1)*POP(NEX(1),j,i,1)
     &                 *popart(1)*roph(1,4)/ropht(1)
                    DO k3 = kgin, NDMscs
                      k2 = (k3 - kgin)*4 + ISD
                      pig(k2 + 1,k1) = pig(k2 + 1,k1)*POP(NEX(1),j,i,1)
     &                                 *popart(k3)*roph(k3,1)/ropht(k3)
                      pig(k2 + 2,k1) = pig(k2 + 2,k1)*POP(NEX(1),j,i,1)
     &                                 *popart(k3)*roph(k3,2)/ropht(k3)
                      pig(k2 + 3,k1) = pig(k2 + 3,k1)*POP(NEX(1),j,i,1)
     &                                 *popart(k3)*roph(k3,3)/ropht(k3)
                      pig(k2 + 4,k1) = pig(k2 + 4,k1)*POP(NEX(1),j,i,1)
     &                                 *popart(k3)*roph(k3,4)/ropht(k3)
                    ENDDO
                  ENDDO
                  IF(NOUt.GT.3)THEN
                    WRITE(8,1170)(pig(1,k1),k1 = 1,NG)
 1170               FORMAT(1X,'Normalized PIG to be used ',/,
     &                     (1X,12E11.4))
                    WRITE(8,1220)
                  ENDIF
C-----------------------
...
C--------------------------------propagator
                        prop = 1./
     &                         (egdrr**2 + 0.25*(gdrw*D1Fra + gdrw*(1-
     &                         D1Fra)*.75)**2)
C--------------------------------absorption cross section times propagator
                        sdabs = popart(1)*prop*POP(NEX(1),j,i,1)
     &                          *0.25/awj(1,j)
                        WRITE(8,
...
C--------------------------------state density for gamma emission
     ...
     &                           /4.0 + (ip(kg) - ih(kg))
     &                           /4.0 - (ih(kg) - 1)/2.0)/G
                          rog = WT(IE(kg) - 2,ip(kg) - 1,ih(kg) - 1,
     &                          ew - ecor)*DE
                        ELSE
                          ncont = 10000 + (ip(kg) - 1)*100 + ih(kg) - 1
     ...
     &                                  *OMJd(IE(kg) - 2,j - 1)
                        tg2 = tgc*roph(kg,2)*OMJd(IE(kg) - 2,j)
                        tg3 = tgc*roph(kg,3)*OMJd(IE(kg) - 2,j + 1)
                        DO k1 = 1, NG
                          ix = (kg - kgin)*4 + 1 + ISD
                          sp = pig(k1,ix)*tg1/DE
                          spr(ix) = spr(ix) + sp
     ...
     &                        = POPcse(kcn,0,ks,INExc(1)) + sp
                          ENDIF
                          gspmsc(kg,ks) = gspmsc(kg,ks) + sp
                          POP(kcn,j,ie1,1) = POP(kcn,j,ie1,1) + sp
                          emis = emis + sp*DE
                          ix = ix + 1
                          sp = pig(k1,ix)*tg3/DE
                          spr(ix) = spr(ix) + sp
     ...
     &                        = POPcse(kcn,0,ks,INExc(1)) + sp
                          ENDIF
                          gspmsc(kg,ks) = gspmsc(kg,ks) + sp
                          POP(kcn,j + 1,ie1,1) = POP(kcn,j + 1,ie1,1)
     &                      + sp
                          emis = emis + sp*DE
                        ENDDO
                      ENDIF
...
C-----------------------t gamma gamma estimate done
                  IF(NOUt.GT.0)THEN
                    WRITE(8,1220)
                    IF(ISD.EQ.1)WRITE(8,1180)j, i, spr
 1180               FORMAT(1X,//,1X,'J=',I2,' I=',I1,
     &                     ' Gamma emission from subsequent stages:',/,
     &                     1X,E11.4,/,(1X,4E11.4))
                    IF(ISD.EQ.0)WRITE(8,1190)j, i, spr
 1190               FORMAT(1X,//,1X,'J=',I2,' I=',I1,
     &                     ' GAMMA EMISSION FROM SUBSEQUENT STAGES:',/,
     &                     (1X,4E11.4))
                  ENDIF
                ENDIF
            ...
            ENDIF
          ENDIF
    5     IF(POP(NEX(1),j,i,1).GT.0)REDmsc(j,i)
...
C-----
      WRITE(8,1200)NDMscs
 1200 FORMAT(1X,//,30X,'H e i d e l b e r g  M. S. C.  d e c a y  (',I2,
     &       ' stages)',//)
...
C     CALL AUERST(1, 2, 0)
     ...
     &           '(2X,A2,'' MSC emission cross section'',G12.5,'' mb'')'
     &           )SYMbe(2), CSMsc(2)
 1210 FORMAT(1X,12E11.4)
 1220 FORMAT(1X,/)
 1230 FORMAT(1X,12E11.4)
      END SUBROUTINE HMSC
