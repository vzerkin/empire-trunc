...
C-----------levels for nucleus NNUC copied to file *.lev 
     ...
     &                  chelem, izr 
                  WRITE(6, '('' WARNING: EXCITED STATE '',I3, 
     ...
     &                     '' HAS GOT ZERO EXCITATION ENERGY'')')ilv 
                  ...
                  IF(ilv.EQ.1 .AND. ELV(ilv, Nnuc).GT.4.)THEN 
                     ...
                     WRITE(6, '('' WARNING: ELEMENT ='',A5,2x,2HZ=,I3)') 
     &                     chelem, izr 
                     WRITE(6, '('' WARNING: EXCITED STATE No.'',I3, 
     &                     '' HAS ENERGY OF '',F6.3,'' MeV'')')ilv, 
     &                     ELV(ilv, Nnuc)  
                  ENDIF 
                  IF(ndbrlin.GT.NDBR)THEN 
                     ...
                     WRITE(6, '('' WARNING: ELEMENT ='',A5,2x,2HZ=,I3)') 
     &                     chelem, izr 
                     WRITE(6, '('' WARNING: TOO MANY GAMMA DECAYS ='', 
     &                     I3)') ndbrlin 
                     WRITE(6, '('' WARNING: DIMENSION ALLOWS FOR ='', 
     &                     I3)') NDBR 
...
C----- 
     ...
     &              '('' Microscopic HFBCS level densities selected'')') 
            GOTO 100 
         ENDIF 
C----- 
...
C        checking for fission data in the optional input 
         IF(name.EQ.'SUBEFF')THEN 
            ...
            IF(iloc.EQ.1)THEN 
               WRITE(6, '('' NUCLEUS '',I3,A2,'' NOT NEEDED'')')i2,  
     &               SYMb(nnuc) 
               WRITE(6, '('' SUBEFF SETTING IGNORED'')') 
               GOTO 100 
            ENDIF 
            SUBeff(nnuc) = val 
            WRITE(6,  
     &            '('' SUBEFF  in '',I3,A2,'' set to ''          ,F6.3)' 
     &            )i2, SYMb(nnuc), val 
            GOTO 100 
         ENDIF 
C------------------------------------------------------------------------- 
...
C--------determination) 
 ...
 100     WRITE(6, *)'ZA=', zr*1000 + ar, ' NNUC=', nnur 
         WRITE(6, *)'BNDG: NUCLEUS NOT FOUND. EXECUTION STOPPED!!!' 
         STOP 
      ENDIF 
...
C     DOUBLE PRECISION r0, mm2,enh_ld
C     DOUBLE PRECISION A2, A23, ACR, ACRt, AP1, AP2, ATIl, BF, DEL, 
C    &                 DELp, DETcrt, ECOnd, GAMma, SCR, TCRt, UCRt
      DOUBLE PRECISION enh_ld, ACR, ACRt, ATIl,   
     &                 DETcrt, ECOnd, SCR, TCRt, UCRt
 
C     INTEGER ka, kz, NLWst, nrbarc
      INTEGER ka, kz, nrbarc
...
C     To be used with care, model dependent !!!!! 
 200  IF(FISbar(Nnuc).EQ.1.)THEN 
     ...
     &        STATUS = 'OLD') 
         ...
         READ(52, '(a100)')simb 
         READ(52, '(a100)')simb 
 250     READ(52, '(2(1x,i3),1x,a2,2(f8.3,4a),f9.3)', END = 300)kz,  
     &        ka, simb, EFB(1), symma, EFB(2), symmb, deltaf 
...
C     Default value for curvatures and protection !! 
 600  DO I=1,NFwells 
      ...
      ENDDO 
C 
      IF(NRWel.NE.(NRBar-1)/2) SUBeff(Nnuc) = 0.  
      NRBarc=NRBar-NRWel  
...
C------- writing data in FISSION.INP 
     ...
     &    hc      Vi      hi      Vo      ho        (in Mev) ' 
      ...
      ENDIF  
C 
      IF(SUBeff(Nnuc).EQ.0.)cara1 = 'Subbarrier effects not considered' 
      IF(SUBeff(Nnuc).EQ.1.)cara1 = 'Subbarrier effects considered' 
      WRITE(79, '(a8,f2.0,a36)')'SUBEFF=', SUBeff(Nnuc), cara1 
      WRITE(79, *)' '  
 
      IF(SUBeff(Nnuc).EQ.1.)THEN 
         WRITE(79,*) '  ' 
         WRITE(79,'(a161)') 'Parabolic energy dependent imaginary potent 
     &ial in the isomeric well Wimag = W0 + W1*E + W2*E^2, where E is ex
     &citation energy with respect to the isomeric well'          
         wimag(1)=1. 
         wimag(2)=0. 
         wimag(3)=0. 
         WRITE(79,*) '      W0         W1         W2' 
...
c         the fission level densities;   
      ...
      ENDDO           
      WRITE(79,*) '   '      
      WRITE(79,'(a87)')'Coefficients of a linear energy dependent factor 
     & adjusting the fission level densities' 
      WRITE(79,'(a87)')'(a0 + a1*E + a2*E**2) * rho(E,J) where E stands  
     &for excitation energy above the barrier' 
      WRITE(79,*) '                a0       a1       a2 ' 
      DO nr=1, NRBarc 
...
C      
C     DOUBLE PRECISION A2, A23, ACR, ACRt, AP1, AP2, ATIl, BF, DEL, 
C    &     DELp, DETcrt, ECOnd, enh_ld, GAMma, SCR, TCRt, UCRt
      DOUBLE PRECISION ACR, ACRt, ATIl,   
     &     DETcrt, ECOnd, enh_ld, SCR, TCRt, UCRt
        
C     INTEGER NLWst, nrbarc,iz,ia
      INTEGER nrbarc,iz,ia 
...
C      
 ...
 1560 READ(79,'(A8)')cara8 
     ...
     &        (EFB(i), H(i), i = 1, NRBar)  
         READ(79, '(a8,f2.0,a36)')cara8, SUBeff(nnuc), cara1 
         READ(79, *) 
C      
         IF(SUBeff(Nnuc).EQ.1.)THEN 
            READ(79,*)  
         ...
         DO ibar = 1, NRBar 
            READ(79, '(a39,I2,a2,I2)')line, ibaro, cara8,  
     &           NRFdis(ibar) 
            READ(79, *) 
            DO nr = 1, NRFdis(ibar) 
...
c        READ(79, *) !!!!!!!!!   
         ...
         READ(79,*)   
         DO nr=1, NRBarc 
            READ(79,'(1x, A8, 1x, I1, 3f9.3)') cara8, i,  
     &           enh_ld(1,nr),enh_ld(2,nr),enh_ld(3,nr)  
         ENDDO 
      ELSE 
         GOTO 1560         
...
C 
     ...
     &                              /(1.D0 + (HO(k)/HO(k+1))**2))) 
     &                              /(SMIu*HO(k)) 
C        Below according to my old coding, check this please
C        RCN 01/2004 there is no ABS function !!!!!
C        IF(k.LT.NRBar)EJOin(2*k) = EPSil(k)
C    &                              + SQRT(( - 1)**k*(VJJ(k+1) - VJJ(k))
C    &                              /(1.D0 + (H(k)/H(k+1))**2))
C    &                              /(SMIu*H(k))
...
C 
c------please find a smarter way to define excn1  
      IF(Nnuc.eq.1)excn1=excn 
      IF(Nnuc.eq.2)excn1=excn-Q(1,1) 
      IF(Nnuc.eq.3)excn1=excn-Q(1,1)-Q(1,2)  
      bet2=deffis(ibars) 
      xminn(ibars)=0.0001 
      DO nr=1,NRfdis(ibars) 
         IF(EFDis(nr,ibars).GT.xminn(ibars)) 
     &      xminn(ibars)=EFDis(nr, ibars) 
      ENDDO    
      IF(excn1.GT.efb(ibars))THEN 
         xmax=excn1-(efb(ibars)+xminn(ibars))+5. 
      ELSE 
         xmax=5. 
      ENDIF   
      destepp=(xmax-xminn(ibars))/100. 
c      destepp=0.2   !!!!! 
      nrbinfis(ibars)=int((xmax-xminn(ibars))/destepp)  
      IF(FISden(Nnuc).eq.0.)THEN 
         FISCON=2. 
