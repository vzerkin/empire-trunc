...
C--------------retrieval from the remote database
     ...
     &//'-0" -Rea ct:"'//proj//',*"'//' -quant:"CS;DA;DAE;DE"#'
...
C
     ...
     &      .GT.1.0D0 .AND. AEJc(0).LE.1.D0)THEN
         ...
         IF(DIRect.GT.0 .AND. INT(AEJC(0)).EQ.0)THEN
            DIRect = 0
            WRITE(6, *)' '
            WRITE(6, *)' WARNING!!!! Direct mechnism is not supported '
...
C           IF(i21p.EQ.0 .AND. xjlvr.EQ.2.D0 .AND. lvpr.EQ.1)THEN
            IF(i21p.EQ.0 .AND. xjlvr.EQ.2.D0 .AND. lvpr.EQ.1 .AND. 
     &         DIRect.NE.3)THEN
               i21p = ilv
               ND_nlv = ND_nlv + 1
               ICOllev(ND_nlv) = ilv
...
C           IF(i4p.EQ.0 .AND. xjlvr.EQ.4.D0 .AND. lvpr.EQ.1)THEN
            IF(i4p.EQ.0 .AND. xjlvr.EQ.4.D0 .AND. lvpr.EQ.1 .AND. 
     &         DIRect.NE.3)THEN
               i4p = ilv
               ND_nlv = ND_nlv + 1
               ICOllev(ND_nlv) = ilv
...
C           IF(i0p.EQ.0 .AND. xjlvr.EQ.0.D0 .AND. lvpr.EQ.1)THEN
            IF(i0p.EQ.0 .AND. xjlvr.EQ.0.D0 .AND. lvpr.EQ.1 .AND. 
     &         DIRect.NE.3)THEN
               i0p = ilv
               ND_nlv = ND_nlv + 1
               ICOllev(ND_nlv) = ilv
            ...
            IF(DIRect.EQ.3)THEN
               IF(i20p.NE.0 .AND. i3m.NE.0)THEN
                  ...
                  GOTO 600
               ENDIF
            ELSEIF(i20p.NE.0 .AND. i21p.NE.0 .AND. i4p.NE.0 .AND. 
